...
formal. Die implementierte Grenze ergänzt den Effectport um die erforderliche
synchrone Notification `observationClosed()` nach `O0` und vor Cleanup: zwei
getrennte Capabilityrollen, einmaliger Konsum auch nach Exchange-Portschluss,
fail-closed Fehler-/Reentranzbehandlung und unvergrößerter Testexportzugang.
Die netzwerkfreie Foundationanpassung ist mit 595/595 fokussierten Tests
(`Δ = 173` gegenüber der historischen 422er-Basis) geprüft. 27 neue
Notificationmutanten und vier Deadlinevarianten werden erkannt; die 18
Joinfälle sowie das Drei-Microtask-Präfix mit strukturellem Pending-Oracle
bleiben erhalten. Setup und Cleanup zählen am Dequeue-Envelope `get`,
`getPrototypeOf`, `ownKeys` und `getOwnPropertyDescriptor` getrennt bei
`deadline-1`, `deadline` und `deadline+1`: unterhalb ist Reflection erreichbar,
bei Gleichheit und Überschreitung bleiben alle vier Traps null. Die bestehenden
Regressionen bestehen mit 423/423, 466/466 und 735/735 Tests. Die vollständige
serielle Gesamtsuite besteht mit 2350/2350, exakt `1755 + 595`, bei jeweils
0 Fehlschlägen, Cancellations, Skips und Todos. Der Produktions-Build
transformiert exakt 46 Module; `bundle:n8n:check` besteht driftfrei. Der rohe
Schutz-/Hashaudit bestätigt die übrigen 14 gebundenen Dateien gegen Baseline
...
oder Transportdependency und ist nun ausschließlich über die kontrollierte
ADR-0025-Produktionskomposition mit dem lokalen SyncGateway verbunden; der
Browserpfad bleibt unkomponiert. Seine gezielte Suite besteht mit 103/103
Tests, die
...
Entscheidungsgate vor der ersten operativen Komposition. Die kontrollierte
Komposition ist nun ausschließlich im bestehenden lokalen Gateway-Prozess
implementiert: `server/startLocalSyncGateway.js` erzeugt genau eine lokale
SyncAgent-Instanz pro HTTP-Server-Factory und injiziert sie als erforderliche
Dependency. Nur die von der Boundary akzeptierte defensive Sechs-Felder-
Requestidentität darf `processSyncRequest` synchron, mit exakt einem Argument
und höchstens einmal erreichen. Das Agentenresultat bleibt unvertrauenswürdig;
nur der exakte ADR-0024-Erfolg darf nach vollständiger Originalprüfung in einen
frischen, erneut validierten und tief eingefrorenen normalen Responsegraphen
projiziert und allein vom Gateway serialisiert werden. Ein exakter Erfolg endet
lokal mit HTTP `200`; beherrschte Agenten-, Projektions- oder terminale Fehler
enden statisch redigiert mit HTTP `500`.

Die fokussierte Local-SyncGateway-Suite besteht nach dieser Komposition mit
...
Pflichtbeobachtungen sind an ein vollständiges unveränderliches Basistupel
`T₀` gebunden. Die beiden Negativvektoren verwenden ausschließlich ihre
geschlossen allowlisteten Tupel `T_origin = T₀ + Δ_origin` und
`T_redirect = T₀ + Δ_redirect`; jede weitere Abweichung bleibt `UNPROVEN` oder
...
bleiben geschlossen. Der Befund wird nicht rückwirkend neu bewertet; seine
Ursache bleibt `CAUSE_NOT_PROVEN`.

ADR 0030 ergänzt ADR 0028 und ADR 0029, ersetzt keinen ADR und bewertet ADR
0020 ausdrücklich erneut, ohne dessen Produktions-Gateway-Baseline zu ändern.
ADR 0020 sowie ADR 0026 bis ADR 0029 und der historische Evidence-Record
bleiben bytegleich. Der neue Diagnosepfad ist weder vierter ADR-0029-Vektor
noch zusätzliches Gate oder Erweiterung des `BrowserRuntimeEvidenceRecord`.

Die Diagnosebindung lautet ausschließlich `T_replay ≡R T₀` und
`T_diag = T_replay + Δ_observer`. `T_replay` ist eine vollständig neue
Referenzbindung vor dem späteren einzelnen Transportstimulus und kein
observerfreier Kontrolllauf. Neue Run-ID, Messzeit, Repositorycommit und
Wegwerfprofilinstanz werden separat gebunden und sind kein Observerdelta. Die
Relation vergleicht nur tatsächlich persistierte historische Klassifikationen
und aus dem historischen Git-Tree ableitbare Einzelartefakthashes. Jede
fehlende, mehrdeutige oder unerklärte kausal relevante Abweichung hält den
Ursachenstatus `CAUSE_NOT_PROVEN`.
...
Network-Operations-Allowlist, eine Main-World-Auswertung, eine argumentlose
Factoryerzeugung und einen gültigen synthetischen v1-`syncTest` begrenzt. Der
Observer dokumentiert nur das öffentliche Promise-Settlement und sanitierte
Netzwerkmetadaten als geschlossene primitive Werte. Sourceinstrumentierung,
Composition-Seams, Runtimeoberflächenmutation, Fetch-Interception, Debugger,
Profiler, Tracing, Responsebody-Lesen, freie Rohinspektion, Zusatzrequests und
Observerausgabe während des Laufs bleiben verboten. Absolute
Observerneutralität wird nicht behauptet; `internalStage` und `internalOwner`
bleiben `unknown`.

Der unabhängige `BrowserTransportDiagnosticRecord` besitzt die getrennten
Achsen `observerGate`, fünf geschlossene Findings und ausnahmslos
`causeStatus: CAUSE_NOT_PROVEN`. Er ändert das ADR-0029-`overallGate: FAIL`
nicht. ADR 0030 ist ein reiner Dokumentationsslice und autorisiert weder
Foundationimplementierung noch Diagnoselauf. In diesem inzwischen historischen
Stand sollte als Nächstes ausschließlich die netzwerkfreie Implementierung und
Prüfung der passiven Diagnosefoundation folgen.
...
Der ADR-0027-Entscheidungsstand bleibt als historische Ebene unverändert
dokumentiert.

ADR 0032 ersetzt ADR 0031 formal und übernimmt alle nicht ausdrücklich
korrigierten ADR-0030-/ADR-0031-Regeln. ADR 0030 bleibt historisch durch ADR
0031 ersetzt; ADR 0031 bleibt mit bytegleichem Hauptteil ab `## Kontext` als
historische Entscheidungsebene erhalten. ADR 0032 ist am `2026-09-02`
angenommen, aber weder Diagnosefoundation noch Adapter oder Runtimevorgang sind
implementiert oder autorisiert. Schema 1 bleibt ohne Versionswechsel nur
deshalb exakt fortführbar, weil weder eine Implementierung noch eine konforme
persistierte Instanz existiert. Recordtyp, 17 Rootfelder, sechs CDP-Operationen,
17 Integritätschecks, neun Requestzähler plus Sequenz, zehn
Stages, drei Clock-Domänen, exakt 20 Cleanup-IDs und fünf Findings bleiben
unverändert. Das ADR-0029-Gesamtgate bleibt vor und nach einer Diagnose `FAIL`;
`causeStatus` bleibt ausnahmslos `CAUSE_NOT_PROVEN`.

Der globale Setupcap beträgt getrennt exakt `6.000 ms`
(`setupWindowMilliseconds = 6000`). Vor dem einzigen Sendeversuch gilt exakt:

```text
Cap-/Clock-Fähigkeit validieren und scharf schalten
→ m_setup exakt einmal controller-monoton erfassen
→ t_setup := 0 und Deadline m_setup + 6000 armieren
→ Stage 1 observer-armed setzen
→ Target.getTargets nichtblockierend senden
```

Der konkrete Timer ist damit vor dem Send aktiv. Scheitert die Cap-/Clock-
Fähigkeit, entsteht `V` oder kein gültig gestarteter Diagnoseversuch. Die pure
Foundation erzeugt nur einen nichtblockierenden Command-Intent und besitzt
keine reale Timer- oder Clockfähigkeit. Der Setup führt `Target.getTargets`,
`Target.attachToTarget` und `Network.enable` strikt sequenziell mit höchstens
einem ausstehenden Kommando aus; der globale Cap wird nicht pro Kommando
zurückgesetzt.

Für jede antwortartig eingereihte Nachricht wird beim Dequeue vor jeder
Reflection genau einmal `m_answer` erfasst und roh
`d := m_answer - m_setup` gebildet. Nur `d < 6000` darf synchron und begrenzt
gegen die geschlossene Setupantwort-Grammatik aus ADR 0032 und dem Living
Contract geprüft werden. Bei `d >= 6000` gewinnt der Setupcap und der
Nachrichteninhalt bleibt ungelesen. Eine ungültige, negative, rückläufige oder
werfende Clock sowie jede zweite Abtastung sind verboten; ein bestätigter
Clockverstoß ist `V`. Erst drei eindeutig korrelierte, erfolgreiche und
vollständig validierte Antworten setzen `setupReady`. Bei `Target.getTargets`
werden zunächst alle `page`-Einträge mit exakt gebundener URL unabhängig von
`attached` gezählt; nur bei genau einem Kandidaten wird `attached === false`
geprüft und erst danach dessen `targetId` gebunden. Persistierte Rundung
beeinflusst diese rohe Capentscheidung nie, und späte Antworten werden
verworfen. Eine Antwort gilt nur am verarbeiteten Setupcap oder nach
irreversibler Verbindungsschließung als fehlend; eine leere Queue oder bloßes
Noch-nicht-Eintreffen genügt nicht. Solche fehlenden Antworten sowie eindeutig
terminale, aber nicht erfolgreiche oder nicht eindeutige Setupresultate führen
ohne bestätigten Obserververstoß zu `U` und damit `UNPROVEN/inconclusive`,
nicht zu `FAIL`:
...
gesendet werden. Erst sein bestätigter Sendeübergang startet das getrennte
6.000-ms-Capturefenster. Ein gültiger Settlementkandidat `S` und ein eindeutig
dem POST zugeordnetes Terminalereignis `N` machen mit `P := S && N` nur die
Produktevidenz vollständig; sie schließen die Beobachtung nicht. Ohne `V`
bleibt das Capture bis zum verarbeiteten `C` vollständig aktiv. Ein bestätigtes
sticky `V` friert sofort fail-closed ein; ansonsten gewinnt bei Gleichzeitigkeit
die controllerlokale Verarbeitungsreihenfolge. Bei `U` bleiben Evaluation,
Factory und Transportstimulus `zero`, bewusst gegatete Downstream-Kommandos
`zero/unproven`, nicht zuverlässig beobachtete Networkcounts `unknown` und die
Sequenz `incomplete`; `captureWindowState` ist `not-started`. Die bereits vor
dem ersten Send gesetzte Stage 1 `observer-armed` bleibt auch nach einem
späteren `U` `observed/match`. Nur die gegateten Stages 2 bis 8 werden
`not-observed/unproven` mit `null` für Timing und `receiptOrder`. Ein
Diagnoseversuch enthält höchstens einen Stimulus, während `PASS` und jeder
nicht-inkonklusive Befund genau einen verlangen.

Bei `U`, `V` oder `C` entsteht zuerst ein frischer tief eingefrorener
Pre-Cleanup-Observation-Snapshot. Danach führt ein getrenntes Cleanup-Ledger
aus dem tatsächlich erreichten partiellen Zustand `Network.disable`,
`Target.detachFromTarget`, Stages 9 und 10 sowie exakt die 20 Cleanupchecks.
Sein eigener Finalisierungscap bleibt exakt `60.000 ms`; offene Checks werden
`unproven`, bestätigte Fehler behalten `FAIL`-Präzedenz. Der Endrecord wird erst
nach Cleanupfinalisierung frisch aus beiden unveränderten Projektionen
zusammengesetzt. Die Abschlussgründe umfassen mindestens `setup-cap`,
`setup-terminal-unproven`, `capture-cap` und `confirmed-violation`.

ADR 0032 bindet außerdem die exakte ebenenlokale 10-ms-Timingfunktion, feste
Stage-/Layer-/Clock-Zuordnungen, rohe Hashdomänen für den späteren einzelnen
Foundationpfad
`scripts/browser/browserSyncTransportRuntimeDiagnosticObserver.js` und den
exakt gesendeten `Runtime.evaluate.params.expression`-String, die geschlossene
verschachtelte Pre-Transport-/Execution-/Settlement-Projektion, descriptor-
basierte Own-Data-Prüfungen der unvertrauenswürdigen CDP-Hüllen sowie eine
totale Replayrelation `adr-0032-causal-replay-v2`. Die Descriptorbeobachtung
bestätigt den unvertrauenswürdigen Eingangsgraphen weder als gewöhnlich oder
Plain Data noch als proxyfrei, parser- oder pipe-authentisch; nur die frisch
vom Controller erzeugte Projektion kann als gewöhnlich und geschlossen
bestätigt werden. Die pure Foundation allein kann kein reales
`observerGate: PASS` belegen. Adapterabhängige Provenienz darf erst aus der
identitätsgebundenen Evidenz des späteren Adapters abgeleitet und nie als
freies Bool übernommen werden. Die Replayrelation vergleicht exakt acht
Artefakte, separat `repository.state` und 50 kausale Kontextwerte, also 59
Vergleiche. Die spätere Foundation darf ausschließlich importinaktive,
netzwerkfreie effects-as-data-Intents und unvertrauenswürdige materialisierte
In-Memory-Beobachtungen verarbeiten; Parser-, Raw-Pipe-, Queue-, Ressourcen-,
Cap-/Timer-, Hashbindungs-, Launcher-, Browser-, Vite-, Gateway- und
Dateisystemadapter bleiben einem eigenen Folge-ADR und einer danach getrennten
netzwerkfreien Implementierung vorbehalten. Erst nach beiden
Implementierungsslices darf ein sichtbarer Diagnoselauf gesondert autorisiert
werden.

ADR 0033 wurde am `2026-09-03` angenommen, ersetzte ADR 0032 formal und
übernahm alle dortigen Regeln, die es nicht ausdrücklich korrigierte. ADR 0032
bleibt mit seinem unveränderten Hauptteil ab `## Kontext` als historische
Entscheidungsebene erhalten. Die damalige Annahme von ADR 0033 implementierte
weder Foundation noch Tests oder Adapter und autorisierte keinen
Diagnoseruntimevorgang. ADR 0029, sein Evidence-Record,
`overallGate: FAIL` und `causeStatus: CAUSE_NOT_PROVEN` bleiben unverändert.

ADR 0034 wurde am `2026-09-04` angenommen und ersetzte ADR 0033 formal. Alle nicht
ausdrücklich korrigierten Regeln aus ADR 0033 und ADR 0032 gelten fort; ADR
0034 ersetzte keinen weiteren ADR. ADR 0033 bleibt mit bytegleichem Hauptteil ab
`## Kontext` als historische Entscheidungsebene erhalten. Schema, einzige
öffentliche API
`createBrowserSyncTransportRuntimeDiagnosticObserver({ effectPort,
runBinding })` und Kardinalitäten bleiben unverändert: `schemaVersion: 1`, 17
Rootfelder der `FoundationProjection`, 59 Replayvergleiche, sieben
Effects-Intents, sechs Protocol Operations, 17 Integrity Checks, zehn Stages,
zehn Capzustände, 20 Cleanupchecks und ein öffentlicher Export.

Der damals angenommene ADR 0034 totalisiert die geschlossenen lokalen Prototyp-,
...
Cleanupableitungen. Ein fehlender Intent ergibt nur bei konstruktiv sicher nie
aktivierter Ressource `zero/match`, sonst `zero/unproven`; ein Intent mit nicht
belegbarem Sendestatus ergibt `unknown/unproven`, exakt ein gültiger Sende-Ack
`one/match` und mindestens zwei bestätigte Sende-Acks `multiple/mismatch`.
Eine fehlende Session-ID beweist keine geschlossene Session, sobald
`Target.attachToTarget` möglicherweise oder bestätigt gesendet wurde. Nur
sicher nie gesendetes Attach erlaubt `targetSessionClosed: confirmed` und
Detach `zero/match`; ohne bindbare Session bleiben Sessionabschluss und Detach
`unproven`, ein korrelierter exakter Detach-Erfolg bestätigt den Abschluss.
Connection-Close ohne korrelierten Detach-Erfolg bestätigt ihn niemals.
Dieselbe epistemische Trennung gilt symmetrisch für `Network.enable`,
`Network.disable` und `networkDomainClosed`: Sicher nie gesendetes
Enable bestätigt den Domainabschluss mit Disable `zero/match`; möglicherweise
oder bestätigt gesendetes Enable bleibt ohne korrelierten exakten
Disable-Erfolg unbewiesen. Eine gebundene Session wird bei sicher nie
gesendetem Enable dennoch nach der bestehenden Detachmatrix behandelt.

Über die unveränderte öffentliche Foundation-API bleiben
`candidateObserverGate: PASS` und PASS-spezifische Findings konstruktiv
unerreichbar; öffentliche Foundationresultate können daher nur
`FAIL/observer-invalid` oder `UNPROVEN/inconclusive` enthalten. Die
vollständigen privaten Gate-/Findingableitungen dürfen im späteren
Implementierungsslice ausschließlich über die beschlossene temporäre Kopie der
exakten Produktionsquellbytes geprüft werden: Ein exakt einmal passender
lexikalischer Anker darf ausschließlich testlokale Exports ergänzen; nur diese
Kopie wird
seriell importiert, im `finally` entfernt und ihre Entfernung bestätigt. Kopie
und Resultate bleiben `NOT_EVIDENCE`, während die öffentliche Original-API die
Unerreichbarkeit black-box belegen muss. Netzwerkfreiheit gilt für
Foundationmodul, neue fokussierte Tests, Effects und jeden
Diagnoselauf dieses Slices. Die unveränderte Repository-Regressionsprüfung
darf ausschließlich ihre zwei bestehenden kurzlebigen Loopback-Fixtures
`tests/localSyncGatewayHttpServer.test.js` und
`tests/n8nCloudIngressProbe.test.js` ausführen; daraus folgt keine Erlaubnis
für neue, externe oder diagnostische Netzwerkaktivität.

ADR 0034 implementierte oder autorisierte weder Foundation, Tests, Adapter
noch Runtimevorgang. ADR 0035 ist am `2026-09-05` angenommen, ersetzt ADR 0034
formal und keinen weiteren ADR. ADR 0034 bleibt mit bytegleichem Hauptteil ab
`## Kontext` als historische Entscheidungsebene erhalten. Alle nicht
ausdrücklich korrigierten ADR-0034-, ADR-0033- und ADR-0032-Regeln gelten fort.
ADR 0035 implementiert oder autorisiert weder Foundation noch Tests, Adapter
oder Runtimevorgang. `schemaVersion: 1`, öffentliche Factory,
ein öffentlicher Export, sieben Intentarten, sechs Protocol Commands, 59
Replayvergleiche, 17 Integritychecks, zehn Stages, zehn Capzustände und 20
Cleanupchecks bleiben unverändert. Ebenso unverändert bleiben das
...
vor Evidenzdemotion und unabhängig von `zero|unknown|multiple|one` Stimuli
`FAIL/observer-invalid`-Präzedenz; nur Operand 53 projiziert den rohen
Vier-Codeunit-Portstring in sechs Codeunits einschließlich U+0022.
Die K2-Korrektur ist konstruktiv ausgearbeitet und im begrenzten R1–R4-Dokumentationsscope geprüft: Das disjunkte Profil
`derivation-conformance` exportiert neben der Factory nur Gate, Finding und
Finalizer und sperrt seinen öffentlichen Factorypfad synchron vor dem realen
Capabilityselector. `virtual-runtime-conformance` exportiert nur denselben
produktiven Owner, die vollständig definierte Producer-Eventgrenze und einen
einmaligen virtuellen Capabilityinstaller. Dessen exaktes Profil umfasst
Entropie, Clock, Runtime, Scheduler, Pipe, Launcher und Ressourcen; mutable
Bytes werden defensiv kopiert, Handles bleiben opaque und Raw-Fixtureevents
erreichen Foundationwerte nur über produktiven Parser, Ownerguard und FIFO.
Beide Namespaces besitzen exakt vier Namen, stammen aus bytegeprüften
Produktionsbytes außerhalb des Repositorys und bleiben
`adapterEvidenceEligible:false`; ein Selector-Poisonmutant muss jeden
Realfallback vor I/O töten. Die bestehende ADR-0035-Foundationtestkopie bleibt
unverändert.

`browser.engineBuild`, globale Portfreiheit, effektive Profil-/Netzzustände
und unabhängige Adapterattestierung bleiben ohne authentische Quelle
...
Spiegelungsverbot keinen authentischen Pre-Cleanup-Marker für alle Pfade
auswählen, und ein Promisezaun kommt für Old-Cap sowie portlosen Pre-Cleanup zu
spät. Als minimale eindeutige Phasenbindung unter den bestehenden
Architekturgrenzen gewählt ist `D_K4`, genau eine synchrone argumentlose und
einmalige Foundationnotification `effectPort.observationClosed()` unmittelbar
nach `O0` und vor jedem Cleanup. Sie ist kein achter Intent und trägt weder
...
benannte `.mjs`-Datei eines aufgelösten Betriebssystem-Temporärverzeichnisses
außerhalb des Repositorys kopieren und dort über genau eine lexikalisch
eindeutige Exportdeklaration zusätzlich exakt diese vier bereits produktiv
verwendeten privaten Bindings öffnen:

- `deriveCandidateObserverGate` mit Arity 1, rein;
- `deriveCandidateFinding` mit Arity 1, rein;
- `createBrowserSyncTransportRuntimeDiagnosticRunMachine` mit Arity 1,
  zustandsbehaftet und tatsächlich vom öffentlichen Factorypfad verwendet;
- `requestBrowserSyncTransportRuntimeDiagnosticExchange` mit Arity 2,
  zustandsbehaftet und einzige produktive Exchange-Request-Grenze aller sieben
  Intentarten.

Der Konstruktor gibt in der Testkopie denselben produktiven Machine-State als
testkopielokal erreichbaren Handle zurück; die Exchange-Grenze wird mit genau
diesem Handle und dessen aktiver Lease adressiert. So bleiben Lease,
`nextIntentId`, Ledgerdeltas, kontrollierte Effekte und Owner-Settlement ohne
fünften Inspector-, Debug- oder Produktionsseam prüfbar. Die unveränderte
Konformitätskopie und gesonderte mutationswirksame Mutantenkopien sind
disjunkte Kopietypen. Nur Mutantenkopien dürfen die ausdrücklich
allowlisteten Join-Abweichungen tragen; keine Kopie darf als Evidenz dienen.
Alle Importe erfolgen seriell über eine File-URL; jede Kopie wird im `finally`
entfernt und ihre vollständige Entfernung bestätigt.

Die spätere Join-Matrix treibt dieselbe produktive Run-Machine über normale
Effects-as-Data-Übergänge getrennt nach `prestart`, `observation` und
`cleanup` bis zu genau einer `observable-pending` Lease. Ein zweiter Aufruf der
echten Exchange-Grenze muss vor Intentkonstruktion, ID-Erhöhung, Count- oder
Ledgermutation, Cap-, Snapshot- oder Cleanupmutation und vor Capability- oder
Portaufruf stoppen. Gegen den unmittelbar vorherigen Baselinesnapshot müssen
Intentanzahl, nächste Intent-ID, Portaufrufe, Sendcount und
Capabilityaufrufe jeweils Delta null besitzen; genau ein Exchange bleibt
pending. Ausschließlich `pendingInternalExchangeViolation` wird mit der
jeweiligen Phase gesetzt.

Der finite Settlementteil ist exakt das 18-Fälle-Produkt aus drei Phasen,
`fulfillment|rejection` und den Zeitlagen `pre-invocation`,
`synchronous-post-invocation` sowie `observable-pending`. Im ersten Fall gibt
die Capability ein bereits gesetteltes Promise zurück, im zweiten wird das bei
Rückgabe noch pending Promise unmittelbar danach gesettelt, und im dritten
erst nach dem zweiten Boundaryaufruf. In allen drei Fällen erfolgt dieser
zweite Aufruf synchron im selben Turn vor jedem kontrollierten Handlerzutritt
und ohne Microtask-Checkpoint; bis zum Handler bleibt die Lease
`observable-pending`.

Fulfillment und Rejection werden in allen 18 Fällen ohne Lesen von Payload
oder Grund geprüft. Danach sind Lease
und Port geschlossen, `activeExchange` ist `null`,
`furtherExchangeCount: zero`, und lebende Caps werden `terminal-unknown`.
Prestart endet statisch ohne
`O0` oder Cleanup-Ledger; Observation setzt sticky `V`, friert `O0` und erlaubt
nur portlosen Cleanup; Cleanup bewahrt `O0`, setzt `cleanupViolation` und
endet mit `cleanup-terminal-failure`. Späte oder doppelte Handler bleiben
token- und zustandsgegatete `undefined`-No-ops.

Forever-pending ist von den 18 settelnden Fällen getrennt und wird nicht als
empirisch beobachtete Unendlichkeit behauptet.
Die dynamische Probe bestätigt nach sofortiger Prüfung und exakt drei
testlokalen Microtask-Checkpoints unveränderte Deltas, genau einen pending
Exchange, null Run-Settlements sowie weder künstliches `O0`, vorzeitigen
Cleanup, Terminalmaterialisierung noch weitere Capability oder Intent-ID.
Eine zusätzliche Probe wartet den Owner-Run bewusst nicht ab. Erst zusammen
mit der vollständigen privaten Transitionstabelle, nach der aus
`observable-pending` ohne kontrollierten Fulfillment- oder Rejectionhandler
kein Fortschritts-, Snapshot-, Cleanup-, Terminalisierungs- oder Resolvepfad
existiert, bildet dies den Nachweis. Timer, Timeout, `Promise.race`, Retry,
Polling, zweiter Port und konkurrierender Resolver bleiben ausgeschlossen.

Die mutationswirksame Matrix muss mindestens einen umgangenen oder verspäteten
Join-Guard, zweite Intent-/ID-/Count-/Ledger-/Port-/Capabilitywirkungen,
Freigabe zu `idle`, künstliches Owner-Settlement, vorzeitiges `O0` oder
Cleanup, verlorene Phasenklassifikation und Mutation des eingefrorenen `O0`
im Cleanupfall töten. Die drei zusätzlichen öffentlichen Black-box-
Regressionen binden ferner einen fortlaufenden
`lastValidBrowserNetworkTimestamp` einschließlich `10 -> 12 -> 11`, die
...
`{ exchange, observationClosed }` in dieser Reihenfolge, keine Realdefaults und
kein Factoryeffekt. Beide Portfelder sind erforderlich und werden je genau
einmal descriptorbasiert als Own Data erfasst. Für `observationClosed` wird
genau einmal der eigene nicht-enumerable Data-Descriptor `length: 0` geprüft;
weitere Callable-Klassifikationen oder spätere Lengthprüfungen erfolgen nicht.
Die erfolgreiche Factory liefert eine frische gewöhnliche tief eingefrorene
`{ run }`-API.
...
reentrant beobachtbar `capturedExchange -> activeExchange` und
`capturedObservationClosed -> activeObservationClosed` und löscht beide alten
Slots. Falsche Erst-Arity und ein terminaler Prestartpfad verwerfen beide
Rollen ohne Notification. Zweit-, Parallel-
...
alle Outputgraphen. Genau zwei callerlieferte Capabilityrollen sind jeweils
zeitlich disjunkt in ihrem Capture- und danach ihrem aktiven Owner-Slot
zulässig; identische Funktionsidentitäten für beide Rollen sind erlaubt.
Der Portcontainer wird nach der einmaligen Own-Data-Erfassung verworfen.
Exchangeaufrufe verwenden nur die erfasste Apply-Intrinsic, `undefined` und
ein frisches geschlossenes Frozen Intent. Die Capabilities werden nie
ausgegeben, serialisiert, eingefroren, persistiert, verglichen oder als
Provenienz verwendet. Der Owner löscht `activeExchange` nach dem letzten
benötigten Exchange und vor Promise-Erfüllung; der Exchange-Portschluss
verwirft eine noch für `O0` benötigte Notification nicht. Ein ewig pending
Exchange darf den Lauf pending halten.

Unmittelbar nach dem erfolgreichen Freeze und Binden von `O0` konsumiert der
einzige zentrale Callsite in `freezeObservationSnapshot` die Notification:
aktiven Slot löschen, `armed -> invoking`, synchron per erfasster Apply-
Intrinsic mit `undefined` als Receiver und null Argumenten aufrufen,
anschließend `consumed`. Erst danach folgen Cleanup-Phase, Old-Cap-Scan und
Cleanup-Ledger. Vorstartiges Verwerfen lautet `armed -> discarded`.
Ausschließlich der Rückgabewert `undefined` ist gültig; jeder andere Wert,
Throw oder interner Exchange-Reentranzversuch latcht einen statisch redigierten
sticky Cleanupverstoß, ohne `O0` zu ändern. Der Reentranzguard liegt nach
Arity-/Identitätsprüfung und vor Profilreflection, Join, IDs, Zählern, Ledger
und Effekten. Rückgabewerte werden weder reflektiert noch assimiliert oder
awaited; Fehlergründe werden nicht gebunden. Der lokale Callback- und
Rückgabewertbezug endet unmittelbar mit diesem synchronen Callsite.
...
beiden geplanten Zielpfade blieben unverändert, und weder Browser noch Netzwerk
oder Gateway wurden angesprochen. Ursache waren zwei unbeweisbare beziehungsweise
für gültige Version-1-Requests unerreichbare Nachweisanforderungen, keine
Produktlücke. Es entsteht weder eine neue API noch eine Dependency oder ein
zusätzlicher Seam. Die Implementierung bleibt bis zum Merge von ADR 0027
pausiert und ist danach der nächste getrennte Slice.

Der folgende Absatz hält den damaligen, inzwischen durch ADR 0027 ersetzten
Entscheidungsstand von ADR 0026 historisch unverändert fest:

ADR 0026 ergänzt ADR 0017, ADR 0020, ADR 0023 und ADR 0025, ersetzt keine
bestehende Entscheidung und legt ausschließlich den späteren lokalen
Browser-SyncTransport-Vertrag fest. Der Vertrag ist angenommen, aber noch
nicht implementiert. Es existieren weiterhin weder
`src/transports/browserSyncTransport.js` noch Browserrequest,
...
Das Boundary-Artefakt bleibt ein korrektes, aber weiterhin nicht komponiertes
und nicht aktiviertes n8n-Derivat; das Evidence-Tool komponiert es ausdrücklich
nicht. ADR 0025 ist im lokalen Gateway-Prozess implementiert: Ein manuell
...
3. lokales SyncGateway und lokaler SyncAgent werden getrennt und kontrolliert
   komponiert;
4. ADR 0026 entscheidet den konkreten Browser-SyncTransport-Vertrag;
5. ADR 0027 korrigiert dessen unbeweisbare Realm-Provenienzanforderungen und
   trennt den privaten Request-Cap von der öffentlich erreichbaren Version-1-
   Körpergröße;
6. der gemäß ADR 0027 entschiedene Browser-SyncTransport wird isoliert mit der netzwerkfreien
...
13. ADR 0034 ersetzt ADR 0033 formal und übernimmt alle nicht ausdrücklich
    korrigierten Regeln aus ADR 0033 und ADR 0032; ADR 0033 bleibt mit
    bytegleichem Hauptteil ab `## Kontext` historische Entscheidungsebene;
14. ADR 0035 ersetzt ADR 0034 formal und dokumentiert als angenommener
    Korrekturslice ausschließlich die ausführbare Testkopie-v2 für den internen
    Pending-Join, die 18 finiten Phasen-/Outcome-/Zeitlagenfälle, das getrennte
    Forever-pending-Oracle sowie die drei Ableitungsregressionen, ohne
    Foundation oder Tests zu implementieren; ADR 0034 bleibt mit bytegleichem
...
16. ADR 0036 schlug Raw-Pipe-, Parser-, Queue-, Ressourcen-, Cap-/Timer-,
    ...
    inzwischen getrennt implementiert und mit 595/595 fokussierten Tests geprüft;
17. der unabhängige R1–R4-Dokumentreview ist abgeschlossen;
18. ADR 0037 entscheidet D_K4 als gezielte Ergänzung von ADR 0035; der
...
22. der einmalige sichtbare Diagnoselauf wird ausgeführt und getrennt
    dokumentiert;
23. nur bei ausreichendem Befund folgt ein neuer Produktentscheidungs-ADR;
24. Produktänderungen erfolgen ausschließlich in einem eigenen
    Implementierungsslice;
25. anschließend wird ein vollständig neuer ADR-0029-Runtime-Evidence-Lauf mit
    neuer Run-ID und vollständiger Gatematrix separat autorisiert;
26. die lokale Browserkomposition folgt nur nach dessen Gesamt-`PASS` als
    eigener Entscheidungs- und Implementierungsslice;
27. der Browser-End-to-End-`syncTest` folgt danach als weiterer getrennter
    Slice;
28. globale/systemweite Missbrauchs-, Parallelitäts-, Zeit- und
    Ressourcenbegrenzung für den lokalen Pfad wird ergänzt;
29. erst danach werden Provider gesondert entschieden;
30. OpenAI-, lokaler Modell- und n8n-Adapter folgen jeweils als getrennte
    Slices;
31. private Daten, weitere Aktionen, Tools und Nebenwirkungen folgen nur nach
    neuen Contract-, Identitäts-, Berechtigungs-, Replay-, Idempotenz- und
    Datenschutzentscheidungen.

Die Schritte 1 bis 9 sind mit ADR 0023, ADR 0024, der Implementierung von ADR
...
`CAUSE_NOT_PROVEN` beziehungsweise `FAIL`. Schritt 13 ist mit der Annahme von
ADR 0034 abgeschlossen: ADR 0034 ersetzt ADR 0033 formal, übernimmt alle nicht
ausdrücklich korrigierten Regeln aus ADR 0033 und ADR 0032 und bewahrt ADR 0033
mit bytegleichem Hauptteil ab `## Kontext` als historische Entscheidungsebene.
...
ADR 0029 ergänzt ADR 0020 und ADR 0028, operationalisiert die fortgeltenden
ADR-0026-/ADR-0027-Runtimeanforderungen und ersetzt keinen ADR. Das Gesamtgate
bleibt an das vollständige Basistupel `T₀` gebunden; ausschließlich die
...
ADR 0033 ersetzte ADR 0032 formal und übernahm dessen nicht ausdrücklich
korrigierte Regeln sowie die fortgeltenden Ergänzungen von ADR 0028 und ADR
0029, ohne die ADR-0020-Produktions-Gateway-Baseline zu ändern. ADR 0034 ersetzt
ADR 0033 formal und übernimmt alle nicht ausdrücklich korrigierten Regeln aus
ADR 0033 und ADR 0032. Beide bleiben historische Entscheidungsebenen; der
Hauptteil von ADR 0033 ab `## Kontext` bleibt bytegleich erhalten. Der separate
...
Methoden-API, die aritäts- und descriptorbasierten Snapshots, der feste
Loopbackendpoint, die einmalige Validierung vor und nach Deep Freeze, die
geschlossene Serialisierung und RequestInit-Projektion, die Deadline-
Zustandsmaschine, das Response-/Header-/Stream-Profil, Cleanup, UTF-8- und
JSON-Handoff sowie die fortbestehenden Aktivierungsgates.

Für fremde Fetch-, Read- und zulässige Cleanup-Promises ist keine historische
Erzeugungsrealm- oder Subclass-Provenienz beweisbar. Maßgeblich ist deshalb nur
das bei der Übergabe beobachtbare geschlossene Profil: echte native Promise-
Brand, exakt der erfasste lokale Promise-Prototyp und dessen unveränderte Kette
bis `null`, eine leere Own-Key-Menge ohne eigene `constructor`-Property sowie
die unveränderten erfassten Constructor- und Species-Deskriptoren einschließlich
ihrer Identitäten. Ausschließlich die bei Modulevaluation erfasste native
`Promise.prototype.then`-Methode darf mit dem Kandidaten als Receiver
angewendet werden. Ein unveränderter Cross-Realm-Wert scheitert; ein echtes
Cross-Realm-Promise oder eine echte native Promise-Subclass darf in der
Testfixture nach vollständig vor Übergabe erfolgter Umprototypisierung bestehen,
wenn kein beobachtbares fremdes oder Subclass-Merkmal verbleibt. Der Transport
selbst ändert niemals Prototypen. Thenables, Proxies, Fakes, Own-Keys, Symbole,
Accessors, sichtbare Subclass-Prototypen, mutierte Constructor-/Species-
Deskriptoren und freie `.then`-Reads oder `Promise.resolve` bleiben unzulässig.
Eine mutierte globale `.then`-Oberfläche ist dabei ein Hostile-Hook-
Unabhängigkeitsnachweis: Die ersetzte Property wird weder frei gelesen noch
aufgerufen; die erfasste Methode bleibt autoritativ. Das äußere Transport-
Promise wird weiterhin mit dem erfassten lokalen Konstruktor erzeugt.

Für Streamchunks gelten analog echte `Uint8Array`- und Backing-`ArrayBuffer`-
Brands, die exakten erfassten lokalen Prototypen und vollständigen Ketten sowie
ein fester, nicht geteilter, nicht resizable, nicht detached Buffer und eine
positive sichere ByteLength innerhalb aller Caps. Nur View oder nur Buffer
umzuprototypisieren genügt nicht; eine vollständig fixtureseitig
umprototypisierte echte fremde View samt ihrem echten Backing-Buffer darf bei
vollständig passendem Profil bestehen. Der Transport ändert auch hier keine
Prototypen und kopiert akzeptierte Bytes unmittelbar in seinen tatsächlich
lokalen, eigenen Zielbuffer. Spätere Änderungen der Quelle wirken sich nicht
auf diese Kopie aus. Dieses Profil begründet weder Herkunft noch Identität,
Vertrauen, Authentisierung oder Datenschutz.

Der private produktive Browser-Request-Cap bleibt unverändert bei 65.536 Bytes;
Byte 65.537 scheitert weiterhin vor Controller, Timer und Fetch. Er ist jedoch
keine öffentlich erreichbare Version-1-Verhaltensgrenze: Der größte gültige
kanonische Requestbody umfasst exakt 193 UTF-8-Bytes, davon 129 feste Bytes und
höchstens 64 ASCII-Zeichen für die gesamte `requestId` einschließlich `req_`.
Eine 65 Zeichen lange `requestId` scheitert bereits an der Contractvalidierung
vor Stringify, Encode, Controller, Timer und Fetch. Diese private Browsergrenze
bleibt von der realen Gateway-Raw-Wire-Grenze 65.536/65.537 Bytes und der
öffentlich erreichbaren Browser-Response-Grenze 16.384/16.385 Bytes getrennt.

Die spätere Unit-Suite muss deshalb einen gültigen 193-Byte-Request bis exakt
einen Fetch führen und die 65-Zeichen-ID früh ablehnen. Zusätzlich muss sie in
temporären, anschließend entfernten Quellkopien die Caps 193 und 192 verwenden:
193 besteht, 192 scheitert, und Entfernung, Umgehung oder ein falscher
Vergleichsoperator macht den Gegenbeweis rot. Das beweist nur Verdrahtung,
inklusive Vergleichssemantik und die Position vor Nebenwirkungen, nicht eine
öffentlich erreichbare 65.536/65.537-Requestkante. Cross-Realm-Regressionsfälle
verwenden `node:vm`, native Intrinsics und kontrollierte Doubles, laufen seriell,
stellen globale Mutationen in `finally` wieder her und besitzen weder reale
Netzwerk-/Browserzugriffe noch Skips oder Todos.

Der erste Implementierungsversuch wurde vor jeder Dateiänderung beendet;
Working Tree, Index und beide geplanten Zielpfade blieben unverändert. Es gab
keinen Browser-, Netzwerk- oder Gatewayzugriff. Die beiden Korrekturen schließen
unbeweisbare beziehungsweise unerreichbare Nachweisforderungen, keine
Produktlücke; sie führen keine API, Dependency oder neuen Seam ein. Transport,
...
Der durch ADR 0026 entschiedene, aber noch nicht implementierte
**Browser SyncTransport Contract** plant ausschließlich
`src/transports/browserSyncTransport.js` mit dem einzigen Export
`createBrowserSyncTransport`. Die Factory liefert eine frische gewöhnliche und
eingefrorene API exakt mit `{ sendSyncRequest }`. Die Methode besitzt genau
einen formalen Parameter, akzeptiert exakt ein Argument und gibt immer sofort
ein echtes natives Promise zurück. Falsche Arity endet vor
Argumentinspektion, Dependencyzugriff oder -aufruf, Timer oder Netzwerk als
statisch redigierte Rejection. Import und Factory starten keine Request- oder
Runtimeverarbeitung; die Factory prüft und erfasst nur synchron ihre
Composition-Funktionen, ohne sie aufzurufen.

Bei Modulevaluation werden die benötigten Reflection-, Apply-, Freeze-, JSON-,
Typed-Array-Buffer-/ByteLength-/Kopier-, ArrayBuffer-Brand-/ByteLength-/
optionalen Resizable-, Encoder-/Decoder-Prototypmethoden und
Plattformdefaults privat erfasst. Für Promise werden insbesondere der native
Same-Realm-Konstruktor, `Promise.prototype`, die `then`-Methode,
`Symbol.species`, die ursprünglichen Own-Deskriptoren von
`Promise.prototype.constructor` und `Promise[Symbol.species]`, die
Species-Getteridentität sowie die vollständigen Promise-/Object-Ketten bis
`null` erfasst; ein bloß erfasster Konstruktor schützt nicht vor einer später
ersetzten Prototypmethode. `createBrowserSyncTransport()` mit null Argumenten
prüft und verwendet die erfassten Browserdefaults. Ein explizites
`createBrowserSyncTransport(undefined)`, zusätzliche Argumente oder fehlende
beziehungsweise unbrauchbare Defaults sind statische Factoryfehler. Bei genau
einem Composition-Argument werden dessen vollständige Own-Key-Menge einmal und
die vier erforderlichen Deskriptoren in fester Reihenfolge je einmal erfasst.
Zulässig ist nur ein gewöhnliches exaktes Vier-Felder-Own-Data-Record aus
`fetchRequest`, `createAbortController`, `setDeadlineTimer` und
`clearDeadlineTimer`; Accessors, Symbole, Zusatzfelder, partielle Shapes,
Reflection-Throws und nicht funktionale Werte ergeben denselben statischen
`TypeError("Ungültige BrowserSyncTransport-Komposition.")`. Danach wird keine
Composition-Property erneut gelesen.

Die Seams werden ausschließlich als
`fetchRequest(fixedEndpoint, freshRequestInit)`, `createAbortController()`,
`setDeadlineTimer(onDeadline, 5000)` und
`clearDeadlineTimer(timerHandle)` mit `undefined` als Receiver aufgerufen.
Andere Seams sind unzulässig. Endpoint, Deadline und Größenlimits bleiben feste
private Modulwerte. Der Transport garantiert höchstens einen Fetch-Seam-Aufruf
mit der festen URL und dem geschlossenen Argumentgraphen, nicht die interne
Netzwerkwirkung einer bösartigen injizierten Funktion. Der produktive
Defaultpfad verwendet die erfasste native Browserfunktion.

Der einzige Zielendpoint ist exakt
`http://127.0.0.1:8787/api/sync-test`. `localhost`, IPv6, relative oder
auflösungsabhängige URLs, Discovery, Redirect, Fallback und Zielwerte aus UI,
Request, Payload, Query, DOM, Storage oder Environment sind ausgeschlossen.
Der spätere Browserfluss verlangt deshalb ausdrücklich
`GOLDENDAWN_SYNC_GATEWAY_PORT = '8787'`; die weiterhin getrennte
`GOLDENDAWN_SYNC_GATEWAY_ALLOWED_ORIGIN` muss exakt der tatsächlichen lokalen
Frontend-Origin entsprechen. Der Transport setzt keine Origin; `Origin`,
`Host` und `Content-Length` bleiben browserverwaltet.

Nach bestandener Arity entsteht mit erfasster Reflection genau ein
autoritativ descriptor-basierter Requestsnapshot in dieser beobachtbaren
Reihenfolge: vollständige Root-Own-Key-Menge einmal, Rootprototyp einmal, dann
die sechs Deskriptoren `version`, `action`, `source`, `requestId`, `timestamp`,
`payload` je einmal. Die Payloadidentität stammt ausschließlich aus dem
erfassten `payload`-Descriptor; anschließend folgen Payload-Own-Keys einmal und
Payloadprototyp einmal. Zulässig sind nur die sechs Stringkeys, aufzählbare
Own-Data-Properties und ein exakt leeres gewöhnliches Payloadrecord. Symbole,
Accessors, Zusatz- oder fehlende Keys, Descriptor-Throws und widersprüchliche
Beobachtungen werden abgelehnt. Danach wird keine Caller- oder Payloadproperty,
kein Key, Descriptor oder Prototyp erneut gelesen.

Ausschließlich die fünf erfassten primitiven Strings erzeugen einen vollständig
frischen disjunkten Sechs-Felder-Requestgraphen mit frischem leerem Payload. Der
Snapshot bleibt nur die interne Reflectionmenge und ist kein Requestobjekt.
Nur derselbe frische Graph ist Validatorinput: genau einmal vor und genau
einmal nach seinem Deep Freeze mit derselben Snapshot-`timestamp`-Referenz.
Caller, Callerpayload oder ein zweites Snapshotobjekt erreichen den Validator
nie; es gibt weder dritten Aufruf noch Alternativpfad. Die Differenz null beweist nur interne
Konsistenz, keine unabhängige Frische, Uhrzeit oder Replayabwehr; die operative
Frischeprüfung bleibt Aufgabe der Gatewaygrenze und es gibt keine Browserclock.
Der Callergraph wird weder verändert noch eingefroren.

Vor Serialisierung werden Root und Payload terminal auf exakte Own-Data-Shape,
exakte gewöhnliche Prototypketten bis `null`, tatsächlichen Frozen-Zustand,
fehlende eigene `toJSON`-Properties, einen unveränderten
`Object.prototype` ohne eigenes `toJSON` und fehlende fremde verschachtelte
Identitäten geprüft. Danach folgen genau ein erfasster `JSON.stringify`-Aufruf
ohne Replacer, ein primitiver String, genau ein Aufruf der erfassten
`TextEncoder.prototype.encode`-Methode mit richtigem Receiver, eine echte
brand- und prototypgeprüfte `Uint8Array` und die Grenze 65.536/65.537 Bytes.

Erst nach dieser erfolgreichen Serialisierung wird pro zulässigem Aufruf der
Controller genau einmal erzeugt; Signal und Abortmethode werden je genau
einmal kontrolliert aufgelöst und mit dem Controller als erforderlichem
Receiver gespeichert. Das Signal bleibt ein opaker fremder Wert: Der Vertrag
prüft weder Typ, Brand noch Ownership und friert es nicht ein.

Der Fetch-Argumentgraph besteht aus einem frischen tatsächlich eingefrorenen
Null-Prototyp-RequestInit mit exakt `method`, `mode`, `credentials`, `cache`,
`redirect`, `referrerPolicy`, `keepalive`, `headers`, `body`, `signal` und
einem getrennten frischen tatsächlich eingefrorenen Null-Prototyp-Headersrecord
mit ausschließlich `Content-Type: application/json; charset=utf-8`. Die festen
Werte bleiben `POST`, `cors`, `omit`, `no-store`, `error`, `no-referrer` und
`keepalive: false`; Body ist nur der einmal erzeugte String und Signal nur die
einmal erfasste Controller-Signalidentität. Das Freeze des RequestInit begründet
keine Ownership- oder Freezegarantie für das Signal. Es gibt weder Cookie,
Authorization, Caller-Signal, Retry, Redirect, Fallback noch zweiten Versuch.

Die event-loop-basierte 5.000-ms-Deadline gilt nur für die asynchrone Fetch-
und Response-Stream-Phase. Sie präemptiert weder synchrone UTF-8-Decodierung
noch `JSON.parse` und ist bei blockiertem Eventloop keine harte Echtzeitgrenze.
...
Zustandsmaschine `active → success | transportFailure | deadline` besitzt genau
einen ersten terminalen Owner. Ein synchron während `setDeadlineTimer`
feuernder Callback gewinnt, Fetch bleibt nullmal und ein anschließend
zurückgegebener Handle wird einmal best effort gelöscht. Timerthrow ohne
vorherige Deadline scheitert vor Fetch. Unmittelbar vor dem tatsächlichen
Fetch-Seam-Aufruf wird `fetchStarted` gesetzt. Jeder danach gewinnende
`transportFailure`- oder `deadline`-Pfad abortiert den Controller höchstens
einmal nicht blockierend mit richtigem Receiver, einschließlich synchronem
...
JSON- oder Handoff-Fehler. Vor Fetch und bei Erfolg bleibt Abort nullmal. Nach Readerübernahme
kommen Cancel und Release je höchstens einmal hinzu; vorher wird keine
Body-Cleanupmethode aufgelöst. Timer-Clear erfolgt pro Handle genau einmal;
Cleanup wartet nicht und erzeugt weder zweiten Abschluss noch Fetch.

Unmittelbar vor jeder Anwendung der erfassten `Promise.prototype.then`-Methode
auf Fetch-, Read- oder zulässige Cleanup-Promises werden ohne fremden Hook der
exakte Same-Realm-Promiseprototyp, eine leere Own-Key-Menge ohne eigene
`constructor`-Property, die unveränderte Prototypkette, der ursprüngliche
`Promise.prototype.constructor`-Datendescriptor mit Konstruktoridentität sowie
der ursprüngliche `Promise[Symbol.species]`-Accessordescriptor mit
Species-Getteridentität geprüft. Erst danach läuft die erfasste Methode mit
richtigem Receiver. `Promise.resolve`, freie `.then`-Reads, fremde Thenables,
Proxies und Subclassen sind unzulässig. Alle kontrollierten Settlementhandler
fangen beherrschte Throws, prüfen bei späten Settlements zuerst den terminalen
Zustand und geben auf jedem Pfad ausschließlich primitives `undefined` zurück;
kein Folgepromise assimiliert einen fremden Handlerrückgabewert.

Nach Fetch werden `status`, `redirected`, `url`, `type`, `headers`, `body` in
dieser Reihenfolge jeweils genau einmal gelesen und sofort geprüft; ein Fehler
liest alle späteren Felder nullmal. Non-200 stoppt nach `status`, liest weder
Header noch Bodymethode und führt nur zum Post-Fetch-Abort. `headers.get` wird
einmal aufgelöst; `content-type`, `content-length`, `content-encoding` werden
jeweils nur nach bestandener Vorprüfung gelesen und sofort geprüft. Erst danach
wird `body` gelesen. Verlangt werden der exakte JSON-/UTF-8-Type, eine
kanonische vorzeichenlose ASCII-Dezimalzahl bis 16.384 und ein
browserexponierter Encodingwert exakt `null`.

`Content-Encoding` ist nicht CORS-safelisted und wird vom aktuellen Gateway
nicht zusätzlich exponiert. `null` bedeutet daher nur „im gefilterten
Browserheadersobjekt nicht sichtbar“, nicht nachgewiesene Wire-Abwesenheit oder
fehlende Browserdekompression; ein exponierter nicht-null-Wert scheitert. Das
Cap zählt browserexponierte, möglicherweise bereits decodierte Bytes, und die
Gleichheit von browserexponierter Content-Length und kopierten Bytes bleibt nur
ein enger Gateway-Kompatibilitätscheck, kein allgemeiner Kompressions- oder
Wire-Oktett-Beweis. Gateway und CORS-Header bleiben unverändert. Ein
beweiskräftiger sichtbarer Encodingnachweis benötigt einen neuen Gateway-/CORS-
Entscheidungsslice.

`body.getReader` sowie `reader.read`, `cancel`, `releaseLock` werden je einmal
aufgelöst und stets mit richtigem Receiver verwendet. Reads sind strikt
seriell. Für jedes Read-Result werden Prototyp und Own-Key-Menge einmal erfasst;
die Keysequenz muss exakt `value`, `done` sein. Danach folgen die Deskriptoren
je einmal in der anderen festen Reihenfolge `done`, `value`; anschließend
zählen nur Snapshotwerte. Zulässig ist nur ein gewöhnlicher exakter Own-Data-
Record. Bei `done: false` ist `value` eine echte brandgeprüfte, nicht
unterklassifizierte `Uint8Array` mit exaktem Prototyp und sicherer positiver
Ganzzahl-ByteLength. Null Länge scheitert nach genau diesem Read ohne Kopie oder zweiten
Read; Abort und Readercleanup folgen. Bei `done: true` ist `value` exakt
`undefined`. Accessors, Zusatzkeys und malformed Chunks sowie beobachtbare
Proxy-Traps oder Snapshotinkonsistenzen werden abgelehnt.

Die backing-buffer-Identität wird ausschließlich über die erfasste native
Typed-Array-Buffer-Intrinsic mit dem Chunk als richtigem Receiver ermittelt:
Sie muss einen echten Same-Realm-`ArrayBuffer` mit exakt erfasstem
`ArrayBuffer.prototype` ergeben, nicht
geteilt, nicht growable, nicht detached und, sofern prüfbar, nicht resizable.
`SharedArrayBuffer`, Growable SharedArrayBuffer, Proxy, fremder Buffer,
malformed Buffer und ein falscher Bufferprototyp scheitern. ByteLength und
Kopie verwenden ausschließlich erfasste Typed-Array-/ArrayBuffer-Intrinsics.
Genau ein eigener fester, nicht geteilter Zielbuffer bis 16.384 Bytes wird angelegt. Zwischen letzter Bufferprüfung und
sofortiger Kopie liegt kein fremder Hook; die Chunkidentität bleibt nicht bis
zur Decodierung erhalten. Byte 16.385 stoppt vor Kopie, weiterer Allokation
oder weiterem Read. Weil jeder akzeptierte Nicht-EOF-Chunk mindestens ein Byte
beiträgt, gibt es höchstens 16.384 solcher Reads und keine Nullchunk-
Microtask-Starvation. Sauberer EOF verlangt exakte Längengleichheit,
`cancel` nullmal und `releaseLock` genau einmal; Fehler und Deadline verwenden
Cancel/Release höchstens einmal nicht blockierend best effort; Throws und
zulässige native Cleanup-Rejections werden konsumiert.

Nach sauberem EOF und normalem Release wird die Deadline deaktiviert und der
Timer vor der synchronen Terminalphase einmal gelöscht. Danach folgen genau
eine erfasste `TextDecoder.prototype.decode`-Anwendung mit richtigem Receiver,
`fatal: true`, `ignoreBOM: true` und sichtbarer U+FEFF-BOM sowie genau ein
erfasstes `JSON.parse` ohne Reviver oder Normalisierung. Vor unmittelbarem
Promise-Fulfillment müssen Object- und Arrayroots exakt die erfassten
Prototypketten bis `null` besitzen; `Object.prototype` und `Array.prototype`
dürfen kein eigenes `then` besitzen. Ein top-level eigenes `then` ist nur als
nicht aufrufbare Own-Data-Property zulässig. Accessor-, callable oder geerbte
kompromittierte Formen ergeben statischen Transportfehler.

Nur der einmal geparste, weiterhin unvertrauenswürdige Wert erfüllt den
Transport. Semantische Responsevalidierung, Korrelation und Projektion bleiben
beim SyncService. Jeder beherrschte Methodenfehler rejected ausschließlich mit
dem statischen tief eingefrorenen Profil
`{ code: "BROWSER_SYNC_TRANSPORT_FAILED", message: "Der lokale Browser-SyncTransport ist fehlgeschlagen." }`
ohne fremde URL, Header, Body, Bytes, Sentinel-, Timer-, Dependency- oder
Exceptiondetails und ohne Console, Logging, Storage, Persistenz oder
Telemetrie. Transportrejections werden `transportFailed`; parsebares malformed
oder nicht korreliertes `200`-JSON wird ausschließlich `invalidResponse`; eine
vollständig gültige fachliche Contract-Fehlerresponse bleibt eine normale
SyncResponse.

Der Anwendungscode sendet keine Cookies, Credentials, Authorization,
anwendungsseitigen Referrer, privaten Payloads oder Providersecrets. Der
Browser kann dennoch `Origin`, `User-Agent`, `Accept`, `Accept-Language`,
`Sec-Fetch-*`, Client Hints und PNA-/LNA-Preflightmetadaten senden; der Prozess
auf Port 8787 kann sie beobachten. `omit` und `no-referrer` beweisen keine
Metadatenfreiheit oder Datenschutz. Private Module und Vaults bleiben außerhalb.

Vor Browserkomposition oder End-to-End-Fluss ist ein separat freigegebener,
an Betriebssystem, Browser und Version, tatsächliche GoldenDawn-Origin und
festen Loopbackendpoint gebundener Runtime-PASS für CORS/Preflight, PNA/LNA,
lokale Netzwerkberechtigung, Secure Context/Mixed Content, Erreichbarkeit,
Redirectverhalten, finale URL, Response-Typ sowie sichtbare und blockierte
Responseheader, Browserunterschiede und nötige Benutzerfreigaben erforderlich.
Ein `PASS` bleibt kontext- und versionsgebunden und ist keine allgemeine
Browsergarantie. Benötigt der Browser neue
PNA-/LNA- oder CORS-Header beziehungsweise eine Berechtigung, bleibt die
Komposition geschlossen und ADR 0020/0026 benötigen einen neuen Slice; es gibt
keinen Fallback auf `localhost`, HTTPS, Proxy, Remote oder Cloud.

Tor A klassifiziert nur diesen aktuellen reinen Dokumentationsslice vorläufig
als Nicht-KI. Vor Merge der späteren Implementierung werden tatsächlicher Code,
...
fachliche Nebenwirkungen geprüft. Browserkomposition und menschliche Interaktion
benötigen nochmals ein eigenes vollständiges Gate. Die spätere netzwerkfreie
Unit-Suite ist verbindlich unter `tests/browserSyncTransport.test.js`
dokumentiert und deckt die exakte Reflectionfolge, genau zwei Validatoraufrufe
auf demselben frischen Graphen, Promise-Constructor/Species und ausschließlich
`undefined` liefernde Handler, Nullchunk-Fortschritt, native
`value`-/`done`-Keyfolge, feste nicht geteilte ArrayBuffer, einheitlichen
Post-Fetch-Abort, fail-fast Response-/Headerzahlen, die gefilterte
Content-Encoding-Semantik, UTF-8/JSON/Handoff, Redaction und
Serviceintegration mutationswirksam ab. Sie wird in diesem Slice nicht
implementiert und führt keine echten Netzwerkrequests aus.
...
Der durch ADR 0025 entschiedene und nun implementierte Kompositionsvertrag
verbindet ausschließlich im bestehenden Gateway-Prozess den von der Boundary
akzeptierten defensiven Request mit genau einer in
`server/startLocalSyncGateway.js` erzeugten und als erforderliche HTTP-Factory-
Dependency injizierten SyncAgent-Instanz. Die HTTP-Factory besitzt keinen
versteckten Agentendefault; `syncAgent.processSyncRequest` wird bei der
Komposition genau einmal sicher aufgelöst. Eine fehlende, nicht funktionale
oder werfend aufgelöste Methode verhindert den Serveraufbau vor dem Listener.
Boundary-Factory und Boundary-Default bleiben unverändert, die öffentliche
Gateway-API exakt `{ start, stop }` und der Agent ohne eigenen Lifecycle.

Ausschließlich die exakte defensive Boundary-Requestidentität darf den Agenten
synchron, mit genau einem Argument und pro akzeptiertem Requestpfad höchstens
einmal erreichen. Raw Bytes, Raw Body, Parsed-JSON-Original, Header, Origin,
HTTP-/Socketobjekte, Fehlerresults, Gateway-Fehlerresponses, Secrets und private
Modulwerte enden vorher. Es gibt keinen Retry, Fallback, dynamische Aktion oder
Handlerregistry. Das Gateway verwendet weder `await` noch `Promise.resolve` und
assimiliert oder löst keine Promises beziehungsweise Thenables auf. Ein echter
Promise, ein Result mit zusätzlicher eigener `then`-Property oder ein
anderweitig malformed Result scheitert an der exakten Resultform. Eine
geerbte oder nur über einen Proxy-`get`-Trap virtuell angebotene `then`-
Property wird nicht eigens gelesen und bestimmt den synchronen Kontrollfluss
nicht; eine portable universelle Proxy-/Thenable-Erkennung sowie
Parallelitäts- oder Exactly-once-Garantien werden nicht behauptet.

Das Agentenresultat bleibt unvertrauenswürdig. Nur der exakte tief eingefrorene
ADR-0024-Erfolg `{ ok: true, status: "syncResponseCreated", syncResponse,
error: null }` ist zulässig. Seine unveränderte normale SyncResponse wird gegen
denselben Boundary-Request geprüft, erst danach descriptor-basiert ohne fremde
verschachtelte Identitäten in einen frischen gewöhnlichen Zehn-Felder-
Responsegraphen des festen synthetischen `syncTest`-Erfolgsprofils projiziert,
erneut validiert, tief eingefroren, auf Freeze geprüft, final revalidiert und
genau einmal serialisiert. Dafür erfasst `server/localSyncGatewayHttpServer.js`
bei erfolgreicher Modulevaluation mindestens `Object.prototype`,
`Array.prototype`, `Object.getPrototypeOf`,
`Object.getOwnPropertyDescriptor`, `Object.hasOwn`, `Object.freeze`,
`Object.isFrozen`, `Reflect.ownKeys`, `Array.isArray` und `JSON.stringify`.
Terminale Projektion, Shape-, Prototype-, Freeze- und `toJSON`-Prüfung sowie
Vorabserialisierung verwenden nur diese erfassten Referenzen. Nach der letzten
möglicherweise unvertrauenswürdigen Reflection müssen Records und Arrays exakt
die erfassten Prototypen, ausschließlich erlaubte Own-Data-Properties und
frische verschachtelte Werte besitzen sowie über die erfasste Frozen-Referenz
weiterhin tatsächlich eingefroren sein. Danach bestätigt die erfasste
`Object.getPrototypeOf`-Referenz exakt
`capturedGetPrototypeOf(capturedArrayPrototype) === capturedObjectPrototype`
und anschließend
`capturedGetPrototypeOf(capturedObjectPrototype) === null`. Terminal zulässig
...
eine allgemeinere oder dynamisch erweiterbare Kette wird nicht akzeptiert.
Erst nach beiden Identitätsprüfungen werden der erfasste `Array.prototype` und
danach der erfasste `Object.prototype` auf eine eigene `toJSON`-Property
geprüft. Erst dann wird ohne weiteren absichtlichen untrusted Hook die erfasste
Serialisierungsfunktion exakt einmal angewendet.

Eine Kettenabweichung wird vor Responsebesitz erkannt, ruft die erfasste
Erfolgsserialisierung nullmal auf, serialisiert den kompromittierten Graphen
nicht und ergibt ausschließlich das bereits materialisierte statische
`500 gatewayFailed`, ohne fremden Body, Sentinel, Exceptiontext oder zweite
Response. Throw, ein Rückgabewert mit `typeof !== "string"` oder eine andere
terminale Inkonsistenz führt ebenfalls mit dem bereits materialisierten
statischen Profil vor Responsebesitz zu `500 gatewayFailed`. Post-import
ersetzte globale Serialisierungs-, Reflection-, Freeze- oder Frozen-Funktionen
beeinflussen diese Grenze nicht; vor
...
HTTP-, Header-, CORS-, Serialisierungs-, Socket- und Cleanup-Owner.

Ein exakter Erfolg ergibt HTTP `200` ausschließlich mit der defensiven
normalen SyncResponse. Agentenfehler, Throws, echte Promises, Results mit
zusätzlicher eigener `then`-Property oder anderweitig malformed Results,
ungeeignete Responses sowie Projektions-,
terminale Shape-/Prototype-/Freeze-/`toJSON`-, Revalidierungs- oder
Vorabserialisierungsfehler ergeben statisch `500 gatewayFailed`; sie werden
nicht als neue `400`-, `502`-, `503`- oder `504`-Profile offengelegt. Der frühere
statische `503 upstreamUnavailable`-Pfad ist aus der HTTP-Implementierung
entfernt.

Der Implementierungsslice prüft mutationswirksam post-import ersetztes
globales `JSON.stringify`, installierte eigene `Object.prototype.toJSON`- und
`Array.prototype.toJSON`-Properties sowie ein durch unvertrauenswürdige
Reflection zwischen beide erfassten Prototypen eingeschobenes Objekt mit
`toJSON` und privatem Test-Sentinel. Bei dieser Mutation bestehen die direkten
Response-Prototyp- und Own-`toJSON`-Prüfungen weiterhin; ausschließlich die neue
Kettenprüfung führt vor Responsebesitz statisch zu `500 gatewayFailed`, ruft
die Erfolgsserialisierung nullmal auf und leakt weder Sentinel, fremden Body
noch Exceptiontext; eine zweite Response entsteht nicht. Die saubere Kontrollprobe
bestätigt die exakte Array-Prototypkette bis `null` und genau einen erfassten
Erfolgsserialisierungsaufruf. Globale Instrumentierungen laufen mit
`concurrency: false` und vollständigem Restore der ursprünglichen
Prototypkette, globalen Funktionen und Descriptoren im `finally`; Tests und
Kompositionscode sind umgesetzt.

Der enge Phase-0-/EU-Tor-A-Nachweis aus ADR 0025 ordnet ausschließlich diesen
bei demselben stabilen Request und Clockwert deterministischen, modell-,
provider-, workflow- und credentialfreien Slice vorläufig als Nicht-KI ein.
Er verwendet kein Modell, keine modell-, lern- oder statistikbasierte Inferenz,
kein Training, Lernen oder Adaptieren, sondern ausschließlich fest
programmierte Validierungs-, Projektions-, Korrelations- und Mappingregeln.
Das Inhalts-Payload ist bestimmungsgemäß exakt leer; der Pfad greift nicht auf
PromptVault, LearningHub, LichtwaldLog oder GoldenDawn-Vault zu und verarbeitet
oder überträgt bestimmungsgemäß keine privaten Inhalte. `source`, `requestId`
und `timestamp` bleiben jedoch Metadaten, die private Bedeutung codieren
können; Contract und leeres Payload beweisen weder semantische Nicht-Privatheit
noch Datenschutz. Diese Arbeitshypothese ist keine Rechtsberatung, keine
Klassifikation von GoldenDawn OS insgesamt oder späteren Agenten/Providern und
kein Compliance-Siegel. Browsertransport, menschliche
Interaktion, Modell, Provider, Workflow, neue Aktion, nicht leeres oder
privates Payload, Tool, Nebenwirkung, Logging, Persistenz, Telemetrie, Hosting,
Nutzung durch andere oder Zweckänderung lösen eine neue Tor-A-Prüfung aus;
Phase 1 bis Phase 3 bleiben offen. `Agent`, Determinismus und
`dataOrigin: "synthetic"` sind keine gesetzlichen Klassifikations-, Herkunfts-
oder Transparenznachweise.
...
Boundaryfehler verwenden eine getrennte statische Envelope. Bei einem
akzeptierten Request erreicht ausschließlich die defensive Boundary-
Requestidentität den injizierten SyncAgent synchron und höchstens einmal. Nur
der vollständig geprüfte, frisch projizierte und terminal abgesicherte
synthetische `syncTest`-Erfolg endet mit HTTP `200`; jeder beherrschte Fehler
dieser Agentengrenze endet statisch redigiert mit HTTP `500`. Die
Requestprojektion wird weder gespiegelt noch extern weitergesendet. Pro
physischem Socket darf genau ein Anwendungs- oder Raw-Socket-Pfad
...
weder gelesen noch exportiert. Das n8n-Artefakt bleibt von der lokalen
ADR-0025-Komposition getrennt und unkomponiert. Ein explizit gestarteter lokaler
Gateway kann den leeren synthetischen `syncTest` zwar nun mit HTTP `200`
beantworten; da weiterhin kein Browser- oder Cloudtransport komponiert ist,
...
Der lokale `SyncAgent` ist die autoritative Policy-, Defense-in-depth-
...
Browser-, UI-, Caller- und Requestwerte bestimmen weder Fachagent, Provider,
Modell, Workflow, Endpoint, Umgebung noch Berechtigungen. Das feste
Browsertransportziel stammt ausschließlich aus ADR 0027. Provideroutput bleibt unvertrauenswürdig;
Modelloutput darf niemals Berechtigungen, Routing, Providerwahl oder
Toolausführung bestimmen. Der ADR-0025-Implementierungsslice komponiert den
lokalen `SyncAgent` ausschließlich als logisch getrennte, injizierte
serverseitige Komponente im bestehenden lokalen Prozess. ADR 0023 autorisiert
keinen zweiten
...
- Führe zunächst kein allgemeines Fachbackend ein. Das mit ADR 0020 als lokale
  ...
  Storage. ADR 0025 entscheidet und implementiert den davon logisch getrennten
  lokalen `SyncAgent` als injizierte serverseitige Komponente im selben Prozess;
  ein zweiter Listener, IPC oder weiterer lokaler Netzwerkdienst ist nicht
  autorisiert.
