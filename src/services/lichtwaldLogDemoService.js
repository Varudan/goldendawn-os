...
const SERVICE_FAILURES = Object.freeze({
  invalidInput: Object.freeze({
    ...
    message: 'Bitte korrigiere die markierten LichtwaldLog-Demo-Felder.',
  }),
  limitReached: Object.freeze({
    status: 'limitReached',
    code: 'lichtwaldLogEntryLimitReached',
    message: 'Die LichtwaldLog-Demo kann keine weiteren Einträge aufnehmen.',
  }),
  entryNotFound: Object.freeze({
    status: 'notFound',
    code: 'lichtwaldLogEntryNotFound',
    message: 'Der angeforderte LichtwaldLog-Demo-Eintrag wurde nicht gefunden.',
  }),
  idGenerationFailed: Object.freeze({
    status: 'generationFailed',
    code: 'lichtwaldLogEntryIdGenerationFailed',
    message: 'Für den LichtwaldLog-Demo-Eintrag konnte keine gültige ID erzeugt werden.',
  }),
  invalidState: Object.freeze({
    status: 'validationFailed',
    code: 'invalidLichtwaldLogState',
    message: 'Die LichtwaldLog-Demo-Änderung ergibt keinen gültigen Gesamtzustand.',
  }),
  storageUnavailable: Object.freeze({
    status: 'unavailable',
    code: 'lichtwaldLogStorageUnavailable',
    message: 'Der flüchtige LichtwaldLog-Demospeicher ist nicht verfügbar.',
  }),
  storageReadFailed: Object.freeze({
    status: 'readFailed',
    code: 'lichtwaldLogStorageReadFailed',
    message: 'Die LichtwaldLog-Demo konnte nicht sicher gelesen werden.',
  }),
  storageWriteFailed: Object.freeze({
    status: 'writeFailed',
    code: 'lichtwaldLogStorageWriteFailed',
    message: 'Die LichtwaldLog-Demo-Änderung konnte für diese Sitzung nicht übernommen werden.',
  }),
  unexpectedStorageResult: Object.freeze({
    status: 'storageFailed',
    code: 'unexpectedStorageResult',
    message: 'Der flüchtige LichtwaldLog-Demospeicher hat kein verwertbares Ergebnis geliefert.',
  }),
})
...
const DEMO_STORAGE_FAILURES = Object.freeze({
  invalidDemoData: createStorageFailure(
    'invalidStoredData',
    'invalidLichtwaldLogDemoData',
    'Die LichtwaldLog-Demo enthält keinen gültigen synthetischen Gesamtzustand.'
  ),
  syntheticRequired: createStorageFailure(
    'invalidStoredData',
    'syntheticLichtwaldLogRequired',
    'Der LichtwaldLog-Demospeicher enthält Daten mit ungültiger Herkunft.'
  ),
  invalidSaveData: createStorageFailure(
    'validationFailed',
    'invalidLichtwaldLogDemoData',
    'Die LichtwaldLog-Demo kann diese Änderung nicht übernehmen.'
  ),
  syntheticSaveRequired: createStorageFailure(
    'validationFailed',
    'syntheticLichtwaldLogRequired',
    'Der LichtwaldLog-Demospeicher akzeptiert nur synthetische Daten.'
  ),
  serializationFailed: createStorageFailure(
    'serializationFailed',
    'lichtwaldLogDemoSerializationFailed',
    'Die LichtwaldLog-Demo konnte nicht sicher vorbereitet werden.'
  ),
  sizeLimitExceeded: createStorageFailure(
    'sizeLimitExceeded',
    'lichtwaldLogDemoSizeLimitExceeded',
    'Die LichtwaldLog-Demo überschreitet die zulässige Größe.'
  ),
})
...
const READ_STORAGE_FAILURES = new Map([
  [
    ...
    DEMO_STORAGE_FAILURES.invalidDemoData,
  ],
  [
    'invalidStoredData\0syntheticLichtwaldLogRequired',
    DEMO_STORAGE_FAILURES.syntheticRequired,
  ],
  [
    'serializationFailed\0lichtwaldLogDemoSerializationFailed',
    DEMO_STORAGE_FAILURES.serializationFailed,
  ],
  [
    'sizeLimitExceeded\0lichtwaldLogDemoSizeLimitExceeded',
    DEMO_STORAGE_FAILURES.sizeLimitExceeded,
  ],
  [
...
const SAVE_STORAGE_FAILURES = new Map([
  ...
  [
    ...
    DEMO_STORAGE_FAILURES.invalidSaveData,
  ],
  [
    'validationFailed\0syntheticLichtwaldLogRequired',
    DEMO_STORAGE_FAILURES.syntheticSaveRequired,
  ],
])

const FIELD_ERROR_MESSAGES = Object.freeze({
  form: 'Die LichtwaldLog-Demo-Eingabe konnte nicht sicher gelesen werden.',
...
function isValidSyntheticLichtwaldLog(lichtwaldLog) {
  try {
    return (
      validateLichtwaldLog(lichtwaldLog).ok === true &&
      lichtwaldLog.dataOrigin === SYNTHETIC_DATA_ORIGIN
    )
  } catch {
    return false
  }
...
function validateAndCloneSyntheticLichtwaldLog(lichtwaldLog) {
  if (!isValidSyntheticLichtwaldLog(lichtwaldLog)) {
    return { ok: false }
  }
  ...
  if (!isValidSyntheticLichtwaldLog(clonedLichtwaldLog)) {
    return { ok: false }
  }
...
function parseLoadResult(storageResult) {
  ...
  if (ok !== true || status !== 'found') {
    return { ok: false, failure: SERVICE_FAILURES.unexpectedStorageResult }
  }

  const clonedResult = validateAndCloneSyntheticLichtwaldLog(lichtwaldLog)
...
function generateDefaultLichtwaldLogDemoEntryId() {
  if (typeof globalThis.crypto?.randomUUID !== 'function') {
    throw new Error('randomUUID unavailable')
  }

  return `lichtwald-demo-entry-${globalThis.crypto.randomUUID()}`
}
...
export function createLichtwaldLogDemoService({
  lichtwaldLogDemoStorage,
  generateLichtwaldLogDemoEntryId = generateDefaultLichtwaldLogDemoEntryId,
} = {}) {
  function readCurrentLichtwaldLog() {
    let loadMethod

    try {
      loadMethod = lichtwaldLogDemoStorage?.loadLichtwaldLog
    } catch {
    ...
    try {
      storageResult = loadMethod.call(lichtwaldLogDemoStorage)
    } catch {
      return { ok: false, failure: SERVICE_FAILURES.storageReadFailed }
    }
  ...
  function persistLichtwaldLog(previousLichtwaldLog, candidateLichtwaldLog) {
    if (!isValidSyntheticLichtwaldLog(candidateLichtwaldLog)) {
    ...
    try {
      saveMethod = lichtwaldLogDemoStorage?.saveLichtwaldLog
    } catch {
