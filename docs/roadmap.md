...
Der Foundationabgleich von ADR 0036 ist dokumentiert. Laut Jans Chatbericht
ist sein unabhängiger Daybreak-Blue-Latest-/xhigh-Dokumentreview mit gebundenem
PASS ohne Befund abgeschlossen; Jan hat ADR 0036 anschließend am 2026-09-12
ausdrücklich angenommen. Die [acht vollständigen Vorannahmehashes](../CHANGELOG.md#adr-0036-annahme-und-abgeschlossener-dokumentreview--2026-09-12)
binden ausschließlich die geprüften Fassungen, nicht die Vollhashes dieser
Statusnachführung; der frühere R1–R4-Review bindet weiterhin nur seine damaligen
ADR-0036-Bytes. Die Adaptergrenze ist entschieden. Ein eigener netzwerkfreier
Adapterimplementierungs- und Testslice benötigt einen neuen gesonderten Auftrag;
Adapter, Adaptertests und authentisches adapterseitiges `A_obs` fehlen weiterhin.
Writer, Persistenz, Diagnoselauf, Browserkomposition und End-to-End bleiben
geschlossen. Dieser Auftrag endet nach Statusnachführung und Verifikation;
...
- Die aktuelle Implementierungsreihenfolge lautet: **ADR 0027 – beobachtbare
  ...
  Envelope- und Observation-Completion-Grenze ✅ → ADR 0032 – Capture-, Timing-
  und Projection-Determinismusgrenze ✅ → ADR 0033 – Diagnostic Foundation
  Effects Protocol Boundary ✅ → ADR 0034 – historische Grammar-, Derivation-
  und Testability-Totalisierung ✅ → ADR 0035 – angenommene Pending-Join-
  ...
  K2 im R1–R4-Dokumentationsscope geprüft, damals nicht annahmereif →
  ADR 0037 – angenommene
  Foundationentscheidung für synchrone `observationClosed()`-Notification ✅ →
  getrennte Foundationimplementierung und Tests ✅ → gebundener unabhängiger
...
- Die Reihe `v0.2.x` ist bewusst lokalen GoldenDawn-OS-Modulen vorbehalten.
  ...
  `CAUSE_NOT_PROVEN`. ADR 0035 ist angenommen, ersetzt ADR 0034 formal und
  ...
  historische Entscheidungsebene erhalten. ADR 0035 adressiert die bislang
  öffentlich unerreichbare interne Pending-Join-Testtransition; Candidate-
  `PASS` bleibt öffentlich unerreichbar.
  Browserkomposition, Browser-End-to-End-Fluss und
...
- ✅ Slice-Grenze eingehalten: Der BrowserSyncTransport ist weder mit dem
  ...
  Im damaligen ADR-0027-Entscheidungsstand folgte als nächster Slice
  ausschließlich das getrennte reale, kontext- und versionsgebundene PNA-/
  LNA-/Mixed-Content-Runtimegate einschließlich der übrigen CORS-,
  Berechtigungs-, Loopback-, Redirect-, Header- und Responsebeobachtungen.
  Erst dessen gebundenes `PASS` konnte die getrennte Browserkomposition
  öffnen. ADR 0035 ist inzwischen angenommen, ersetzt ADR 0034 formal und
  übernimmt alle nicht ausdrücklich korrigierten ADR-0034-/ADR-0033-/ADR-0032-
  Regeln; ADR 0034 bleibt mit bytegleichem Hauptteil ab `## Kontext` als
  historische Entscheidungsebene erhalten. Die getrennte netzwerkfreie
...
- ✅ In diesem Slice weder Recordvorlage noch Record, Controller, Harness,
  ...
  ausgeführt.
- ⬜ Damals sollte als nächster Slice ausschließlich die passive
  Diagnosefoundation in einem vollständig netzwerkfreien
  Implementierungsslice erstellt und getestet werden. Diese Folgereihenfolge
  wurde vor der Implementierung durch ADR 0031 und anschließend ADR 0032
  präzisiert.

### Historischer abgeschlossener Korrekturentscheidungsslice: Diagnostic Envelope and Observation Completion Boundary / ADR 0031
...
- ⬜ Der damals nächste Slice war ausschließlich die vollständig netzwerkfreie
  Implementierung und Prüfung der passiven Diagnosefoundation. Dieser Stand
  wurde vor einer Implementierung durch ADR 0032 ersetzt.

### Abgeschlossener Entscheidungsslice: Diagnostic Capture, Timing and Projection Determinism Boundary / ADR 0032

- ✅ ADR 0032 am `2026-09-02` als reinen Dokumentationsslice angenommen. Er
  ersetzt ADR 0031 formal und übernimmt sämtliche nicht ausdrücklich
  korrigierten ADR-0030-/ADR-0031-Regeln. ADR 0031 bleibt mit bytegleichem
  Hauptteil ab `## Kontext` historische Ebene; ADR 0020, ADR 0028, ADR 0029 und
  der historische Evidence-Record bleiben unverändert. Foundation, Adapter und
  Lauf sind weder implementiert noch autorisiert.
- ✅ Schema 1, Recordtyp und sämtliche Kardinalitäten unverändert gelassen: 17
  Rootfelder, sechs CDP-Operationen, 17 Integritätschecks, neun Requestzähler
  plus Sequenz, zehn Stages, drei Clock-Domänen, exakt 20 Cleanup-IDs und fünf
  Findings. Die exakte Fortführung ohne Versionswechsel ist nur zulässig, weil
  weder eine Implementierung noch eine konforme persistierte Instanz existiert.
  `overallGate` bleibt vor und nach der Diagnose `FAIL`;
  `causeStatus` bleibt ausnahmslos `CAUSE_NOT_PROVEN`.
- ✅ Einen einzigen globalen Setupcap von exakt `6.000 ms` über
  `Target.getTargets`, `Target.attachToTarget` und `Network.enable` entschieden.
  Vor dem einzigen GetTargets-Sendeversuch werden Cap-/Clock-Fähigkeit
  validiert und scharf geschaltet, `m_setup` genau einmal erfasst,
  `t_setup := 0` gesetzt, der konkrete Timer auf `m_setup + 6000` armiert und
  Stage 1 `observer-armed` gesetzt; erst danach entsteht der nichtblockierende
  Command-Intent. Der Setup bleibt
  strikt sequenziell mit höchstens einem ausstehenden Kommando und setzt den
  Cap nicht pro Kommando zurück. Eine fehlende Cap-/Clock-Fähigkeit ergibt `V`
  oder keinen gültig gestarteten Versuch; die pure Foundation besitzt diese
  reale Fähigkeit nicht.
- ✅ Für jede antwortartig eingereihte Nachricht beim Dequeue vor Reflection
  ...
  Nur `d < 6000` erlaubt die begrenzte synchrone Prüfung gegen die in ADR 0032
  und dem Living Contract geschlossene Setupantwort-Grammatik; bei
  `d >= 6000` gewinnt der Cap bei ungelesenem Inhalt. Ungültige, negative,
  rückläufige oder werfende Clockwerte ergeben `V`. Nur drei eindeutig
  korrelierte, erfolgreiche und vollständig validierte Antworten ergeben
  `setupReady`. Bei `Target.getTargets` wird zuerst die Anzahl der `page`-
  Kandidaten mit exakt gebundener URL unabhängig von `attached` bestimmt; nur
  bei genau einem wird `attached === false` geprüft und danach `targetId`
  gebunden. Eine Antwort gilt erst am verarbeiteten Setupcap oder nach
  irreversibler Verbindungsschließung als fehlend, nicht schon bei leerer Queue
  oder bloßem Noch-nicht-Eintreffen. Fehlende oder terminal unbrauchbare
  Setupergebnisse werden ohne bestätigten Obserververstoß `U` und damit
  `UNPROVEN/inconclusive`.
- ✅ Die Abschlussformeln auf `setupClosed := setupReady || U || V` und
  `observationClosed := V || U || C` totalisiert. `Runtime.evaluate` darf nur
  nach `setupReady` und höchstens einmal gesendet werden; erst sein bestätigter
  Sendeübergang startet das getrennte 6.000-ms-Capturefenster. `S && N` ist nur
  Produkt-Evidence-Completeness und schließt den Observer nicht. Ohne sticky
  bestätigtes `V` bleibt das vollständige Capture bis zum verarbeiteten `C`
  aktiv; späte Antworten werden verworfen.
- ✅ Für `U` `captureWindowState: not-started`, null Evaluation, Factory und
  Stimulus, `zero/unproven` für gegatete Downstream-Kommandos, `unknown` für
  nicht zuverlässig beobachtete Networkcounts und `incomplete` für die Sequenz
  festgelegt. Stage 1 bleibt `observed/match`; nur die gegateten Stages 2 bis 8
  werden `not-observed/unproven` mit `null` für Timing und `receiptOrder`. Der
  Record beschreibt genau einen Diagnoseversuch und höchstens einen Stimulus;
  `PASS` und jeder nicht-inkonklusive Befund verlangen exakt einen.
  Abschlussgründe umfassen mindestens `setup-cap`,
  `setup-terminal-unproven`, `capture-cap` und `confirmed-violation`.
- ✅ Einen tief eingefrorenen Pre-Cleanup-Observation-Snapshot und ein danach
  getrennt erzeugtes Cleanup-Ledger entschieden. Cleanup beginnt auch nach `U`
  aus dem tatsächlich erreichten partiellen Setupzustand; die 20 Cleanup-IDs
  und der eigene 60.000-ms-Finalisierungscap bleiben unverändert. Der
  Finalrecord wird erst nach Cleanupfinalisierung frisch aus beiden
  unveränderten Projektionen zusammengesetzt.
- ✅ Die exakte 10-ms-Timingfunktion mit getrennten Nullpunkten, Stage-/Layer-/
  Clock-Matrix und ohne Cross-Clock-Verrechnung gebunden. Die Foundation-
  Hashdomäne verwendet rohe Inhaltsbytes des später tatsächlich geladenen
  Pfads `scripts/browser/browserSyncTransportRuntimeDiagnosticObserver.js`;
  `evaluationSha256` verwendet UTF-8 ohne BOM über exakt den gesendeten
  primitiven Expression-String.
- ✅ Die Main-World-Ausgabe auf den geschlossenen verschachtelten By-Value-Baum
  aus `preTransportContext`, `execution` und optionalem `settlement` begrenzt.
  Untrusted CDP-Hüllen und jedes konsumierte Feld werden descriptorbasiert als
  eigene Data-Properties geprüft; verbotene Inhalte werden nicht gelesen. Das
  bestätigt den Eingangsgraphen weder als gewöhnlich oder Plain Data noch als
  proxyfrei, parser- oder pipe-authentisch; nur die frische
...
- ✅ Die totale Replayrelation `adr-0032-causal-replay-v2` auf exakt 59
  Vergleiche erweitert: acht Artefakte einschließlich Frontend-Runtime-Source-
  Set, separat `repository.state` und 50 Kontextwerte einschließlich
  `toolchain.vite.lockfileVersion`.
- ⬜ Im damaligen Stand nach Annahme von ADR 0033 sollte die reine,
  importinaktive und vollständig netzwerkfreie Effects-as-Data-Foundation als
...
### Abgeschlossener Entscheidungsslice: Diagnostic Foundation Effects Protocol Boundary / ADR 0033

- ✅ ADR 0033 am `2026-09-03` als reinen Dokumentations- und
  Entscheidungsslice angenommen. Er ersetzt ADR 0032 formal und übernimmt alle
  nicht ausdrücklich korrigierten Regeln. ADR 0032 bleibt mit bytegleichem
  Hauptteil ab `## Kontext` als historische Entscheidungsebene erhalten. ADR
  0029, historischer Evidence-Record, Gesamt-`FAIL` und `CAUSE_NOT_PROVEN`
  bleiben unverändert. Weder Foundation noch Tests wurden implementiert;
  Adapter und Runtimevorgang bleiben nicht autorisiert. ADR 0035 hat ADR 0034
  inzwischen formal ersetzt; ADR 0034 und ADR 0033 bleiben mit bytegleichem
  Hauptteil als historische Entscheidungsebenen erhalten.

Die folgenden Haken kennzeichnen die angenommenen Entscheidungsinhalte, nicht
die Implementierung oder eine Runtimefreigabe.

- ✅ Die einzige spätere Factory auf exakt `{ effectPort, runBinding }` und den
  Port auf exakt `{ exchange }` geschlossen. Keine Realdefaults oder
  Factoryeffekte; `runBinding` wird vollständig in der Factory geprüft und
  primitiv kopiert. Factoryfehler sind ausschließlich synchrone statische
  Dependency-`TypeError`s ohne API, Promise oder `FoundationResult`; nach
  erfolgreicher Factory liefert jeder kontrollierte Runpfad ein lokales
  Promise ohne synchronen Throw. Ergebnis exakt frische tief eingefrorene
  `{ run }`-API und `run.length === 0`. Der erste Aufruf
  wird vor Arityprüfung Owner und transferiert bei gültiger Arity atomar
  `capturedExchange -> activeExchange`; falsche Erst-Arity und Nicht-Owner-
  Aufrufe bleiben effekt- und cleanupfrei.
- ✅ Die Referenzgrenze korrigiert: Genau eine callerlieferte Capability darf
  zeitlich disjunkt erst als `capturedExchange`, dann als `activeExchange`
  erhalten bleiben. Sie wird exact-once erfasst, der Container verworfen, nur
  über erfasstes Apply/`undefined`/Frozen Intent aufgerufen und vom Owner nach
  dem letzten Exchange vor Resulterfüllung gelöscht. Nicht-Owner berühren sie
  nicht; sie wird nie ausgegeben oder als Provenienz verwendet.
- ✅ Vier transiente Referenzrollen erlaubt: Promise-Kandidat, aktueller
  Fulfillmentgraph jeder Antwortart, unreflektierter Dequeuegraph während des
  Clock-Samples und synchron benötigte Nachfahr-/Descriptor-/Prototypreferenzen.
  Nur die festgelegte Clock-Überlappung ist zulässig; keine Inputreferenz darf
  Result, Projection, Snapshot oder Ledger erreichen.
- ✅ Den einzigen Effectport auf sieben Intentarten mit geschlossenen
  Payload-, Ack-, Fehler-, Dubletten- und Übergangsprofilen begrenzt:
  Capabilityprobe, Clockprobe, Cap Arm/Cancel, Protocol Send, Dequeue und
  Cleanup Step. Intents beginnen bei ID `1`, sind lückenlos, frisch,
  gewöhnlich, geschlossen, tief eingefroren und nicht persistent. Höchstens
  ein Kandidat mit bestandenem
  `currently-observable-local-native-promise-profile` darf ausstehen. Leere
  OwnKeys, erwarteter lokaler Promiseprototyp und unveränderte Constructor-/
  Species-/Then-Descriptoren werden exact-once vor der einzigen nativen
  `then`-Anwendung geprüft; Handler geben nur `undefined` zurück. Das Profil
  behauptet weder Same-Realm-/Erzeugungsrealm-, Constructor-, Subclass-, Native-
  noch Proxyfreiheit. Ewig pending bleibt bewusst ohne Foundation-
  Livenessgarantie; ein getrennter hostabhängiger `unhandledrejection`-/
  `unhandledRejection`-Restkanal bleibt ausdrücklich offen.
- ✅ Die Lease vollständig als
  `idle|observable-pending|settlement-unobservable|closed` geschlossen. Nur
  `idle` darf den Port aufrufen; erst der Eintritt in einen kontrollierten
  Handler ist `observed-settlement` und gibt sie vor der intent-spezifischen
  Transition frei. Ein gültiges pending Promise hält den Run pending.
  Synchroner `exchange`-Throw, malformed Promise-Kandidat, Promiseprofil-
  Reflectionthrow oder native-Then-Throw vor Handlerzutritt ist
  ...
  gelöscht, und Cancel, Retry sowie jeder Folge-Exchange sind verboten. Ein intern angeforderter
  zweiter Exchange wird vor Intent, ID, Count und Port unterdrückt. Beim
  Settlement des alleinigen Originals entsteht ohne nutzbares `idle`-Fenster
  je nach Phase ein statischer Pre-start-Fehler, `V` mit lokalem Cleanup oder
  post-`O0` nur `cleanupViolation`; ein möglicher ungelesener Send-Ack bleibt
  `unknown`, nie `multiple`.
- ✅ Alle erreichbaren kontrollierten Rejection-Tupel ausschließlich als
  `observed-settlement` geschlossen: Probe und
  ...
  Best-effort-Cancel,
  ein unobservables Arm-Settlement setzt dagegen das vollständige Closed-Tupel
  und endet ohne Cancel oder Exchange. Setup-Send, Setup-/Capture-Dequeue-Clock,
  Setup-/Capture-Dequeue und Capture-Arm setzen während Observation zuerst den
  spezifischen `V`-Zustand und quieszenzieren danach den bekannten Cap genau
  einmal vor `O0`; exakter, beobachtet fehlerhafter, unobservabler und pending
  Cancel besitzen je einen eindeutigen Ausgang; der unobservable Ausgang setzt
  das Closed-Tupel und erlaubt keinen Folge-Exchange. Beobachtete Cleanup-
  Rejections verändern ausschließlich Ledger und bei offenem Port sichere
  Folgeschritte; unobservable Settlements finalisieren portlos. Eine Rejection des bereits
  laufenden Cancels erzeugt nie einen zweiten Cancel; jeder nicht aufgeführte
  Tupel ist intern unmöglich und allein dem Catch-all vorbehalten.
- ✅ Capture-Arm-Settlement getrennt totalisiert: beobachtete Rejection oder
  malformed Ack setzt `V`, Evaluate `zero` und `activation-unknown` vor dem
  einmaligen Cancel; unobservables Arm-Settlement setzt das vollständige
  Closed-Tupel und `V`, verbietet Cancel und weiteren Portaufruf
  und führt unmittelbar zu `O0` mit rein lokalem Cleanup; pending bleibt
  pending.
- ✅ `runBinding` auf neun Keys und exakt 59 kanonische Replayoperanden
  begrenzt. ADR und Living Contract schreiben Vergleichsbasis, historischen
  Wert, Typ-/Längengrenze, Projektion und Cross-Field-Invarianten aus. Caller
  liefern weder historische Werte noch Status, Gates, Findings, Counts,
  Provenienz, IDs oder Evaluationbytes.
- ✅ `attemptStarted` erst nach positiver Capability, gültigem `m_setup`,
  sicherer Deadline, bestätigtem Setupcap und eingefrorener Stage 1 gesetzt.
  Frühfehler erzeugen keine Projection, keinen Snapshot und kein Cleanup;
  bestätigte Verstöße nach `attemptStarted` und vor `O0` sind sticky `V`, nach
  `O0` bleibt der Snapshot unverändert und nur der Cleanup-Ledger darf noch
  `cleanupViolation` beziehungsweise `cleanup-terminal-failure` aufnehmen.
- ✅ Exact-once-OwnKeys-, Prototyp- und Descriptorreflection für alle
  geschlossenen Inputs und Arrayelemente sowie höchstens einmalige allowlistete
  Descriptorreads offener CDP-Hüllen festgelegt; freie Reads und Rereads
  verboten.
- ✅ Capture pending vor Evaluate armiert und den einzigen positiven
  Evaluate-Sende-Ack `sent-and-capture-cap-started` atomar an Send und Capstart
  gebunden. Dequeuewerte bleiben bis zu ihrem genau einen phasenpassenden
  Clock-Sample unreflektiert. Für Setup/Cleanup gewinnt
  `m_answer >= deadline` einschließlich Gleichheit vor Envelope-Reflection;
  unterhalb der Deadline ist ein korreliertes Capereignis vorzeitig. Die zehn
  Zustände lauten exakt `absent|arm-pending|armed|pending-activation|active|activation-unknown|cancel-pending|cancelled|fired|terminal-unknown`;
  alle vier Cancelausgänge, falsche, vorzeitige, späte und doppelte Cap-IDs,
  höchstens ein Cancel je Arm, kein Retry und kein zweiter Cancel sind total.
- ✅ Die durch den kontrollierten Handler als `observed-settlement` beobachtete
  Evaluate-Exchange-Rejection intent- und zustandsspezifisch vor jedem
  allgemeinen Catch-all geschlossen: Lease
  `idle`, `closeClass: V`, Cap `activation-unknown`, danach genau ein Cancel
  der gebundenen `armIntentId` und erst danach `O0`. Dabei gelten exakt
  ...
  `captureWindowState: truncated` und `publicSettlement: null`. Kein Send oder
  Nichtsenden, keine Aktivierung oder spätere Ausführung wird behauptet. Der
  exakte Cancel-Ack setzt `cancelled`, erhält `V` und führt zu `O0`;
  beobachtete Cancel-Rejection oder malformed Ack setzt `terminal-unknown`,
  erhält `V`, führt zu `O0` und danach zu Cleanup mit negativer Capbehauptung;
  unobservables Cancel-Settlement setzt das vollständige Closed-Tupel, erhält
  `V`, führt zu `O0` und ausschließlich lokaler portloser Cleanup-
  Terminalisierung ohne zweiten Cancel oder Exchange; ein gültiges pending Cancel hält den Run
  unbegrenzt pending. Kein Ausgang erlaubt ein zweites Evaluate oder ein
  Capture-Dequeue.
- ✅ Evaluate-Intent, bestätigten Send, Reply, Main-World-Auswertung,
  Factoryaufruf, Transportaufruf und Networkrequest getrennt. Controller-
  abgeleitete `factoryCallCount` und `transportCallCount` sind jeweils
  `zero|one|unknown`; `zero` ist nur konstruktiv beweisbar, `unknown` nie
  callerlieferbar. Intent ist kein Sendbeweis, POST kein Transportaufrufbeweis
  und Reply kein Beweis eines weiteren Sends.
- ✅ Das erste irreversible Capture-Connection-Close als
  `U/capture-terminal-unproven` mit `truncated` und
  `observationCloseReason: capture-terminal-unproven` totalisiert. Ein noch
  live gebundener Capturecap wird dabei vor `O0` genau einmal storniert;
  exakter Ack erhält `U`, beobachteter Fehler promoviert zu `V`; unobservables
  Settlement promoviert ebenfalls zu `V`, setzt aber das Closed-Tupel und lässt
  nach `O0` nur portlosen Cleanup zu. Pending bleibt pending. Gewöhnliches
  Setup-`U|V` und Capture-`V` frieren bei Lease `idle` und offenem Port dagegen
  zuerst `O0` und stornieren danach. Dequeue-Reihenfolge allein
  entscheidet Connection-Close gegen Cap; `S && N` schließt Capture nicht.
- ✅ Ressourcen- und Descriptorcaps geschlossen: `targetInfos` 128,
  flüchtige IDs 256, CDP-URLs 2048, HTTP-Methodentokens 16 Codeunits, Dequeues
  128; nur vier Networkevents und keinerlei Header-, Body- oder
  `loadingFailed.errorText`-Lesung.
- ✅ Cleanup aus Frozen Snapshot und getrenntem Ledger totalisiert. Detach folgt
  jedem terminalen Disable-Sendeversuch; für alle zwölf externen Fakten gilt
  `false -> failed`, `true -> unproven`. Nach `O0` bleiben `U`, `V`, `C`,
  `closeClass`, Stages 1 bis 8, Replay, Settlement, Budget, Counts und alle
  Observationwerte unveränderlich; Clock-, Routing- und Portausgänge wirken
  nur über Cleanup-Ledger, Checkzustände und Candidateableitung, niemals
  rückwirkend als `V`. `cleanupViolation` und `cleanup-terminal-failure`
  entstehen nur auf den dafür festgelegten Pfaden. Die beiden Dequeue-Purposes
  `await-cleanup-protocol-responses(openCommandIds)` und
  `await-cleanup-fact(checkId)` halten Disable-/Detach-Antworten von externen
  Fakten getrennt; unbekannte, doppelte, falsch variantierte oder unlesbare
  IDs erhalten keine zufällige Checkattribution. Connection-Close, offene
  Commands und sichere externe Steps folgen der geschlossenen Routingmatrix.
  Nach terminalen Checks 1 bis 19 wird der Cleanupcap bei Lease `idle` und
  offenem Port genau einmal storniert; nur exakter Ack erlaubt
  `controller-clock-sample(cleanup-completion-after-cap-cancel)`. Ein
  unobservables Cancel-Settlement erlaubt weder diesen noch einen anderen
  Exchange. Jeder nicht vollständig gültige Completion-Sample setzt zwingend
  `relativeMilliseconds: null` und `timingState: unavailable`; kein vorheriger
  oder provisorischer Timingwert wird erhalten, gerundet, projiziert, gehasht
  oder serialisiert, und `receiptOrder` beweist niemals gültiges Timing. Check 20,
  Stage 10, `cleanup.result` und `cleanupFinalized` enden exakt über
  `all-steps-terminal`, `cleanup-cap` oder `cleanup-terminal-failure`; kein
  `pending` wird ausgegeben.
- ✅ Den privaten einzeiligen ASCII-Evaluationstring mit 4259 UTF-8-Bytes und
  SHA-256
  `a623ffafee8dfcbc1d2ddc374cc35f0dbf800defd97619a3b58337d972090f7b`
  bytegenau dokumentiert, aber nicht ausgeführt. `foundationSha256` bleibt
  `null`; Actual-Send-, Commit-, Parser-, Pipe- und Adapterbindung `unproven`.
- ✅ `FoundationResult` als Null-Prototyp-Record auf sieben Felder ohne
  Own-`then`, mit `NOT_EVIDENCE` und beiden
...
### Historischer Entscheidungsslice: Diagnostic Foundation Grammar, Derivation and Testability Boundary / ADR 0034

- ✅ [ADR 0034](decisions/0034-browser-sync-transport-diagnostic-foundation-grammar-derivation-and-testability-boundary.md)
  wurde am `2026-09-04` angenommen und ersetzte ADR 0033 formal. ADR 0035 hat
  ADR 0034 am `2026-09-05` formal ersetzt; dessen bytegleicher Hauptteil ab
  `## Kontext` bleibt historische Entscheidungsebene. Alle nicht ausdrücklich
  korrigierten ADR-0034-/ADR-0033-/ADR-0032-Regeln gelten fort. Foundation,
  Tests und Adapter sind nicht implementiert, ein Runtimevorgang ist nicht
  autorisiert.
- ✅ Schema und Kardinalitäten bleiben unverändert:
  `schemaVersion: 1`, 17 Projection-Rootfelder, 59 Replayvergleiche, sieben
  Intents, sechs Protokolloperationen, 17 Integritychecks, zehn Stages, zehn
  Capzustände, 20 Cleanupchecks und ein Export. ADR 0029, sein Record,
  `overallGate: FAIL` und `causeStatus: CAUSE_NOT_PROVEN` bleiben unverändert.
- ✅ Die geschlossene Prototypgrammatik prüft geschlossene fremde
  Records nach `typeof/null -> Array.isArray false -> lokaler
  Object.prototype -> ownKeys -> Descriptoren` und geschlossene fremde Arrays
  nach `Array.isArray true -> lokaler Array.prototype ->
  Dense/Length/Keys/Descriptoren` jeweils mit exakt einmaligen Reads.
  Offene CDP-Hüllen/-Einträge und Promise-Kandidaten bleiben ihre ausdrücklichen
  Ausnahmen. `fresh` verlangt jeden Container konstruktiv neu, callerdisjunkt
  und ungeteilt; `deep frozen` umfasst nur foundationeigene Records/Arrays über
  Own-Data-Kanten. Proxy-, Realm-, Parser- und Pipeprovenienz werden
  nicht behauptet.
- ✅ Die ASCII-, Zeitzonen- und SemVer-Grammatiken sind geschlossen:
  `printable-ascii-v1` schließt `A<n>` auf `1..n` Codeunits
  `U+0020..U+007E` ohne Koerzierung oder Normalisierung;
  `iana-shaped-ascii-time-zone-v1` bestätigt nur `UTC` oder mindestens zwei
  ASCII-Komponenten der festgelegten Slashgrammatik bis 64 Codeunits, aber
  keine IANA-Mitgliedschaft; `S32` lässt ausschließlich
  `^(0|[1-9][0-9]*)\.(0|[1-9][0-9]*)\.(0|[1-9][0-9]*)$` bis 32
  ASCII-Codeunits ohne numerische Konvertierung zu.
- ✅ I1 bis I7 leiten privat exakt `match|mismatch|unproven` aus den
  geschlossenen URL-/Origin-/Loopback-/Port-/Endpoint-/Vite-/Profilrelationen
  ab. I8 bleibt reine private Provenienzgrenze ohne 60. Vergleich und
  ohne Authentizitätswirkung. `DIVERGED` gewinnt bei irgendeinem Row- oder
  I1–I7-Mismatch beziehungsweise widersprochener Kausaldeviation;
  `UNPROVEN` folgt danach bei irgendeinem offenen Row-/Invariantenwert;
  `EQUIVALENT` entsteht nur bei 59 Row-Matches, sieben Invarianten-Matches und
  bestätigter Abwesenheit unerklärter Abweichung.
- ✅ I1–I8, Replay, Observer, 17 Integritychecks, zehn Stages, Hash und Cleanup
  sind quellengenau totalisiert. Nur `protocolAllowlistOnly` und
  `closedPrimitiveProjectionConfirmed` können Foundation-v1 positiv by
  construction bestätigen; adapter-, parser-, pipe-, Actual-Send-,
  Ressourcen-, Runtime- und Abwesenheitsbehauptungen bleiben ohne gebundene
  Provenienz `unproven`. Stage 1 bleibt lokal; Stage 2 und 8 dürfen nur aus
  akzeptierter Main-World-v2-Execution beziehungsweise -Settlement, Stages 3
  bis 7 nur aus eindeutig attribuierten Networkereignissen stammen; Stage 9
  bleibt lokaler Cleanupstart nach `O0`, Stage 10 die bestehende totale
  Finalisierung. Fehlend, mehrfach/widersprüchlich und bekannter Gegenwert
  werden getrennt als `not-observed`, `ambiguous` und
  `observed/mismatch` behandelt; Receipt Order darf Timing nicht ersetzen.
- ✅ Die globale Pauschale `kein Intent -> zero/unproven` entfällt.
  Konstruktiv nie aktivierte Ressource ergibt `zero/match`,
  möglicherweise aktive oder unbeweisbare Ressource ohne Intent
  `zero/unproven`, Intent ohne belegbaren Sendestatus `unknown/unproven`, ein
  exakter Ack `one/match`, mindestens zwei bestätigte Acks
  `multiple/mismatch`. Antwortdubletten erhöhen Sendcounts nicht.
- ✅ Die sendzustandsabhängige Ressourcenmatrix klassifiziert Attach sicher nie
  gesendet als `targetSessionClosed: confirmed` und Detach `zero/match`.
  Möglicherweise oder bestätigt gesendetes Attach ohne sicher
  bindbare Session ergibt dagegen `targetSessionClosed: unproven` und Detach
  `zero/unproven`. Nur ein korrelierter exakter Detach-Erfolg schließt
  eine gebundene Session sicher; fehlender/unklarer Ack, Rejection,
  malformed/unobservables Settlement, fehlende/malformed/doppelte/
  unkorrelierbare Antwort und Connection-Close beweisen dies nie.
  Dieselbe Trennung gilt für Enable/Disable. Bei gebundener Session
  und sicher nie gesendetem Enable entfällt Disable, Detach wird aber nach
  seiner Matrix versucht.
- ✅ Fünf rein foundationeigene Cleanupchecks werden bei jeder
  erfolgreichen selbstgeprüften Projection konstruktiv bestätigt: lokaler
  Cleanupstart und Ledger nach Frozen `O0`, geschlossene Object-Group-
  Zustände, verworfene Ack-/Dequeue-/Descriptorreferenzen und aus Outputs
  entfernte ephemere IDs. Ein interner Abschlussfehler endet statisch statt
  mit `ok:true`; externe positive Facts bleiben `unproven`.
- ✅ Öffentliche `candidateObserverGate: PASS`-Resultate und
  PASS-Findings bleiben konstruktiv unerreichbar; die produktive API darf nur
  `FAIL/observer-invalid` oder `UNPROVEN/inconclusive` ausgeben. Die private
  Ableitung muss trotzdem alle Präzedenzen und hypothetischen PASS-Findings
  enthalten. Nur eine im `finally` gelöschte serielle Temporärkopie exakter
  Produktionsquellbytes darf an einem eindeutigen lexikalischen Anker
  testlokale Exports ergänzen; permanenter Testexport, Produktseam,
  manipulierte PASS-Projection und Evidenznutzung bleiben verboten.
- ✅ Die spätere Pflichttestmatrix muss insbesondere Attach sicher nie
  gesendet, unbekannten Sendestatus, Senderejection, Ack ohne Antwort,
  malformed/doppelte/unkorrelierbare Antwort, Detach-Erfolg/-Fehler,
  Connection-Close ohne Detach sowie `zero/match` gegen `zero/unproven`
  getrennt prüfen. Aus einer fehlenden Session-ID darf kein Test auf eine
  geschlossene Session schließen.
- ✅ Netzwerkfreiheit umfasst nur neues Foundationmodul, fokussierte
  Foundationtests/-Effects und Diagnoselauf. Die vorgeschriebenen
  bestehenden Repositorysuites dürfen weiterhin ausschließlich ihre beiden
  unveränderten Loopback-Fixtures
  `tests/localSyncGatewayHttpServer.test.js` und
  `tests/n8nCloudIngressProbe.test.js` ausführen. Neue/externe/diagnostische
  Verbindungen, DNS, Listener, Browser, CDP, Vite, Diagnoseprozesse oder neue
  Childprozesse bleiben verboten.

### Angenommener Korrekturslice: Join and Internal Transition Testability Boundary / ADR 0035
...
- ✅ [ADR 0035](decisions/0035-browser-sync-transport-diagnostic-foundation-join-and-internal-transition-testability-boundary.md)
  ist `Angenommen – 2026-09-05` und ersetzt ADR 0034 formal. ADR 0034 bleibt
  mit bytegleichem Hauptteil ab `## Kontext` historische Entscheidungsebene;
  kein weiterer ADR wird ersetzt und alle nicht ausdrücklich korrigierten
  ADR-0034-/ADR-0033-/ADR-0032-Regeln gelten fort.
- ✅ Die Entscheidung schließt die Testbarkeitslücke zwischen der verpflichtenden
  internen Pending-Join-Matrix und der absichtlich engen öffentlichen API. Die
  Testkopie v2 darf über genau eine zusätzliche Exportdeklaration nur
  `deriveCandidateObserverGate`/1, `deriveCandidateFinding`/1,
  `createBrowserSyncTransportRuntimeDiagnosticRunMachine`/1 und
  `requestBrowserSyncTransportRuntimeDiagnosticExchange`/2 aus derselben
  unveränderten produktiven Maschine und Exchange-Grenze öffnen. Ein fünfter
  Inspector, permanenter Seam oder öffentlicher Zustand bleibt verboten.
- ✅ Die Konformitätskopie liegt als eindeutig benannte `.mjs`-Datei außerhalb
  des Repositorys, wird seriell per File-URL importiert und im `finally`
  nachweislich entfernt. Funktionskörper, Inputs, Konstanten und produktive
  Callsites bleiben bytegleich. Gesonderte Mutantenkopien sind davon disjunkt;
  Original, Kopien und Ergebnisse bleiben `NOT_EVIDENCE`.
- ✅ Die dynamische Matrix treibt dieselbe produktive Run-Machine über normale
  Effects-as-Data-Übergänge getrennt nach `prestart`, `observation` und
  `cleanup` zu genau einer `observable-pending` Lease. Der zweite echte
  Boundaryaufruf muss vor Intent-, ID-, Count-, Ledger-, Cap-, Snapshot-,
  Cleanup-, Capability- und Portwirkung stoppen. Gegen den Baselinesnapshot
  bleiben Intentanzahl, nächste ID, Portaufrufe, Sendcount und
  Capabilityaufrufe jeweils bei Delta null; ausschließlich die phasengenaue
  `pendingInternalExchangeViolation` wird gesetzt.
- ✅ Der finite Join-Nachweis umfasst exakt `3 × 2 × 3 = 18` Pflichtfälle:
  `prestart|observation|cleanup`, `fulfillment|rejection` und
  `pre-invocation|synchronous-post-invocation|observable-pending`. Das Promise
  ist vor `activeExchange` bereits gesettelt, wird nach Rücksprung der ersten
  Grenze ohne Yield gesettelt oder bleibt bis nach dem zweiten Boundaryaufruf
  pending. Dieser zweite Aufruf erfolgt stets synchron im selben Turn vor
  Handlerzutritt; Settlement allein verändert die Machine nicht.
- ✅ Fulfillment und Rejection werden in allen 18 Zellen ohne
  Payload-/Reason-Lesen geprüft. Danach bleiben Lease und Port
  geschlossen, `activeExchange: null`, keine weiteren Exchanges und lebende
  Caps `terminal-unknown`. Die Folgen sind statischer Prestartfehler ohne
  `O0`, sticky `V` mit Frozen `O0` und portlosem Observation-Cleanup oder
  unverändertes `O0` mit `cleanupViolation` und
  `cleanup-terminal-failure`. Späte/doppelte Handler sind `undefined`-No-ops.
- ✅ Das von den 18 settelnden Fällen getrennte Forever-pending-Oracle
  kombiniert eine endliche dynamische
  Präfixprobe mit exakt drei testlokalen Microtask-Checkpoints und die
  vollständige private Transitionstabelle. Nur zusammen schließen sie jeden
  Fortschritts-, Snapshot-, Cleanup-, Terminalisierungs- oder Resolvepfad aus
  `observable-pending` ohne kontrollierten Handler. Kein Bericht darf daraus
  eine empirisch beobachtete unendliche Zeitspanne ableiten; Timer, Timeout,
  `Promise.race`, Retry, Polling, zweiter Port und konkurrierender Resolver
  bleiben verboten.
- ✅ Eine mutationswirksame Matrix muss insbesondere fehlenden oder verspäteten
  Join-Guard, zweite Intent-/ID-/Count-/Ledger-/Port-/Capabilitywirkung,
  Freigabe zu `idle`, künstliches Owner-Settlement, vorzeitiges `O0` oder
  Cleanup, verlorene Phasenklassifikation und Mutation des bereits Frozen
  `O0` töten. Mutiert werden ausschließlich kontrollierte Testkopien, niemals
  der Produktionssource.
- ✅ Drei öffentliche Effects-as-Data-Regressionen werden für den späteren
  Implementierungsslice verbindlich: Networkmonotonie über
  `lastValidBrowserNetworkTimestamp` einschließlich `10 -> 12 -> 11`;
  sticky unbewiesene Attribution einer exakten Endpoint-Response mit
  unbekannter Request-ID ohne stillen Drop; und wohlgeformte beziehungsweise
  malformed Dubletten von `Target.getTargets` mit getrennten `U`-/`V`-
  Ableitungen. Eine Antwortdublette ist nie ein zweiter Intent oder Send-Ack;
  bei exakt einem gültigen Ack bleibt die Operation `one/match`.
- ✅ Schema 1, öffentliche Factory, ein Export, sieben Intentarten, sechs
  Commands, 59 Replayvergleiche, 17 Integritychecks, zehn Stages, zehn
  ...
  `PASS` und PASS-Findings bleiben geschlossen. Die ADR-Annahme selbst
  implementierte weder Foundation noch Tests. Der danach getrennt autorisierte
  Foundation-Slice ist nun netzwerkfrei implementiert und mit 422/422
  fokussierten Tests geprüft; Adapter-ADR, Adapterimplementierung und Lauf
  bleiben nachgelagert. `overallGate: FAIL` und `CAUSE_NOT_PROVEN` bleiben
  unverändert.
- ✅ Die unveränderten Regressionen bestehen mit 423/423, 466/466 und 735/735
  Tests; die vollständige serielle Suite mit 2177/2177, exakt `1755 + 422`.
  Der Produktions-Build transformiert weiterhin 46 Module,
  `bundle:n8n:check` ist driftfrei, alle gebundenen Hashes bleiben
  unverändert und drei unabhängige read-only Daybreak-xhigh-Reviews meldeten
  keinen belegten Befund. Im Foundation-Slice fand kein Browser-, CDP-,
  Gateway-, Vite-, Netzwerk- oder Diagnoselauf statt.

### Angenommener Dokumentationsslice: Runtime Diagnostic Adapter Boundary / ADR 0036
...
- ✅ Entschieden sind genau ein import- und factory-inaktiver
  ...
  fataler UTF-8-/Duplicate-Key-Parser, eine FIFO, lokale Write-Acks und die
  unveränderten Setup-/Capture-/Cleanupcaps.
- ✅ Clock und Gates sind präzisiert: Dequeue liest nicht; der nachgelagerte
  Foundation-Clockintent liest exakt einmal denselben Wert für Foundation und
  Ledger. Setup/Cleanup nutzen `>=`, Capture nur `cap-fired`. Bestätigte
  Verletzung ergibt vor Evidenzdemotion und bei jedem Stimuluszustand
  `FAIL/observer-invalid`; exakt ein Stimulus gilt nur für PASS/die drei
  Produktfindings. `derivation-conformance` öffnet Factory plus
  Gate/Finding/Finalizer und sperrt den Factoryselector synchron vor Owner,
  API, Promise oder Hostzugriff. Das disjunkte
  `virtual-runtime-conformance`-Profil öffnet Factory plus denselben
  produktiven Owner, die vollständige Producer-Eventgrenze und einen
  einmaligen virtuellen Capabilityinstaller. Sein Profil umfasst Entropie mit
  getrennten 17-/15-Byte-Reads, Clock, Process-/Environment-/Runtimequellen,
  Scheduler, Pipe, Launcher und geschlossene Ressourcenoperationen; mutable
  Bytes, opaque Handles, Raw-Fixtureevents und Foundationwerte sind getrennt.
...
- ✅ Der Adapter würde Launcher-, Handle-, Profil-, Environment- und
  ...
  nach terminalem Cleanup frisch ableiten. Callerwerte und positive
  Fake-Seams bleiben ausgeschlossen; Writer und Persistenz sind getrennt.
- ✅ Der rohe Gateway-Port-Environmentwert bleibt `8787` mit vier Codeunits;
  ausschließlich Replayoperand 53 erhält die separate Sechs-Codeunit-
  Projektion mit umgebenden U+0022. Trimmen, Double-Wrap und
  Ziffernormalisierung sind verboten.
- ✅ `browser.engineBuild`, globale Portfreiheit, effektive Proxy-/VPN-/
  Policy-/Extension-/Permission-/Service-Worker-/Cachewerte und unabhängige
  Adapterattestierung bleiben ohne authentische Quelle `UNPROVEN`. Ein
  siebter CDP-Command, Probe-Listener, zweiter Timer oder positiver Schluss aus
  frischem Profil, Childexit beziehungsweise `cleanup-fact:true` ist
  verboten.
- ✅ Node Core bindet hier weder Windows-Prozessnachfahren über einen Job-
  Owner noch Profil-/Fragmentlöschung handle-relativ. Nach möglichem Spawn
  beziehungsweise Create bleiben die betroffenen Cleanupchecks `UNPROVEN`;
  Root-Childexit und Pfad-Vorcheck dürfen keinen Erfolg erfinden.
- ✅ Einzelne Cancelpayloads und spätere Projektionen tragen keine
  ...
  Zeitpunkt für alle Pfade auswählen; ein Promisezaun ist für Old-Cap und
...
11. ✅ **ADR 0030 / ADR 0031 / ADR 0032 – totalisierte passive Runtime-
   Diagnosegrenze:** den unabhängigen Diagnoseversuch, `T_replay ≡R T₀`,
   externe Stages, das exakte Requestbudget und den
   `BrowserTransportDiagnosticRecord` entschieden; ADR 0032 ersetzt ADR 0031
   formal und totalisiert Setupcap `U`, vollständiges Capture bis `V` oder `C`,
   Timing, Hashdomänen, verschachtelte Projektion, descriptorbasierte CDP-
   Grenzen, Snapshot/Ledger, 59 Replayvergleiche und Cleanup, ohne Foundation,
   Adapter oder Lauf zu implementieren oder zu autorisieren.
12. ✅ **ADR 0033 – Diagnostic Foundation Effects Protocol Boundary:** als
   angenommene Entscheidung die einzige Factory-/Port-API,
   `capturedExchange`, sieben Intents, Lease/Pending-Join, intent-spezifische
   Evaluate-Rejection, Capture-Close, Frozen Snapshot, Cleanup-Purposes,
   geschlossene Projection, Referenzinvariante und `NOT_EVIDENCE`
   dokumentiert; ADR 0032 formal ersetzt und dessen nicht ausdrücklich
   korrigierte Regeln übernommen. ADR 0032 bleibt mit bytegleichem Hauptteil ab
   `## Kontext` als historische Entscheidungsebene erhalten.
13. ✅ **ADR 0034 – historische Grammar-, Derivation- und
   Testability-Totalisierung:** vollständige Prototyp-/Stringgrammatiken,
   I1–I8-/Replay-, Observer-, Integrity-, Stage-, Hash-, Operations- und
   Cleanupableitungen, die sendzustandsabhängige Target-/Networkmatrix,
   Candidate-PASS-Unerreichbarkeit, temporäre Testkopie und präzise
   Netzwerk-/Regressionsgrenze verbindlich totalisiert; vormals ADR 0033 formal
   ersetzt und nun selbst durch ADR 0035 ersetzt, mit bytegleichem Hauptteil als
   historische Entscheidungsebene erhalten.
14. ✅ **ADR 0035 – angenommene Pending-Join-Testbarkeitskorrektur:** die
   Testkopie v2, die vollständigen 18 finiten Phasen-/Outcome-/Zeitlagenfälle,
   das davon getrennte zweiteilige Pending-Oracle, die Mutationsmatrix und drei
   Ableitungsregressionen dokumentieren, ohne Foundation oder Tests zu
   implementieren; ADR 0034 formal ersetzen und dessen bytegleichen Hauptteil
   als historische Entscheidungsebene erhalten.
15. ✅ **Reine netzwerkfreie Effects-as-Data-Foundation:** den importinaktiven
   Controller und die geschlossene Projektions-/Integritätslogik ohne
   Realdefaults oder I/O implementiert und mit 422/422 fokussierten Tests
   geprüft.
16. ✅ **ADR 0036 – angenommene dokumentarische Adaptergrenze:** Raw-
...
29. ⬜ **Provideradapter als getrennte Slices:** OpenAI-, lokales-Modell- und
   n8n-Adapter unabhängig voneinander entscheiden und implementieren; kein
   Adapter ist durch ADR 0023 autorisiert.
30. ⬜ **Private Daten, weitere Aktionen, Tools und Nebenwirkungen:** erst nach
...
ADR 0031 ist als historische Ebene angenommen; vor Foundation oder Lauf wurde
dieser Stand durch ADR 0032 ersetzt.
ADR 0032 ist am `2026-09-02` als reine Dokumentationsentscheidung angenommen.
Er ersetzt ADR 0031 formal und totalisiert die Setup-, vollständige Capture-,
Timing-, Hash-, Projection-, Replay-, Zwei-Phasen-Snapshot-/Ledger- und
Cleanupgrenze. Der getrennte globale Setupcap beträgt 6.000 ms und schließt
ohne `setupReady` als `U`; `Runtime.evaluate` bleibt bis dahin verboten. Nach
seinem höchstens einmaligen bestätigten Send bleibt das Capture ohne sticky
`V` bis zum 6.000-ms-Cap `C` offen. Der getrennte Cleanup-Finalisierungscap
bleibt 60.000 ms. Schema 1 und sämtliche Kardinalitäten bleiben unverändert;
die Replaymenge umfasst exakt 59 Vergleiche. Weder Foundation, Adapter noch
Lauf sind implementiert oder autorisiert. Das ADR-0029-`overallGate` bleibt
`FAIL`; `causeStatus` bleibt ausnahmslos `CAUSE_NOT_PROVEN`.
ADR 0033 ist am `2026-09-03` als nachfolgende reine Dokumentationsentscheidung
angenommen worden und hat ADR 0032 formal ersetzt. ADR 0035 ist am
`2026-09-05` angenommen worden, ersetzt ADR 0034 formal und übernimmt alle
nicht ausdrücklich korrigierten ADR-0034-/ADR-0033-/ADR-0032-Regeln. ADR 0034
...
Pfadcleanupfähigkeit die bewusst offenen Grenzen schließen; unabhängige
Adapterausgabestille bleibt ebenfalls ein sichtbarer Laufblocker. Erst anschließend
können Zielbrowser, `T_replay`, Observer, höchstens ein Transportstimulus,
