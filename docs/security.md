...
| Feld | Wert |
| --- | --- |
| Projektphase | `v0.3.0 – ADR-0037-D_K4-Foundation implementiert und netzwerkfrei geprüft – 2026-09-08; 595/595 fokussierte und 2350/2350 Gesamttests; unabhängiger Implementierungsreview ausstehend; ADR 0036 vorgeschlagen und nicht annahmereif; Adapter und Diagnoselauf geschlossen; overallGate FAIL; causeStatus CAUSE_NOT_PROVEN` |
| Geltungsbereich | Version 1 und Portfolio-Demo |
| Status | ADR 0037 ist angenommen; der exakte Zwei-Felder-Port und die synchrone Notification sind implementiert und fokussiert geprüft. Der getrennte unabhängige Implementierungsreview steht aus. ADR 0036 bleibt vorgeschlagen und nicht annahmereif; das Foundationdelta belegt keinen Adapter und keinen Diagnoselauf. |
...
formal. Das Delta ist nun im gesondert beauftragten netzwerkfreien
Foundationimplementierungsslice umgesetzt und geprüft. Der Effectport besitzt
exakt `{ exchange, observationClosed }`; die synchrone argumentlose
Notification folgt unmittelbar auf den frisch tief eingefrorenen `O0` und
vor Old-Cap-Scan, Cleanup-Ledger sowie regulärem oder portlosem Cleanup.

Die beiden getrennten Capabilityrollen werden einmal descriptorbasiert
erfasst und beim ersten gültigen Owneraufruf atomar transferiert. Für
`observationClosed` wird der eigene nicht aufzählbare Data-Descriptor
`length` exakt einmal auf den primitiven Wert `0` geprüft. Der private
Konstruktor erhält exakt
`{ activeExchange, activeObservationClosed, runBinding }`. Vorzeitig
geschlossener Exchange-Port und die Notification besitzen getrennte
Lebenszyklen; der Notification-Slot wird vor dem einzigen Aufruf gelöscht.
Fehler, nicht-`undefined`-Rückgaben und interne Reentranz setzen nach dem
unveränderten `O0` sticky `cleanupViolation` mit `FAIL`-Vorrang. Der
Notificationguard liegt vor Pending-Join, Profilreflection, IDs und Effekten.
Während `invoking` setzt er ausschließlich
`observationNotificationViolation` und kehrt mit `undefined` zurück; erst der
...
`get`-, `getPrototypeOf`-, `ownKeys`- und
`getOwnPropertyDescriptor`-Zählern. Die 18 Joinfälle, die dreifache
Microtask-Pending-Präfixprobe und ihre strukturelle Transitionstabelle bleiben
erhalten. Diese Foundationnachweise liefern keinen Adapter- oder
`A_obs`-Freeze-Nachweis.

Die Regressionen bestehen mit 423/423 Transporttests, 466/466 Tests von
SyncService und Transport sowie 735/735 Tests der sechs Sync-Suites. Die
vollständige serielle Suite besteht mit 2350/2350 (`1755 + 595`), jeweils ohne
Fehlschlag, Cancellation, Skip oder Todo. Der Produktions-Build transformiert
exakt 46 Module; `bundle:n8n:check` besteht driftfrei. Die alten 422/422 und
...
Foundation und neue Tests wurden vollständig netzwerkfrei geprüft. Die einzige
Netzwerkaktivität der vorgeschriebenen Regressionen lag in den unveränderten
etablierten Loopback-Fixtures in `tests/localSyncGatewayHttpServer.test.js`
und `tests/n8nCloudIngressProbe.test.js`; deren Cleanupprüfungen bestanden.
Es gab keinen manuellen Gateway-, Browser-, CDP- oder Diagnoselauf und keinen
externen Netzwerkzugriff.
...
Die implementierte zweite Capabilityrolle ist ausschließlich für diesen
Kontrollübergang bestimmt und verleiht keine Daten- oder Hostberechtigung.
Fremde Rückgaben werden weder reflektiert noch assimiliert. Ein fremder nie
zurückkehrender Callback bleibt nicht präemptierbar; Hoststille,
Sandboxwirkung und tatsächlicher Adapterfreeze bleiben unbeweisbar.
Prozessbaumownership und handle-relative Pfadlöschung werden durch ADR 0037
nicht gelöst.

Der nächste Schritt ist ausschließlich der getrennt zu beauftragende
unabhängige Daybreak-Implementierungsreview mit Reasoning `xhigh`; er steht
aus. Danach folgen Jans manueller Featurecommit und ein gesonderter
ADR-0036-Load-/Hash-/Testvertragsabgleich mit erneutem unabhängigen Review vor
einer möglichen Annahme durch Jan. ADR 0036 bleibt vorgeschlagen und nicht
annahmereif. Adapterimplementierung, Adaptertests, `A_obs`, Writer,
Diagnoselauf, Browserkomposition und End-to-End bleiben geschlossen.
...
### Durch ADR 0023 bis ADR 0025, ADR 0028 und ADR 0035 entschiedene lokale Agenten-, Transport- und Providergrenze
...
Ursache bleibt `CAUSE_NOT_PROVEN`. ADR 0035 ersetzt ADR 0034 formal und
...
fokussierten Tests geprüft. Der unabhängige Implementierungsreview und der
anschließende ADR-0036-Load-/Hash-/Testabgleich stehen aus; ADR 0036 bleibt
vorgeschlagen und nicht annahmereif. Adapterimplementierung, Adaptertests und sichtbarer Diagnoselauf bleiben geschlossen. Browserkomposition, Browser-
...
#### Historische BrowserSyncTransport Diagnostic Capture, Timing and Projection Determinism Boundary / ADR 0032

ADR 0032 ersetzt ADR 0031 formal und übernimmt alle nicht ausdrücklich
korrigierten ADR-0030-/ADR-0031-Regeln. ADR 0033 hat ADR 0032 formal ersetzt;
ADR 0034 hat inzwischen ADR 0033 formal ersetzt und übernimmt alle nicht
ausdrücklich korrigierten ADR-0033-/ADR-0032-Regeln. ADR 0032 bleibt ebenso wie ADR 0031 mit
bytegleichem Hauptteil ab `## Kontext` als historische Ebene erhalten; ADR 0033
bleibt mit bytegleichem Hauptteil ab `## Kontext` als historische
Entscheidungsebene erhalten, und ADR 0030 bleibt durch ADR 0031 ersetzt. ADR
0020, ADR 0028, ADR 0029 und der historische Evidence-Record
...
Dieser Dokumentationsslice erstellte weder Foundation noch Adapter, Test,
Record, Controller, Harness, Fixture oder Launcher. ADR 0034 wurde später
angenommen und ist inzwischen durch ADR 0035 formal ersetzt. ADR 0035 ist
angenommen und übernimmt alle nicht ausdrücklich korrigierten Regeln aus ADR
0034, ADR 0033 und ADR 0032; ADR 0034 bleibt mit bytegleichem Hauptteil
historische Entscheidungsebene. Als nächster Slice darf nur das selbständige
importinaktive und vollständig netzwerkfreie effects-as-data-Foundationmodul
mit immutable Command-/Cleanup-Intents und untrusted materialisierten In-
Memory-Beobachtungen samt fokussierter Tests getrennt implementiert werden.
Debug-Pipe-, Byte-/Message-,
...
#### Historische BrowserSyncTransport Diagnostic Foundation Effects Protocol Boundary / ADR 0033
...
erhalten. In diesem damaligen ADR-0033-Stand sollte als nächster Slice die
getrennte netzwerkfreie Effects-as-Data-Implementierung folgen. ADR 0034 wurde
danach angenommen und ersetzte ADR 0033 formal. Inzwischen ist ADR 0035
...
fokussiert geprüft. Der unabhängige Implementierungsreview und danach der
ADR-0036-Load-/Hash-/Testabgleich stehen aus; ADR 0036 bleibt vorgeschlagen
und nicht annahmereif. Als Nächstes ist ausschließlich der getrennt zu beauftragende unabhängige
Implementierungsreview zulässig. Adapterimplementierung,
Adaptertests und sichtbarer Diagnoselauf bleiben geschlossen.

Factoryfehler sind ausschließlich synchrone statische Dependency-`TypeError`s;
dann entstehen weder API noch Promise oder `FoundationResult`. `runBinding`
wird vollständig während der Factory geprüft und in begrenzte primitive
interne Blätter kopiert. Nach erfolgreicher Factory liefert jeder kontrollierte
`run()`-Pfad ein lokales Promise und wirft nicht synchron. Der erste
`run()`-Aufruf wechselt synchron vor Arityprüfung von `unused` zu
`active`, setzt den primitiven Owner-Token `1` und ist alleiniger Owner. Bei
gültiger Arity transferiert er nach ADR 0037 beide Rollen gemeinsam und ohne
fremden Hook: `capturedExchange -> activeExchange` sowie
`capturedObservationClosed -> activeObservationClosed`. Beide Factoryslots
werden gelöscht. Bei falscher Erst-Arity werden beide Rollen ohne Effekt oder
Notification verworfen. Zweit-, Parallel- und reentrante
Nicht-Owner erhalten eigene lokal erzeugte statische Fehler-Promises; sie
lesen, verändern oder löschen keinen Capabilityslot, berühren weder
Ownerzustand noch Zähler und starten weder Effekt noch Cleanup.

Die durch ADR 0037 eng ergänzte Referenzausnahme umfasst exakt zwei private,
nicht erreichbare und löschbare Capabilityrollen. Jede besitzt höchstens
einen persistenten Slot, zeitlich disjunkt zunächst in
`capturedExchange` beziehungsweise `capturedObservationClosed`, dann in
`activeExchange` beziehungsweise `activeObservationClosed`. Der Effectport
hat exakt die eigenen aufzählbaren Data-Properties `exchange` und
`observationClosed`. Beide Funktionsreferenzen werden einmal
descriptorbasiert erfasst; zusätzlich wird der eigene nicht aufzählbare
Data-Descriptor `observationClosed.length` exakt einmal ohne Koerzierung auf
den primitiven Wert `0` geprüft. Der Container wird verworfen.

Der Owner ruft Exchange ausschließlich über die bei Modulevaluation erfasste
Apply-Intrinsic mit `undefined` als Receiver und einem frischen gewöhnlichen
geschlossenen tief eingefrorenen Intent auf. Gleiche Funktionsidentität
beider Rollen ist zulässig und kein Unabhängigkeits- oder Provenienzbeweis.
Ihr Konsum bleibt getrennt: Exchange-Portschluss löscht `activeExchange`,
erhält aber die noch für `O0` benötigte Notificationrolle. Diese wird direkt
vor ihrem argumentlosen synchronen Aufruf in einen Stackwert übernommen,
aus dem persistenten Slot gelöscht und auf `invoking` gesetzt. Nach
Rückkehr oder beherrschtem Throw gilt `consumed`; Prestart ohne `O0`
verwirft sie ohne Aufruf, jeder Terminalpfad löscht sie vor öffentlichem
Settlement. Beide Rollen werden nie ausgegeben, serialisiert, eingefroren,
persistiert, verglichen oder als Provenienz verwendet. Weder Plain-, Native-,
Realm- noch Proxyfreiheit werden behauptet. Ein gültiger ewig pending
Exchange vor `O0` darf beide benötigten Rollen und den Lauf pending halten;
ein echter Timer oder eine Livenessbehauptung bleibt unzulässig.

Vier transiente Referenzrollen sind erlaubt: aktueller
Exchange-Promise-Kandidat; aktueller Fulfillmentgraph jeder Antwortart
einschließlich Probe-, Clock-, Arm-, Cancel-, Send- und Cleanup-Step-Acks;
vollständig unreflektierter Dequeuegraph während seines Clock-Samples; und
ausschließlich synchron benötigte allowlistete Nachfahr-, Descriptor- und
Prototypreferenzen. Ein Dequeuegraph wechselt ohne Doppelhaltung in seine
Rolle; währenddessen darf die Fulfillmentrolle nur den Clock-Ack halten. Jede
andere Überlappung ist verboten. Own-Key-Arrays und Descriptorrecords sind
foundationeigene temporäre Prüfartefakte, enthaltene fremde Werte bleiben aber
Inputreferenzen. Nur die unmittelbare synchrone Notificationauswertung darf
einen Rückgabewert bis zum strikten Vergleich mit `undefined` halten;
Referenzwerte werden weder reflektiert noch assimiliert und erreichen keinen
weiteren Übergang, Promisehandler, Outputgraph oder Cleanup-Ledger.
Throwgründe werden nicht gebunden, gelesen oder inspiziert.

Der Exchange-Promise-Kandidat bleibt nach Handlerinstallation ausschließlich
bis zu seinem Settlement erhalten und wird dann verworfen. Der
Fulfillmentgraph wird nach unmittelbarer primitiver Projektion, der
Dequeuegraph nach Clock-Sample und anschließender Prüfung und jede
Nachfahr-/Descriptor-/Prototypreferenz unmittelbar nach ihrer synchronen
Prüfung verworfen. Keine Rolle wird eingefroren, mutiert, geloggt, serialisiert
oder persistiert. Der
Rejectionhandler ist formal nullstellig und liest weder Parameter,
`arguments` noch Grund; der erzeugte Folgepromise wird nie gespeichert oder
inspiziert. Abgesehen von der eng begrenzten ADR-0037-Notification bleiben
ein zweiter Port, Callback, EventEmitter, Iterator, Generator, Stream,
`AbortSignal` und versteckter Seam verboten.

Die absolute Outputinvariante gilt für Erfolgs- und Fehlerpfade:
...
Factoryoptions und `runBinding` werden descriptorbasiert geschlossen geprüft;
nur begrenzte primitive Blätter werden in frische interne Deep-Freeze-Records
kopiert. Caller dürfen keine historischen Werte, Vergleichsergebnisse, Gates,
Findings, Counts, Completionstatus, Integritätsresultate, Hashgleichheit,
Provenienzboolesche Werte, IDs oder Evaluationbytes liefern. Die 59
Replayoperanden sind typ- und längenbegrenzt; Vergleichsbasis und historische
Werte bleiben privat. Ein Callerhash kann eine syntaktische Abweichung
fail-closed machen, aber niemals Commit- oder Dateibyteprovenienz bestätigen.

Der einzige Port akzeptiert sieben geschlossene Intents:
`capability-probe`, `controller-clock-sample`, `cap-arm`, `cap-cancel`,
`protocol-command-send`, `observation-dequeue`, `cleanup-step`. Höchstens ein
Kandidat mit bestandenem
`currently-observable-local-native-promise-profile` darf ausstehen. Intents besitzen
lückenlose flüchtige IDs und sind frisch, gewöhnlich, geschlossen und tief
eingefroren. Die Capabilityprobe erlaubt nur Fortsetzung und bestätigt keine
Adapter-, Pipe-, Parser-, Timer-, Queue- oder Byteprovenienz.

Für jeden Promise-Kandidaten werden unmittelbar vor der einzigen erfassten
nativen `then`-Anwendung die leeren OwnKeys, der erwartete lokale
Promiseprototyp sowie
die unveränderten Livedescriptoren von `Promise.prototype.constructor`,
`Promise[Symbol.species]` und `Promise.prototype.then` exact-once geprüft.
Diese Anwendung ist die einzige Brandprobe. Das Profil ist ausschließlich eine
gegenwärtige Beobachtung und behauptet weder Same-Realm-/Erzeugungsrealm-,
Constructor-, Subclass-, Native- noch Proxyfreiheit; vollständig verkleidete
fremde Kandidaten können es bestehen. Beide kontrollierten Handler fangen
eigene Throws und geben nur primitives `undefined` zurück. Öffentliche
Run-Promises stammen ausschließlich aus dem erfassten lokalen Konstruktor;
Reject wird weder gespeichert noch verwendet und Resolve exakt einmal
aufgerufen. Foundationseitiges `await`, freie `.then`-Auflösung und
`Promise.resolve|race|all` bleiben verboten. Die Foundation bindet, kopiert,
serialisiert, persistiert, loggt oder emittiert keinen Rejectiongrund in ihren
kontrollierten Handlern oder Outputs. Ein bereits oder später abgelehntes
Promise, dessen Profil vor Handlerinstallation scheitert, kann jedoch einen
getrennten hostabhängigen `unhandledrejection`-/`unhandledRejection`-Kanal mit
seinem ursprünglichen Grund auslösen; Eintritt, Zeitpunkt, Häufigkeit, Inhalt,
Prozessfortsetzung und Unterdrückbarkeit werden nicht garantiert.

Die vollständige Exchange-Lease lautet:

```text
idle | observable-pending | settlement-unobservable | closed
```

Nur `idle` erlaubt einen Portaufruf. Erst der Eintritt in einen kontrollierten
Fulfillment- oder Rejectionhandler ist `observed-settlement` und gibt die Lease
vor der intent- und zustandsspezifischen Transition zu `idle` frei. Ein gültiges
dauerhaft pending Promise hält den Run pending. Synchroner `exchange`-Throw,
malformed Promise-Kandidat, Promiseprofil-Reflectionthrow oder Throw der nativen
`then`-Anwendung vor Handlerzutritt ist `settlement-unobservable` und setzt mit
höchster Präzedenz exakt `lease: closed`, `portState: closed`,
`activeCapability: null`, `affectedCapState: terminal-unknown` und
`furtherExchangeCount: zero`; `activeExchange` wird gelöscht. Pre-start endet
portlos statisch, während Observation entstehen sticky `V`, `O0` und portloser
Cleanup, nach `O0` bleibt der Snapshot bei `cleanupViolation` und
erforderlichenfalls `cleanup-terminal-failure` unverändert. Cancel, Retry und
jeder Folge-Exchange sind verboten. Ein erst nach beobachtetem Settlement
malformed Fulfillment ist davon getrennt.

Wird intern während `observable-pending` ein weiterer Exchange angefordert,
wird er vor Intent, ID, Zähler und Portaufruf unterdrückt. Das ursprüngliche
Promise bleibt allein. Bei seinem Settlement wird sein möglicher
Fulfillmentgraph ungelesen verworfen beziehungsweise kein Rejectiongrund
gelesen, und die Lease wechselt ohne nutzbares `idle`-Fenster direkt nach
`closed`. Vor Attemptstart entsteht ein statischer Fehler ohne Snapshot oder
Ledger, während Observation sticky `V` mit lokalem Cleanup und nach `O0`
ausschließlich `cleanupViolation` mit Cleanup-Terminalisierung. Bleibt das
Original pending, bleibt der Run pending; ein möglicherweise enthaltenes
Send-Ack bleibt `unknown`, niemals `multiple`.

Die erreichbaren kontrollierten Rejectionpfade sind ausschließlich
`observed-settlement` und geschlossen. Probe und
Setup-Origin-Clock enden pre-start statisch; ein nach beobachteter Rejection
oder malformed Ack bei wieder `idle` unklarer Setup-Arm erhält genau einen
Best-effort-Cancel und endet danach statisch. Ein unobservables Arm-Settlement
setzt das vollständige Closed-Tupel und endet ohne Cancel oder Exchange. Während Observation
setzen Setup-Send, Setup-/Capture-Dequeue-Clock, Setup-/Capture-Dequeue und
Capture-Arm zuerst ihren spezifischen `V`-Zustand; ein gehaltener Dequeuegraph
bleibt ungelesen. Der bekannte Setup- oder Capturecap wird danach genau einmal
vor `O0` storniert: exakter Ack führt zu `O0`, beobachteter Fehler oder
malformed Ack zu `terminal-unknown`, `O0` und initialem `cleanupViolation`,
unobservables Settlement zum vollständigen Closed-Tupel, `O0` und rein lokalem
portlosem Cleanup, pending lässt den Run ohne `O0` pending. Eine Rejection des bereits
laufenden Cancels erzeugt keinen zweiten Cancel. Im Cleanup wirken Clock-,
Arm-, Dequeue-, finaler Cancel- und Completion-Rejection nach beobachtetem
Settlement ausschließlich über Ledger und gegebenenfalls
`cleanup-terminal-failure`; Disable/Detach- und Step-Rejections markieren nur
bei offenem Port den zugehörigen Check und lassen sichere Folgeschritte zu. Ein
unobservables Settlement finalisiert portlos. Alle übrigen kontrolliert
beobachteten Rejection-Tupel sind intern unmöglich; nur sie
dürfen den allgemeinen Catch-all erreichen.

Davon getrennt setzt ein bereits am Capture-Arm unobservables Settlement das
vollständige Closed-Tupel, `closeClass: V` und den
Evaluate-Count `zero`; es gibt dann keinen Cancel und keinen weiteren
Portaufruf, sondern unmittelbar `O0` und ausschließlich lokalen Cleanup.

Die Startgrenze liegt erst nach gültiger Capability, Clock, sicherer
Setupdeadline, positivem Setupcap-Arm und eingefrorener Stage 1. Vorherige
Fehler erzeugen nur das statische Foundationfehlerresultat, niemals Projection,
Snapshot oder Cleanup. Nach `attemptStarted` und vor `O0` sind bestätigte
Port-, Ack-, Descriptor-, Envelope-, Reihenfolge- und Budgetverstöße sticky
`V`; nach `O0` bleiben Snapshot und Observation unverändert und nur der
Cleanup-Ledger darf sich ändern. Der Capturecap bleibt
`pending-activation`, bis der einzige positive Evaluate-Ack
`sent-and-capture-cap-started` Send und Capstart atomar bestätigt. Nur dieser
Ack ist Send- und Aktivierungsbeweis.

Bei durch den kontrollierten Handler beobachteter Rejection des
`Runtime.evaluate`-Exchanges liegt `observed-settlement` vor; die Lease wird vor
dem Übergang `idle`, `closeClass` wird `V` und der Cap
`activation-unknown`. Der bereits intern gebundene Phase-/Intent-/Cap-Tupel
besitzt Vorrang vor jedem allgemeinen Rejection-Catch-all. Die konservative
Projektion lautet vollständig:
...
Damit werden weder Send noch Nichtsenden, Capaktivierung, Auswertung, Factory-,
Transport- oder späterer Stimulus behauptet. Danach ist genau ein Cancel der
bekannten `armIntentId` zulässig: exakter Ack setzt `cancelled` und erhält `V`;
beobachtete Cancel-Rejection oder malformed Ack setzt `terminal-unknown` und
nach `O0` `cleanupViolation`; unobservables Settlement setzt das vollständige
Closed-Tupel und führt ausschließlich zu lokalem portlosem Cleanup ohne zweiten
Cancel oder sonstigen Exchange; ein gültiges
pending Cancel hält den Run pending. Ein zweites Evaluate und Capture-Dequeue
sind auf allen vier Pfaden verboten. Capquieszenz kommt vor dem Snapshot; der
allgemeine Catch-all bleibt nur einem unmöglichen internen Tupel vorbehalten.

Evaluate-Intent, bestätigter Send, Reply, Main-World-Auswertung, Factoryaufruf,
Transportaufruf und Networkrequest sind getrennt. Controllerabgeleitete
`factoryCallCount` und `transportCallCount` besitzen jeweils
`zero|one|unknown`; `zero` ist nur konstruktiv beweisbar, `unknown` niemals
callerlieferbar.

Jeder Dequeuewert bleibt vor seiner einen phasenpassenden Clockprobe vollständig
unreflektiert. Für jeden geschlossenen Inputknoten werden OwnKeys, Prototyp und
jeder erforderliche Descriptor exakt einmal gelesen; Arrays zusätzlich über
genau einen `length`- und einen Descriptor je erlaubtem Index. Werte stammen
nur aus gecachten Descriptoren, Rereads sind verboten. Offene CDP-Hüllen
werden nie vollständig aufgezählt; nur allowlistete Descriptoren werden
höchstens einmal gelesen. `hasOwn`, `in`, Destructuring, Spread,
`Object.assign`, freie Propertyauflösung, Iteratoren, JSON-Roundtrip und
Inputmutation bleiben verboten. Dependency-, Eingabe- oder
Fulfillmenthüllen-Reflectionthrow nach `observed-settlement` wird an Factory,
Pre-start, Post-start und Cleanup statisch beziehungsweise sticky totalisiert.
Promiseprofil-Reflectionthrow vor Handlerzutritt folgt stattdessen immer der
globalen `settlement-unobservable`-Regel ohne Cancel oder Folge-Exchange.

Für absolute Setup- und Cleanupcaps gewinnt nach dem Clock-Sample
`m_answer >= deadline` einschließlich Gleichheit vor jeder Envelope-Reflection
allein aus gebundenem Capzustand und Rohzeit; der Dequeuegraph wird ungelesen
verworfen. Unterhalb der Deadline ist ein exakt korreliertes Setup- oder
Cleanup-`cap-fired` ein vorzeitiger Controlverstoß. Capture wird ausschließlich
durch das korrelierte Capereignis seines atomar aktivierten Arms beendet.
`targetInfos` ist auf 128, IDs auf 256, CDP-URLs auf 2048,
HTTP-Methodentokens auf 16 Codeunits und alle Dequeues auf 128 begrenzt.

Das erste irreversible `connection-closed` während Capture ergibt konservativ
`U/capture-terminal-unproven`, `captureWindowState: truncated` und
`observationCloseReason: capture-terminal-unproven`. Die durch den exakten
Evaluate-Ack bereits bestätigte Captureaktivierung bleibt bestehen; der
Übergang behauptet weder ein Feuern des Caps noch einen regulären
Captureabschluss. Bei noch live gebundenem Capturecap wird genau ein Cancel vor
`O0` ausgeführt: exakter Ack erhält `U`, beobachteter Fehler promoviert zu `V`;
unobservables Settlement promoviert ebenfalls zu `V`, setzt aber das
vollständige Closed-Tupel und lässt nach `O0` nur portlosen Cleanup ohne
Folge-Exchange zu. Dauerhaft pending hält den Run pending. Connection-Close und
Capturecap werden ausschließlich in Dequeue-Reihenfolge entschieden. Dagegen
erzeugen gewöhnliches Setup-`U|V` und Capture-`V` nur bei Lease `idle` und
offenem Port zuerst den Frozen Snapshot und quieszenzieren ihren alten Cap
danach. `S && N` beendet Capture weiterhin
nicht.

Network erlaubt nur vier Events und nie Header-, Body-, `errorText`-,
Initiator- oder Redirectdaten. Timestamps werden ausschließlich innerhalb der
Networkdomäne von Sekunden in Millisekunden umgerechnet. Rücklauf,
Nichtendlichkeit oder unsicherer Überlauf nach Korrelation ist `V`. Eine
abweichende Produktnetzsignatur ist dagegen nicht automatisch
Observerinterferenz. Send- und Replycounts bleiben getrennt.

Die zehn Capzustände lauten exakt
`absent|arm-pending|armed|pending-activation|active|activation-unknown|cancel-pending|cancelled|fired|terminal-unknown`.
Arm, aktive oder unbekannte Aktivierung, genau ein Cancel, alle vier
Cancelausgänge sowie falsche, vorzeitige, späte und doppelte Cap-IDs sind
geschlossen; es gibt weder Retry noch zweiten Cancel.

Cleanup startet aus einem referenzfreien Frozen Observation-Snapshot `O0` und
den phasenspezifisch vorgeschriebenen Setup-/Capture-Cancels. Nach `O0` dürfen
`U`, `V`, `C`, `closeClass`, Stages 1 bis 8, Replay, Settlement,
Requestbudget, Counts und Observationwerte nicht verändert werden. Ein
bestätigter Clockverstoß ist vor `O0` sticky `V`; danach wirkt er ausschließlich
als `cleanupViolation` und gegebenenfalls `cleanup-terminal-failure`, niemals
rückwirkend als `V`. Cleanupableitung, Stage 9 und Checks entstehen vor
Cleanup-Origin, Deadline und Arm, damit frühe Controlfehler total bleiben.
Detach folgt jedem terminalen Disable-Sendeversuch ohne Warten auf die
Disable-CDP-Antwort; Connection-Close verbietet weitere CDP-Sends, nicht
weiterhin sichere externe Schritte.

Cleanup dequeuet ausschließlich unter den beiden privaten Purposes
`await-cleanup-protocol-responses(openCommandIds)` und
`await-cleanup-fact(checkId)`. Disable wird zuerst gesendet; Detach folgt nach
seinem terminalen Sendeversuch. Nur ein exakter Send-Ack öffnet eine Command-
ID; korrelierte Antworten dürfen danach in beliebiger Reihenfolge eintreffen,
werden aber vollständig vor den externen Fakten abgearbeitet. Eine sicher
lesbare unbekannte, doppelte oder falsch variantierte ID setzt
`cleanupViolation`, wird verworfen und niemals zufällig einem Check
zugeordnet. Unlesbares Routing oder ein unsicherer Identifier beendet die
reguläre Portverwendung mit `cleanup-terminal-failure`; bleiben Lease nach
`observed-settlement` `idle`, Port und Cleanupcap beobachtbar, ist genau ein
Recovery-Cancel die einzige weitere Portverwendung. Ein unobservables
Settlement setzt sofort das vollständige Closed-Tupel und erlaubt weder Cancel
noch sonstigen Exchange. Ein erstes Connection-Close macht offene CDP-Checks `unproven`,
verbietet weitere CDP-Sends und lässt sichere externe Steps fortsetzen; eine
Dublette setzt `cleanupViolation`.

Externe Cleanup-Schritte liefern nur boolesche Rohfakten. Für alle zwölf gilt
`false -> failed`, aber `true -> unproven`; callerlieferbare positive Fakten
authentisieren nichts. Kontrolliert beobachtete Rejection, malformed Ack/Fakt
nach Fulfillment oder falsche Korrelation folgt ausschließlich den gebundenen
Purpose- und Checkregeln. Synchroner `exchange`-Throw, malformed
Promise-Kandidat, Promiseprofil-Reflectionthrow oder native-Then-Throw setzt
dagegen das globale Closed-Tupel und beendet jeden weiteren Exchange. Nach
`O0` wirkt jeder solche Ausgang nur über Cleanup-
Ledger, Checkzustände und Candidateableitung; `cleanupViolation` und
`cleanup-terminal-failure` entstehen ausschließlich auf den dafür festgelegten
Pfaden.

Sind Checks 1 bis 19 terminal und kein Purpose offen, folgt bei Lease `idle`
und offenem Port genau ein abschließender Cleanupcap-Cancel. Nur sein exakter
Ack erlaubt genau einen
`controller-clock-sample(cleanup-completion-after-cap-cancel)`. Der Sample
bestimmt ausschließlich Timing und Receipt Order von Stage 10 und kann die
bereits entschiedene Deadline nicht rückwirkend ändern. Ein gültiger Sample
ergibt Check 20 `confirmed`, Stage 10 `observed/match` und
`all-steps-terminal`. Ein unobservables Cancel-Settlement erlaubt weder diesen
Sample noch weiteren Exchange. Für jeden nicht vollständig gültigen
Completion-Sample gelten zwingend `relativeMilliseconds: null` und
`timingState: unavailable`; kein vorheriger oder provisorischer Timingwert wird
erhalten, gerundet, projiziert, gehasht oder serialisiert. Eine nach
Handlerreihenfolge gültige `receiptOrder` bleibt davon getrennt und ist niemals
gültiges Timing. Ein ungültiger Sample, Cancelfehler oder irreversibler
Controlfehler ergibt Check 20 `failed`, Stage 10 `observed/mismatch` und
`cleanup-terminal-failure`. Gewinnt die inklusive Cleanupdeadline, werden
offene Checks `unproven`, Stage 10 `not-observed/unproven` und der Grund
`cleanup-cap`. Kein `pending` wird ausgegeben. `cleanup.result` folgt der
Präzedenz `FAIL` vor `UNPROVEN` vor `PASS`, `cleanupFinalized` entsteht erst
nach einem dieser drei Abschlüsse, und die reine Foundation kann wegen
fehlender positiver Adapterprovenienz praktisch kein Cleanup-`PASS` beweisen.

Die Foundation erfüllt nur einen tief eingefrorenen Sieben-Felder-Wrapper mit
`[[Prototype]] === null` und ohne Own-`then`, mit
`evidenceStatus: NOT_EVIDENCE`, `runtimeAuthorized: false` und
`persistenceAuthorized: false`. Seine Projection besitzt 17 Rootfelder,
`projectionType: browser-transport-diagnostic-foundation-projection`, nur
`candidateObserverGate` und `candidateFinding`, aber kein `recordType`, echtes
`observerGate` oder `finding`. `foundationSha256` bleibt `null`; der private
Evaluationdigest
`a623ffafee8dfcbc1d2ddc374cc35f0dbf800defd97619a3b58337d972090f7b`
bestätigt keine tatsächlich gesendeten Bytes. Kein Ergebnis darf persistiert,
serialisiert oder als Runtimeevidenz verwendet werden.

| Bedrohung | Kontrolle | Verbleibende Grenze |
| --- | --- | --- |
| Austausch oder erneutes Lesen des Callerports | einmalige descriptorbasierte Erfassung des exakten `{ exchange, observationClosed }`-Ports, einmalige eigene `length: 0`-Descriptorprüfung, atomarer Transfer beider Capabilityrollen, Containerverwerfung und getrennter einmaliger Konsum | gleiche Funktionsidentität beider Rollen ist zulässig; Capabilities können intern feindlich sein; keine Native-/Proxybehauptung |
| Eingabereferenz-Leak | primitive defensive Kopien, vier geschlossene transiente Rollen mit nur einer erlaubten Clock-Überlappung, keine Referenz in Outputs | Same-Realm ist keine Sandbox |
| Thenable-/Promise-Manipulation | leere OwnKeys, lokaler Prototyp, exakte Constructor-/Species-/Then-Descriptoren, eine native Brandprobe, `undefined`-Handler und Null-Prototyp-Result | ewig pending und hostabhängiger Rejectionkanal bleiben möglich |
| Gefälschte Provenienz | Callerstatus verboten; positive Hash-/Cleanupfakten bestätigen keine gebundene Quelle | Parser-, Pipe-, Byte-, Timer- und Adapterprovenienz erst im Folge-ADR |
| Cap-/Clock-Race | absolute Setup-/Cleanupdeadline einschließlich Gleichheit vor Envelope-Reflection, ungelesener Capwert, zehn Capzustände, höchstens ein Cancel je Arm und atomarer Evaluate-Ack | reale Timer- und Queue-Liveness bleibt Adapteraufgabe |
| Cleanupdeadlock oder gefälschtes positives Cleanup | Detach nach terminalem Disable-Sendeversuch, `true -> unproven`, sticky Controlverstoß und drei totale Finalisierungsgründe | gültige nie settlende Exchange-Promises bleiben Liveness-Restgrenze |
| Rohdatenabfluss | geschlossene primitive Projektion, ungelesene Header/Body/Fehlergründe, keine Logs oder Persistenz | unvertrauenswürdige Hüllen können Proxies sein |
| Vorzeitiger Evidence-Record | `NOT_EVIDENCE`, beide Autorisierungen `false`, Candidatefelder statt echten Statusachsen | realer Record erst nach Adapter-ADR und gesonderter Laufautorisierung |
...
fokussiert geprüft. Der unabhängige Implementierungsreview und danach der
ADR-0036-Load-/Hash-/Testabgleich stehen aus; ADR 0036 bleibt vorgeschlagen
und nicht annahmereif. Der aktuelle nächste Schritt ist ausschließlich der getrennt zu beauftragende
unabhängige Implementierungsreview.
...
wurde am `2026-09-04` angenommen, ersetzte ADR 0033 formal und ist am
`2026-09-05` durch ADR 0035 formal ersetzt worden. ADR 0034 bleibt mit
bytegleichem Hauptteil ab `## Kontext` historische Entscheidungsebene; alle
nicht ausdrücklich korrigierten Regeln aus ADR 0034, ADR 0033 und ADR 0032
gelten über ADR 0035 fort. Die Entscheidung implementierte weder Foundation,
Tests, Adapter noch Runtimevorgang. `schemaVersion: 1`, 17 Projection-
Rootfelder, 59 Replayvergleiche, sieben Intents, sechs Operations, 17
Integritychecks, zehn Stages, zehn Capzustände, 20 Cleanupchecks und ein Export
bleiben unverändert; ebenso ADR 0029, dessen Evidence-Record,
`overallGate: FAIL` und `causeStatus: CAUSE_NOT_PROVEN`.
I1–I8, Replay, Observer, Integrity, Stages, Hash und Cleanup sind totalisiert.

Die angenommene Sicherheitsverschärfung prüft alle geschlossenen fremden
Records einmalig in der Reihenfolge `typeof/null`, `Array.isArray === false`,
direkter erfasster lokaler `Object.prototype`, `ownKeys`, Descriptoren;
geschlossene Arrays entsprechend mit `Array.isArray === true`, direktem
erfasstem lokalen `Array.prototype` und der Dense-/Length-/Key-/
Descriptorgrammatik. Danach darf derselbe Knoten nicht erneut reflektiert
werden. Offene CDP-Hüllen und offene `targetInfos`-Einträge erhalten keine
Record-/Array-/Realm-/Plain-Data-/Proxyklassifikation; Promises
behalten ausschließlich ihr bestehendes Profil, Functions und Capabilities
werden nicht traversiert oder eingefroren.

`fresh` bezeichnet jeden an der konkreten Vertragsstelle neu konstruierten,
callerdisjunkten und mit keinem separat frisch geforderten Container geteilten
Knoten; ein frischer Baum verlangt diese Eigenschaft für jeden Container.
`deep frozen` umfasst nur foundationeigene Records und Arrays entlang
foundationeigener Own-Data-Kanten. Diese Konstruktion liefert keine Proxy-,
Realm-, Parser- oder Pipeprovenienz.

Die Prototyp-, ASCII-, Zeitzonen- und SemVer-Grammatiken sind geschlossen und
verhindern freie ASCII-/IANA-/SemVer-Interpretation:

- `printable-ascii-v1`/`A<n>`: primitiver String, `1..n` UTF-16-Codeunits,
  jede einschließlich `U+0020..U+007E`, ohne Koerzierung, Normalisierung,
  Trimmung oder Case-Faltung;
- `iana-shaped-ascii-time-zone-v1`: exakt `UTC` oder mindestens zwei
  nichtleere slashgetrennte ASCII-Komponenten bis 64 Codeunits, jede beginnend
  mit Buchstabe und danach nur Buchstabe, Ziffer, `.`, `_`, `+`, `-`; keine
  IANA-Mitgliedschafts-, Alias- oder TZDB-Behauptung, insgesamt codeunitgenau
  und case-sensitive sowie ohne `Intl`-/Host-/Netzwerklookup;
- `S32`: ausschließlich
  `^(0|[1-9][0-9]*)\.(0|[1-9][0-9]*)\.(0|[1-9][0-9]*)$` mit `1..32`
  ASCII-Codeunits, ohne numerische Konvertierung, Präfix, Suffix, Vorzeichen,
  Whitespace oder unzulässige führende Null.

I1 bis I7 leiten privat `match|mismatch|unproven` aus den exakt
codeunitgebundenen Frontend-URL-/Origin-, Transport-URL-/Komponenten-,
Loopback-/Port-/Environment-/Endpoint-, Allowed-Origin-,
Transport-Gateway-, Vite-Runtime- und Profilinstanzrelationen ab. I8
bleibt reine Provenienzgrenze; Caller-Digests dürfen weder Commit- noch
Dateibyteauthentizität erzeugen. `DIVERGED` gewinnt vor `UNPROVEN`;
`EQUIVALENT` ist nur bei allen 59 Row-Matches, I1–I7 jeweils `match` und
bestätigter Abwesenheit unerklärter Kausalabweichung möglich. Private
Invarianten dürfen weder ein sechzigstes Vergleichsfeld noch eine gefälschte
positive Provenienz erzeugen.

Für die sechs Operations entfällt die pauschale Ableitung
`kein Intent -> zero/unproven`:

| Zustand | Operationsergebnis |
| --- | --- |
| Ressource konstruktiv sicher nie aktiviert, deshalb kein Intent | `zero/match` |
| Ressource möglicherweise aktiv oder Zustand unbeweisbar, deshalb kein Intent | `zero/unproven` |
| Intent erzeugt, Sendestatus unbeweisbar | `unknown/unproven` |
| exakt ein gültiger Sende-Ack | `one/match` |
| mindestens zwei bestätigte Sende-Acks | `multiple/mismatch` |

Antwortdubletten erhöhen Sendcounts nicht; `zero` allein trägt keine
positive oder negative Sicherheitsklassifikation. Target/Session und
Network werden symmetrisch aus ihrem Sendzustand abgeleitet:

| Ressourcenlage | Sichere Ableitung |
| --- | --- |
| Attach sicher nie gesendet | `targetSessionClosed: confirmed`; kein Detach; `zero/match` |
| Attach möglicherweise oder bestätigt gesendet, keine Session-ID sicher bindbar | `targetSessionClosed: unproven`; keine erfundene ID; kein Detach; `zero/unproven` |
| Session gebunden, korrelierter exakter Detach-Erfolg | `targetSessionClosed: confirmed` |
| Session gebunden, Detach nicht sicher erfolgreich | fortgeltende ADR-0033-Regeln für `failed\|unproven\|cleanupViolation` |
| Enable sicher nie gesendet | `networkDomainClosed: confirmed`; kein Disable; `zero/match` |
| Enable möglicherweise oder bestätigt gesendet, nicht sicher deaktivierbar | `networkDomainClosed: unproven`; gegebenenfalls Disable `zero/unproven` |
| korrelierter exakter Disable-Erfolg | `networkDomainClosed: confirmed` |

Attach-Intent ohne bekannten Ack, Senderejection, malformed oder
unobservables Settlement, bestätigter Send ohne bindbare Erfolgsantwort,
fehlende, malformed, doppelte oder unkorrelierbare Antwort sowie
Connection-Close ohne Detach-Erfolg beweisen niemals eine geschlossene Session.
Bei gebundener Session und sicher nie gesendetem Enable erfolgt kein Disable;
Detach wird aber nach seiner Matrix versucht. Dieselbe
epistemische Trennung muss in späteren Pflichttests einschließlich
`zero/match` gegen `zero/unproven` einzeln nachgewiesen.

Die Integritymatrix weist für alle 17 IDs `confirmed iff`,
`violated iff`, `unproven iff` und Foundation-v1-Reichweite aus. Nur
`protocolAllowlistOnly` und `closedPrimitiveProjectionConfirmed` können die
reine Foundation positiv by construction bestätigen. Adapter-, Parser-,
Pipe-, Actual-Send-, Ressourcen-, Runtime- und Abwesenheitsbehauptungen
bleiben auf einem widerspruchsfreien Foundationpfad `unproven`; nur bereits
definierte eindeutig beobachtete Gegenpfade dürfen `violated` setzen.
`singleTargetAndSessionConfirmed` und
`singleMainWorldEvaluationConfirmed` bleiben bei bloß positiven Portdaten
`unproven`. Ein gewöhnlicher `U`-Pfad darf dadurch nicht in `FAIL`
promoviert werden. `interferenceObservation` ist bei irgendeinem relevanten
`violated` `contract-visible-detected`, nur bei allen relevanten
`confirmed` `none-contract-visible-detected`, sonst `unknown`.

Die zehn Stages sind nach eindeutigem Einzelkandidaten, fehlendem Kandidaten,
mehreren/widersprüchlichen Kandidaten und genau einem Gegenwert vollständig
als `observed/match`, `not-observed/unproven`, `ambiguous/unproven` oder
`observed/mismatch` totalisiert. Stage 1 bleibt der lokale Armed-Übergang;
Stages 2 und 8 dürfen ausschließlich akzeptierte Main-World-v2-Execution und
-Settlement verwenden; Stages 3 bis 7 nur eindeutig attribuierte
Networkereignisse mit `OPTIONS`, `204`, `POST`, `200` und der bestehenden
Finished-/Failed-Matrix. Stage 9 bleibt lokaler Cleanupstart nach Frozen `O0`,
Stage 10 die bestehende totale Finalisierung. Receipt Order kann fehlendes
ebenenlokales Timing nicht ersetzen.

Jede erfolgreiche `ok:true`-Projection bestätigt fünf rein foundationeigene
Cleanupfacts konstruktiv: Frozen `O0` vor lokal angelegtem Ledger
und Stage 9; kein erzeugter Object-Group-Intent und jeder foundationeigene
Object-Group-Zustand geschlossen; verworfene Dequeue-, Ack-, offene Hüllen-,
Own-Key-, Descriptor- und transiente Nachfahrreferenzen; keine ephemeren Target-, Session-,
Request-, Command-, Cap- oder Intentbindungen in erreichbaren Outputs. Kann
dieser lokale Abschluss nicht hergestellt werden, wird statisch ohne
`ok:true` beendet. Externe positive Cleanupfacts bleiben `unproven`.

Die unveränderte öffentliche API kann konstruktiv weder
`candidateObserverGate: PASS` noch PASS-spezifische Findings erzeugen; nur
`FAIL/observer-invalid` oder `UNPROVEN/inconclusive` dürfen öffentlich sein.
Die private reine Ableitung muss trotzdem jede FAIL-/UNPROVEN-Präzedenz,
`DIVERGED`, alle drei hypothetischen PASS-Findings und den PASS-Fallback
vollständig enthalten. Ihre spätere Testbarkeit darf ausschließlich über eine
serielle, im `finally` gelöschte Betriebssystem-Temporärkopie exakter
Produktionsquellbytes mit testlokalen Exports an genau einem lexikalischen
Anker hergestellt werden. Permanenter `__test`-Export, Environment-/
Debugseam, weitere Produktdatei, manipulierte PASS-Projection und Nutzung als
Evidenz bleiben verboten; die Original-API muss PASS-Unerreichbarkeit
zusätzlich black-box beweisen.

`Vollständig netzwerkfrei` bezeichnet präzise nur das neue Foundationmodul,
seine neuen fokussierten Tests und Effects sowie jeden Diagnoselauf dieses
Slices. Die unveränderte Repositoryregression darf ausschließlich ihre
beiden bestehenden kurzlebigen Loopback-Fixtures
`tests/localSyncGatewayHttpServer.test.js` und
`tests/n8nCloudIngressProbe.test.js` ausführen. Das erlaubt keine neue
Foundationfixture, externe Verbindung, DNS-Auflösung, manuellen Listener,
Browser-, CDP-, Vite-, Diagnose- oder zusätzlichen Childprozess.
Sicherheitsberichte müssen deshalb exakt zwischen keinem neuen, externen oder
diagnostischen Netzwerk und den zwei unveränderten etablierten
Loopback-Regressionsfixtures unterscheiden.

#### Angenommene Join- und Internal-Transition-Testbarkeitsgrenze / ADR 0035
...
ist `Angenommen – 2026-09-05`, ersetzt ADR 0034 formal und keinen weiteren
ADR. ADR 0034 bleibt mit bytegleichem Hauptteil ab `## Kontext` historische
Entscheidungsebene. Alle nicht ausdrücklich korrigierten ADR-0034-/ADR-0033-/
ADR-0032-Regeln gelten fort. Dieser Entscheidungsslice öffnet oder implementiert
weder Foundation, Testdatei, Adapter noch Runtimevorgang.

ADR 0035 schließt ausschließlich die interne Nachweisgrenze für einen zweiten
Exchange bei bereits `observable-pending` gebundener Lease. Die beschlossene
Testkopie v2 liegt als eindeutig benannte `.mjs`-Datei in einem eindeutig
aufgelösten Betriebssystem-Temporärverzeichnis außerhalb des Repositorys und
enthält exakte Produktionsbytes plus genau eine lexikalisch eindeutige
Exportdeklaration für ausschließlich:

| Binding | Arity | Sicherheitsrolle |
| --- | ---: | --- |
| `deriveCandidateObserverGate` | 1 | reine private Gate-Ableitung aus exakt dem geschlossenen tief eingefrorenen `{ hardViolation, proofIncomplete }`-Input zu `FAIL\|UNPROVEN\|PASS` |
| `deriveCandidateFinding` | 1 | reine private Finding-Ableitung aus exakt dem geschlossenen tief eingefrorenen Sechs-Felder-Candidateinput zu einem der fünf Findingwerte |
| `createBrowserSyncTransportRuntimeDiagnosticRunMachine` | 1 | erhält nach dem implementierten ADR-0037-Delta den foundationeigenen internen Record exakt `{ activeExchange, activeObservationClosed, runBinding }` nach atomarem Transfer beider Capabilityrollen und validierter tief eingefrorener Bindingprojektion; gibt exakt dieselbe private mutable Maschinenreferenz, keinen Wrapper, keine API und kein Promise zurück |
| `requestBrowserSyncTransportRuntimeDiagnosticExchange` | 2 | erhält exakt dieselbe Maschinenidentität und ihr produktiv erzeugtes frisches gewöhnliches geschlossenes tief eingefrorenes `{ phase, intentKind, payload }`-Profil; einzige produktive Grenze für alle sieben Intentarten; Rückgabe stets `undefined` |

Der Test adressiert dieselbe vom Konstruktor gelieferte Run-Machine-Instanz und
deren aktive Lease mit `machine.activeExchangeRequestProfile`; der normale
Start verwendet wie der öffentliche Pfad `machine.nextExchangeRequestProfile`.
Die private Maschine enthält produktiv Owner-Run-Promise, Zustand, Lease,
Zähler, `O0` und Ledger. Diese Werte und die beobachtbaren Effects reichen aus;
ein fünfter Inspector-, Debug- oder State-Export bleibt verboten. Konstruktor und Exchange-Grenze sind unveränderte produktive
Bindings, keine testexklusiven Produktfunktionen. Die Konformitätskopie darf
außer ihrer Exportdeklaration weder Funktionskörper, Konstanten, Inputs noch
produktive Callsites ändern, wird seriell über die aufgelöste File-URL
importiert und im `finally` vollständig entfernt. Mutationskopien sind davon
disjunkt und werden jeweils aus frischen Produktionsbytes abgeleitet. Original,
Kopien und Ergebnisse bleiben `NOT_EVIDENCE`; die Produktionsdatei behält
genau einen öffentlichen Export. Permanente Testseams, Flags, zusätzliche
Factoryoptionen, öffentliche State-Inspection, zweite Produktdatei und
manipulierte PASS-Projections bleiben ausgeschlossen.

Die zentrale Grenze prüft bei `lease === observable-pending` nur die
Maschinenidentität und stoppt vor jeder Reflection oder Verwendung des zweiten
Arguments, vor Intentkonstruktion, Intent-ID-Erhöhung, Count-/Ledgermutation,
Cap-, Snapshot- oder Cleanupmutation und jedem Capability- oder Portaufruf. Sie setzt
ausschließlich die phasengenaue Klassifikation
`pendingInternalExchangeViolation = prestart|observation|cleanup`. Die spätere
dynamische Matrix treibt die echte Maschine über normale Effects-as-Data-
Übergänge in jede der drei Phasen, hält genau einen Exchange offen, erfasst
unmittelbar davor eine Baseline und beweist für den zweiten Aufruf jeweils
`Δ Intentanzahl = 0`, `Δ nächste Intent-ID = 0`, `Δ Portaufrufe = 0`,
`Δ Sendcount = 0` und `Δ Capabilityaufrufe = 0`; genau ein Exchange bleibt
pending.

Der finite Join-Nachweis umfasst exakt 18 Fälle: drei Maschinenphasen,
`fulfillment|rejection` und die drei Zeitlagen `pre-invocation`,
`synchronous-post-invocation` sowie `observable-pending`. Die Capability gibt
ein bereits gesetteltes Promise zurück, der Test settelt ein bei Rückgabe noch
pending Promise unmittelbar danach oder hält es bis nach dem zweiten
Boundaryaufruf pending. In jedem Fall erfolgt der zweite Aufruf synchron im
selben Turn ohne Yield und vor Handlerzutritt. Promise-Settlement allein ist
kein Maschinenereignis; Lease und aktueller Exchange bleiben bis zum Handler
`observable-pending` und exakt eins.

Fulfillment und Rejection werden in allen 18 Fällen getestet, ohne Payload
oder Grund zu lesen. Danach gelten
`lease: closed`, `activeExchange: null`, `portState: closed`,
`furtherExchangeCount: zero` und für alle noch lebenden Caps
`terminal-unknown`; späte oder doppelte Handler bleiben token- und
zustandsgegatete `undefined`-No-ops. In `prestart` entsteht ein statischer
Foundationfehler ohne `O0` und Ledger. In `observation` entsteht sticky `V`,
`O0` wird eingefroren und nur lokaler portloser Cleanup ist erlaubt. In
`cleanup` bleibt `O0` unverändert, `cleanupViolation` wird `true`, offene
Portchecks werden `unproven` und der Grund ist
`cleanup-terminal-failure`.

Der von den 18 settelnden Fällen getrennte Forever-pending-Nachweis darf keine
empirische Unendlichkeit behaupten.
Seine dynamische Präfixprobe prüft in jeder Phase die unveränderten Deltas
sofort und nach exakt drei testlokalen Microtask-Checkpoints; es bleiben ein
Exchange, Run-Settlementcount 0, kein künstliches `O0`, kein vorzeitiger
Cleanup, keine Terminalmaterialisierung, keine weitere Capability und keine
neue ID. Eine zusätzliche Probe lässt das Promise bis Testende pending und
wartet den Owner-Run nicht ab. Uhr, Timer, Timeout und Produktions-
`Promise.race` bleiben ausgeschlossen. Erst zusammen mit der vollständigen
privaten Transitionstabelle ist der Fall geschlossen: Aus
`observable-pending` existiert ohne kontrollierten Fulfillment- oder
Rejectionhandler weder Fortschritts-, Snapshot-, Cleanup-, Terminalisierungs-
noch Resolvepfad, insbesondere kein Timerersatz, zweiter Port, konkurrierender
Resolver, Retry, Polling, zusätzliche Capability oder Übergang zu `idle`.

Mutationswirksam müssen später zumindest ein fehlender oder umgangener Guard,
ein Guard nach Intentbau, ID- oder Count-/Ledgermutation, zweiter Port- oder
Capabilityaufruf, Leasefreigabe zu `idle`, künstliches Owner-Settlement,
vorzeitiges `O0` oder Cleanup, Verlust der Phasenklassifikation und Mutation
des eingefrorenen Cleanup-`O0` auffallen. Diese Mutationen dürfen nur in
kontrollierten Testkopien und nie im Produktionssource stattfinden.

Die drei zusätzlich präzisierten Regressionen bleiben über den öffentlichen
Effects-as-Data-Port black-box prüfbar und rechtfertigen keine weitere private
Öffnung:

- Der vollständig geprüfte erste eindeutig attribuierte Endpoint-Request
  initialisiert `firstEndpointRequestTimestamp` und
  `lastValidBrowserNetworkTimestamp`. Jeder weitere eindeutig attribuierte
  Kandidat muss vor jeder Stage-, Count-, Timing- oder Sequenzmutation eine
  primitive numerische, endliche und nichtnegative Networkzeit besitzen; sowohl
  `timestamp * 1000`
  als auch die nichtnegative Differenz zum ersten Endpoint-Timestamp in
  Millisekunden müssen endlich und betragsmäßig höchstens
  `Number.MAX_SAFE_INTEGER` sein, und das Sample muss mindestens so groß wie
  der letzte gültige Timestamp sein;
  Gleichheit ist erlaubt und die Aktualisierung erfolgt erst nach vollständiger
  Prüfung. Timing bleibt relativ zu `firstEndpointRequestTimestamp`. Vor `O0`
  ergibt ein Rücklauf, ausdrücklich auch `10 → 12 → 11`, sticky
  `V/confirmed-violation`, `FAIL/observer-invalid` und `truncated`, ohne den
  letzten Timestamp, Stage, Count, Receipt Order, Sequenz oder Timing zu verändern; nach `O0` ist jede
  Observationmutation verboten. Pflichtfälle sind zusätzlich `10 → 9`,
  Gleichheit, `NaN`, `Infinity` und unsicherer Millisekundenüberlauf.
- Für `Network.responseReceived` werden nach Cap- und Dequeueguard exakt und
  höchstens einmal die eigenen Data-Descriptoren `method`, `sessionId`,
  `params`, `params.requestId`, `params.response` und `response.url`
  konsumiert; dann folgen Endpointvergleich und Request-ID-Korrelation der
  gecachten Werte. Die begrenzte URL-Lesung unterscheidet eine
  Produktendpoint-Response von einer fremden Response. Bei exakter URL und
  unbekannter oder ungebundener Request-ID stoppt die Reflection;
  `params.timestamp`, `response.status`, `statusText`, Header-, MIME-,
  Protocol-, Timing-, Security-, Cache-, Body- und Folgewerte bleiben
  ungelesen. Ohne unabhängige Verletzung entsteht kein `V`, aber die Attribution bleibt bis `O0`
  unbewiesen beziehungsweise mehrdeutig, die Sequenz `ambiguous`,
  `productEvidenceComplete: false` und der Candidate
  `UNPROVEN/inconclusive`. Es entsteht weder zusätzlicher Requestcount noch
  erfundene ID. Eine spätere korrelierte Response darf ihren Stage-Slot
  `observed/match` setzen, stellt die Erfolgssequenz aber nicht wieder her;
  ohne sie bleibt der Slot `not-observed/unproven/null/unavailable`.
- Exakt ein gültiger `Target.getTargets`-Sende-Ack bleibt `one/match`; eine
  Antwortdublette ist kein Intent oder Ack und ändert die Operation nie. Eine
  wohlgeformte Dublette vor Evaluate ergibt ohne künstliches `V` `U` und
  `setup-terminal-unproven`, Targetprofil `unknown`,
  `singleTargetAndSessionConfirmed: unproven`, Evaluate-Intent `zero`,
  Evaluate-Send `zero/match`, Capture `not-started`, Stage 1
  `observed/match`, Stages 2 bis 8 `not-observed/unproven` und
  `UNPROVEN/inconclusive`. Während Capture bleiben Operation `one/match` und
  Candidate bis `C` `UNPROVEN/inconclusive`. Eine eindeutig routbare malformed
  Dublette vor `O0` bleibt `V/FAIL/observer-invalid`; nach `O0` gelten nur die
  bestehenden Cleanup-Purpose-Regeln.

Schema 1, eine öffentliche Factory mit einem Export, sieben Intentarten, sechs
Protocol Commands, 59 Replayvergleiche, 17 Integritychecks, zehn Stages, zehn
Capzustände und 20 Cleanupchecks bleiben unverändert. Öffentlich bleiben nur
`FAIL/observer-invalid` und `UNPROVEN/inconclusive` erreichbar; Candidate-
...
R1–R4 korrigierte dokumentarische Adaptergrenze vorgeschlagen; seine
...
fokussierten Tests geprüft. Der unabhängige Implementierungsreview und der
anschließende ADR-0036-Load-/Hash-/Testabgleich stehen aus; ADR 0036 bleibt
vorgeschlagen und nicht annahmereif.
Adapterimplementierung, Adaptertests und
sichtbarer Diagnoselauf bleiben geschlossen.

#### Vorgeschlagene Runtime Diagnostic Adapter Boundary / ADR 0036

[ADR 0036](decisions/0036-browser-sync-transport-runtime-diagnostic-adapter-boundary.md)
ergänzt ADR 0035 und ersetzt keinen ADR. Der nach R1–R4 korrigierte Vorschlag
bleibt `Vorgeschlagen – 2026-09-06`, arbeitet K2 konstruktiv aus und hat den
begrenzten R1–R4-Dokumentreview bestanden. Die K4-Foundationentscheidung ist durch ADR 0037 angenommen und inzwischen
netzwerkfrei implementiert und fokussiert geprüft. Der getrennte unabhängige
Implementierungsreview und danach der ADR-0036-Load-/Hash-/Testabgleich stehen
aus; ADR 0036 bleibt vorgeschlagen und nicht annahmereif. Die vorgeschlagene Grenze besitzt genau einen import- und
...
Realpath, Commit-Blob, Fileidentität sowie Vorher-/Nachherzustand. Ein
Standardimport mit bloßen Randhashes gilt nicht als ABA-sicher.

Der Adapter würde allein den Sieben-Intent-Effects-Port, den exklusiven
Windows-Debug-Pipe, NUL-Framing, fatalen UTF-8-Decode, bounded lexikalische
Duplicate-Key-Prüfung, genau einen nativen Parse, eine FIFO, Write-Acks und
die drei unveränderten Caps besitzen. Dequeue liest keine Clock; der
nachgelagerte Foundation-Clockintent liest genau einmal denselben primitiven
Wert für Foundation und Ledger. Nur Setup/Cleanup verwenden `>=`, Capture nur
das korrelierte FIFO-`cap-fired`. `sent` beweist nur vollständige lokale
Frameannahme, nie Browserempfang; Cleanup-Ack beweist nur Annahme. Callback-
Hänger dürfen den bereits laufenden Cap nicht durch ein offenes
Nicht-Dequeue-Promise verdecken. Es gibt keinen Debug-TCP-Port, siebten
Command, zweiten Timer, `Promise.race`, Polling oder Vorfilterung.

Sein Nodeprozess wäre exakt an `--experimental-vm-modules` und
`--no-warnings` gebunden. Die zweite Option verhindert nur den
SourceTextModule-ExperimentalWarning-Write; ohne unabhängigen Owner von
Adapter-stdout/-stderr bleibt Ausgabenabwesenheit ausdrücklich `UNPROVEN`.

Launcher-, Handle-, Environment-, Profil-, Repository- und Cleanupfakten
entstehen nur aus einem identitätsgebundenen gemeinsamen Ledger. Der Adapter
baut `runBinding` und 59 Replayoperanden selbst und leitet alle 17 Integrity-
und 20 Cleanupwerte sowie einen exakt 17-feldrigen Finalrecord nach terminalem
Cleanup neu ab. Bestätigte Verletzung hat vor Evidenzdemotion und unabhängig
von `zero|unknown|multiple|one` Stimuli `FAIL/observer-invalid`-Präzedenz; exakt
ein Stimulus gilt nur für PASS/die drei Produktfindings. K2 trennt zwei
bytegeprüfte Vier-Export-Profile: `derivation-conformance` führt die öffentliche
Factory plus Gate/Finding/Finalizer und sperrt die Factory synchron vor dem
realen Capabilityselector; `virtual-runtime-conformance` führt dieselbe
öffentliche Factory plus produktiven Owner, vollständige Producer-Eventgrenze
und einen einmaligen
virtuellen Capabilityinstaller. Sein Profil umfasst Entropie mit getrennten
17-/15-Byte-Reads, Clock, Process-/Environment-/Runtimequellen, Scheduler,
Pipe, Launcher und geschlossene Ressourcenoperationen. Capabilitycontainer
sind gefroren, Bytes werden defensiv kopiert, Handles bleiben opaque, und
Raw-Fixtureevents erreichen Foundationwerte nur über gebundene Sinks,
produktiven Owner-/Generationguard, Parser und FIFO. Beide bleiben
`adapterEvidenceEligible:false`; ein Selector-Poisonmutant muss Realfallback
vor I/O töten. Das ADR-0035-Testkopieprofil bleibt durch K2 unverändert und
wird nicht mit den Adapterprofilen kombiniert; K2 ist dokumentarisch geprüft,
seine Testnachweise bleiben offen.

Die spätere Adaptertestsuite prüft die Deadlineintegration nur über den
Raw-Byte-/Producerpfad: Setup/Cleanup an `deadline-1`, `deadline`,
`deadline+1`, Capture ausschließlich ereignisbasiert, null Clockread im
Dequeue, ein gemeinsames Folgesample und Materialfreigabe. Der JSON-Pfad
erzeugt keine Getter-/Proxy-Envelopes. Die ADR-0037-Foundationtests schließen
die konkrete Deadline-Testlücke: Setup und Cleanup erreichen bei
`deadline` und `deadline+1` jeweils exakt null `get`-,
`getPrototypeOf`-, `ownKeys`- und `getOwnPropertyDescriptor`-Traps.
Gültige Kontrollen bei `deadline-1` belegen die tatsächliche
Envelopeprüfung. Identische Dequeue-/Clockvektoren töten pro Phase die
Mutation `>=` zu `>` und das Vorziehen der Envelopeprüfung vor den Guard.
Die grüne Foundation-Suite ersetzt keinen Adapter-Wiringnachweis.
Positive Caller-/Fake-Werte und `cleanup-fact:true` sind keine Provenienz;
Writer und Persistenz liegen hinter einer getrennten Grenze.

Der rohe Gateway-Port-Environmentwert bleibt der Vier-Codeunit-String `8787`.
Ausschließlich Replayoperand 53 erhält die separate Sechs-Codeunit-Projektion
mit umgebenden U+0022; Trimmen, Double-Wrap und Ziffernormalisierung sind
verboten.

Node Core bietet hier weder einen handlegebundenen Windows-Job-Object-Owner
für Chrome-/Vite-/Gatewaynachfahren noch eine handle-relative, nicht folgende
Deleteprimitive. Nach möglichem Spawn belegt Root-Childexit keinen
vollständigen Stop; nach möglichem Create ist pfadbasiertes Profil- oder
Fragmentlöschen verboten. Die Einzelchecks bleiben dann `UNPROVEN`, während
ein sicher gebundener aktiver Rest `FAIL` und ein konstruktiv nie versuchter
Spawn/Create `confirmed` ergibt. Damit bleibt ein sichtbarer Lauf geschlossen.

Bewusst `UNPROVEN` bleiben ohne unabhängige Quelle
`browser.engineBuild`, globale Portfreiheit, effektive Proxy-/VPN-/Policy-/
Extension-/Permission-/Service-Worker-/Cachewerte und Adapterattestierung.
...
alle Pfade auswählen; ein Promisezaun ist für Old-Cap und portlosen
Pre-Cleanup zu spät. Als minimale eindeutige Phasenbindung unter den
bestehenden Architekturgrenzen entscheidet ADR 0037 für `D_K4` genau eine
synchrone, argumentlose und einmalige Foundationnotification
`effectPort.observationClosed()` unmittelbar nach `O0` und vor jedem Cleanup.
Sie ist kein achter Intent, trägt keine Ursache und ist kein
informationstheoretischer Notwendigkeitsbeweis. Bei
unbeobachtbaren Observationfehlern liegt der Portschluss tatsächlich vor `O0`;
der Old-Cap-Scan liegt danach und `cleanup-origin` erst nach Initialisierung des
Cleanup-Ledgers. Die notwendige Änderung der Sollgrenze ist durch ADR 0037 angenommen und das
Foundationdelta netzwerkfrei implementiert und geprüft; der unabhängige
Implementierungsreview steht aus. Ein Adapter und ein authentischer
`A_obs`-Freeze sind weiterhin nicht implementiert oder nachgewiesen; ein
evidenzfähiger Record bleibt ausgeschlossen.

Dieser ADR implementiert weder Adapter noch Test, Parser, Timer, Launcher,
Writer oder Runtimekomposition und autorisiert keinen Lauf. ADR 0029 bleibt
`FAIL`, Ursache `CAUSE_NOT_PROVEN`; Browserkomposition und Browser-E2E bleiben
geschlossen.
...
ADR-0026-/ADR-0027-Runtimeanforderungen und ersetzt keinen ADR. Es entscheidet
ausschließlich den Sicherheits- und Evidenzvertrag eines später gesondert
freizugebenden Messlaufs. Dieser Dokumentationsslice selbst startete weder
Browser, Gateway noch Devserver und führte keinen Request, Port- oder
Permissionzugriff aus.

Alle positiven Pflichtbeobachtungen bleiben an das vollständige unveränderliche
Basistupel `T₀` gebunden. Die Origin- und Redirect-Negativkontrollen dürfen
ausschließlich `Δ_origin` beziehungsweise `Δ_redirect` verändern. Jede weitere
Abweichung bleibt `UNPROVEN` oder ergibt bei beobachteter Grenzverletzung
`FAIL`; nach jedem Negativvektor müssen Restore auf `T₀` und Cleanup separat
bestätigt sein. Die vier Beobachtungsebenen JavaScript, Browsernetzwerk,
Gatewayprozess und sichtbare Benutzer-/Permissionereignisse beweisen sich
nicht gegenseitig.

Der geschlossene Evidence-Record speichert nur sanitierte Bindungen,
Klassifikationen, Counts und Gatewerte. Benutzer- und Rechnername, Profilpfade,
vollständiger User-Agent, Cookies, Tokens, Credentials, HAR-/Netzwerkdumps,
Rohheader, Konsolen-/Fehlerfreitext, Gatewaylogs, Requestidentitäten, Bodies,
private Netzdetails und GoldenDawn-Inhalte bleiben verboten. Ein bekannter
Cleanupfehler ist `FAIL`, ein fehlender oder mehrdeutiger Nachweis
`UNPROVEN`.

Der danach einmalig autorisierte Lauf unter `chrome-stable-win-t0-01`
beobachtete im einzigen gestarteten Vektor `positive-default` genau einen
gewöhnlichen `OPTIONS 204`, danach einen vollständig beantworteten `POST 200`
und die erwarteten JavaScript-sichtbaren Responsewerte. Das öffentliche
BrowserSyncTransport-Promise wies dennoch statisch redigiert zurück. Dieser
belegte Ebenenwiderspruch ist kein Datenleck und keine Ursachenfeststellung,
setzt aber `normalSyntheticTransport` und das Gesamtgate auf `FAIL`. Die
Korrelation und der positive Vektor bleiben nach Schema 1 `UNPROVEN`.
`pnaLnaPermission` bleibt wegen unbekanntem Zieladressraum `UNPROVEN`; die
beiden Negativvektoren wurden stopregelkonform ohne Retry nicht ausgeführt und
bleiben einschließlich Restore `UNPROVEN`.

Jan beobachtete keinen Local-Network-/Loopback-Dialog und führte keine
Browserinteraktion aus. Daraus folgt weder allgemeine Permissionfreiheit noch
Kompatibilität eines anderen Kontexts. Alle selbst gestarteten Prozesse wurden
...
Browser-E2E bleiben geschlossen. ADR 0034 ersetzt ADR 0033 formal, übernimmt
alle nicht ausdrücklich korrigierten ADR-0033-/ADR-0032-Regeln und erhält ADR
0033 mit bytegleichem Hauptteil als historische Entscheidungsebene. Er ändert
diesen historischen Befund oder das ADR-0029-Gate nicht. ADR 0035 ist
angenommen, ersetzt ADR 0034 formal und übernimmt alle nicht ausdrücklich
...
fokussierten Tests geprüft. Der unabhängige Implementierungsreview und der
anschließende ADR-0036-Load-/Hash-/Testabgleich stehen aus; ADR 0036 bleibt
vorgeschlagen und nicht annahmereif. Adapterimplementierung, Adaptertests sowie sichtbarer Diagnose- oder Runtime-Evidence-Lauf bleiben geschlossen; nur ein späterer vollständig neuer
...
`syncTest` blieb ein weiterer Folgeslice. ADR 0034 ersetzt ADR 0033 formal und
...
ADR-0036-Load-/Hash-/Testabgleich stehen aus; ADR 0036 bleibt vorgeschlagen
und nicht annahmereif. Als Nächstes ist ausschließlich der getrennt zu beauftragende unabhängige
Implementierungsreview zulässig. Adapterimplementierung, Adaptertests und sichtbarer
...
#### Bedrohungen des durch ADR 0034 totalisierten und durch ADR 0035 korrigierten Diagnosefoundation-Zielpfads
...
Response- und Ownergrenze sind implementiert. Der BrowserSyncTransport, seine
beobachtbaren Promise-/Bufferprofile, die korrigierte Requestcap-
Nachweisgrenze sowie die feste v1-Wire-Policy sind isoliert implementiert und
...
ausgeführten Negativkontrollen `UNPROVEN`, Cleanup `PASS`. ADR 0035 ist
angenommen, ersetzt ADR 0034 formal und übernimmt alle nicht ausdrücklich
korrigierten Regeln aus ADR 0034, ADR 0033 und ADR 0032. ADR 0034 bleibt mit
...
| manipulierte Debug-Pipe, Framing-, UTF-8-, JSON- oder FIFO-Grenze | vorgeschlagene ADR-0036-Adaptergrenze | exklusiver Handle-3/4-Owner, NUL-Framing, fataler UTF-8-Decode, generischer bounded Duplicate-Key-Scanner, genau ein nativer Parse, eine totale FIFO und harte Byte-/Knoten-/Queuecaps | Same-Realm, Pipe und Materialisierung bleiben unvertrauenswürdig; Parser-/Queueverletzung darf nie als Close maskiert werden | ADR 0036 vorgeschlagen; Adapter und Tests nicht implementiert |
| testlokale Ersatzmaschine oder versehentlich reale Fähigkeit | vorgeschlagene ADR-0036-K2-Testgrenze | zwei disjunkte bytegeprüfte Vier-Export-Profile; Ableitungsselector durch synchronen Pre-Host-Factorythrow gesperrt; virtueller Selector nur in externer Testkopie; vollständige Entropie-/Clock-/Runtime-/Effects-Capabilities; derselbe produktive Owner, Raw-Sink, Producer-Eventgrenze, Parser und FIFO; `adapterEvidenceEligible:false`; Selector-Poison- und Wiringmutanten | Testkopien sind keine Runtimeevidenz; mutable Bytes und opaque Handles bleiben getrennt; virtuelle Low-Level-Werte dürfen keine fertigen Acks, Checks oder Records liefern; Deadline-Proxytrap-Lücke auf Foundationebene geschlossen, Adapterintegration weiterhin unbewiesen | K2-Korrektur ausgearbeitet, R1–R4-Dokumentreview abgeschlossen; ADR 0037 dokumentarisch angenommen und Foundationdelta netzwerkfrei implementiert und geprüft; unabhängiger Implementierungsreview ausstehend; Adapter und Adaptertests nicht implementiert oder autorisiert |
| voreiliger Send-Ack oder hängender Write-/Cleanupcallback | vorgeschlagene ADR-0036-Adaptergrenze | Ack erst nach vollständiger lokaler Frameannahme; Evaluate-Accept/Capturestart/Ack ein Commit; Cleanup-Ack nur Annahme, Fact separat; Callbacks blockieren keinen bestehenden Cap | Ack beweist weder Flush noch Browserempfang; blockierter Eventloop/Kernel ist nicht als Hostliveness beweisbar | ADR 0036 vorgeschlagen; kein Timer, Pipe oder Lauf implementiert |
| fingierter oder verspäteter `A_obs`-Freeze | vorgeschlagene ADR-0036-Adaptergrenze und angenommene `D_K4`-Abhängigkeit | als minimale eindeutige Phasenbindung unter den bestehenden Architekturgrenzen ausgewählt ist genau eine synchrone argumentlose `effectPort.observationClosed()`-Notification am zentralen Foundationcallsite unmittelbar nach `O0` und vor Cleanup; früher/fehlend/doppelt/spät bedeutet no-record | Cancelpayload und Endprojection tragen keine O0-Bindung; über Injektivität der Vollhistorie wird nichts behauptet. Der bounded syntaktische Tracker darf Semantik nicht spiegeln; Promisezaun und Settlement sind bei Old-Cap/portlos zu spät | ADR-0037-Notification in der Foundation implementiert und mit sieben Ablaufklassen sowie 27 Notificationmutanten geprüft; unabhängiger Implementierungsreview ausstehend; kein Adapter- oder `A_obs`-Freeze-Nachweis; ADR 0036 weiterhin nicht annahmereif, Adaptertests/-implementierung gesperrt |
| fingierte Provenienz aus Callerwert, frischem Profil, Childexit oder positivem Cleanupfact | vorgeschlagene ADR-0036-Adaptergrenze | byte-owned Foundationload, selbst gebauter 59-Operanden-`runBinding`, identitätsgebundene 17/20-Ableitung und frischer Record erst nach terminalem Cleanup | Enginebuild, globale Portfreiheit, effektive Profil-/Netzstores und unabhängige Adapterattestierung bleiben ohne Quelle `UNPROVEN` | ADR 0036 vorgeschlagen; kein Record oder Writer autorisiert |
...
Upstream. Der lokale SyncAgent-Kern ist kontrolliert mit diesem Gateway
komponiert. Der BrowserSyncTransport und seine feste v1-Wire-Policy sind
isoliert implementiert und netzwerkfrei mutationswirksam geprüft. ADR 0029
...
dokumentiert; seine Ursache bleibt `CAUSE_NOT_PROVEN`. ADR 0034 ersetzt ADR
0033 formal und übernimmt alle nicht ausdrücklich korrigierten ADR-0033-/
ADR-0032-Regeln; ADR 0033 bleibt mit bytegleichem Hauptteil als historische
Entscheidungsebene erhalten. ADR 0035 ist angenommen, ersetzt ADR 0034 formal
...
implementiert und mit 595/595 fokussierten Tests geprüft. Der aktuelle nächste Schritt ist ausschließlich der getrennt zu beauftragende
unabhängige Implementierungsreview des implementierten ADR-0037-Deltas.
Danach folgen Jans manueller Featurecommit und der gesonderte
ADR-0036-Load-/Hash-/Testabgleich mit erneutem unabhängigen Review. ADR 0036
bleibt vorgeschlagen und nicht annahmereif; Adapter und Adaptertests sind
nicht implementiert oder autorisiert. `src/main.js`-, UI- und
...
synthetischen `syncTest` operativ erreichbar. Der BrowserSyncTransport samt
ADR-0028-v1-Wire-Policy ist isoliert implementiert und regressionsgeprüft. Das
davon getrennte ADR-0029-Runtime-Evidence-Gate ist für den einmaligen
Chrome-151-Lauf insgesamt `FAIL`; PNA/LNA und die nicht ausgeführten
Negativkontrollen bleiben `UNPROVEN`.
Browserkomposition und End-to-End-Pfad fehlen weiterhin. Der SyncAgent bildet
...
- [ADR 0036: BrowserSyncTransport Runtime Diagnostic Adapter Boundary](decisions/0036-browser-sync-transport-runtime-diagnostic-adapter-boundary.md) – vorgeschlagen, `2026-09-06`
- [ADR 0035: BrowserSyncTransport Diagnostic Foundation Join and Internal Transition Testability Boundary](decisions/0035-browser-sync-transport-diagnostic-foundation-join-and-internal-transition-testability-boundary.md) – angenommen, `2026-09-05`
- [ADR 0034: BrowserSyncTransport Diagnostic Foundation Grammar, Derivation and Testability Boundary](decisions/0034-browser-sync-transport-diagnostic-foundation-grammar-derivation-and-testability-boundary.md) – ersetzt durch ADR 0035; ursprünglich angenommen, `2026-09-04`
- [ADR 0033: BrowserSyncTransport Diagnostic Foundation Effects Protocol Boundary](decisions/0033-browser-sync-transport-diagnostic-foundation-effects-protocol-boundary.md)
