...
`CAUSE_NOT_PROVEN`. ADR 0035 ersetzt ADR 0034 formal und übernimmt alle nicht
...
Adaptergrenze vorgeschlagen und noch nicht angenommen. Die K2-Konstruktion ist
...
Der nächste zulässige Schritt ist ausschließlich der getrennte unabhängige
Implementierungsreview; Adapterimplementierung,
...
End-Fluss bleiben gesperrte spätere Slices. ADR 0032 ändert weder diese
Vertragskette noch das ADR-0029-Runtimegate.
...
[ADR 0032](decisions/0032-browser-sync-transport-diagnostic-determinism-boundary.md)
ersetzt
[ADR 0031](decisions/0031-browser-sync-transport-diagnostic-envelope-and-observation-completion-boundary.md)
formal und übernimmt sämtliche nicht ausdrücklich korrigierten ADR-0030-/
ADR-0031-Regeln. ADR 0033 hat ADR 0032 inzwischen formal ersetzt und übernimmt
alle nicht ausdrücklich korrigierten ADR-0032-Regeln. ADR 0032 bleibt ebenso
wie ADR 0031 mit bytegleichem Hauptteil ab `## Kontext` als historische Ebene
erhalten; ADR 0030 bleibt durch ADR 0031 ersetzt. ADR 0020, ADR 0028,
ADR 0029 und der historische Evidence-Record bleiben unverändert. Die Diagnose
ist weder vierter ADR-0029-Vektor noch zusätzliches Runtimegate oder Erweiterung
des `BrowserRuntimeEvidenceRecord`.

Der historische Chrome-Befund bleibt vor und nach jeder Diagnose exakt
`overallGate: FAIL`; `causeStatus` bleibt ausnahmslos `CAUSE_NOT_PROVEN`. ADR
0032 ist weder Ursachennachweis noch Runtime-`PASS`, Produktfreigabe,
Foundation- oder Adapterimplementierung oder Laufautorisierung. Schema 1 darf
ohne Versionswechsel nur deshalb exakt fortgeführt werden, weil weder eine
Implementierung noch eine konforme persistierte Instanz existiert. Unverändert
bleiben `recordType`, 17
Rootfelder, sechs CDP-Operationen, 17 Integritätschecks, neun Requestzähler plus
Sequenz, zehn Stages, drei Clock-Domänen, exakt 20 Cleanup-IDs und fünf
Findings.

Der Diagnoseversuch beginnt mit einem getrennten globalen Setupfenster:
...
Vor dem einzigen Sendeversuch gilt architektonisch diese Reihenfolge:
...
Der konkrete Timer ist damit vor dem Send aktiv. Kann die Cap-/Clock-Fähigkeit
nicht sicher hergestellt werden, entsteht `V` oder kein gültig gestarteter
Diagnoseversuch. Die pure Foundation erzeugt ausschließlich den
nichtblockierenden Command-Intent; reale Clock- und Timerfähigkeiten gehören
zum späteren Adapter. Der Setup führt `Target.getTargets`,
...
Jede antwortartig eingereihte Nachricht erhält beim Dequeue vor jeder
Reflection genau eine controller-monotone Abtastung `m_answer`; daraus wird roh
`d := m_answer - m_setup` gebildet. Nur `d < 6000` erlaubt die synchrone,
begrenzte Prüfung gegen die geschlossene Setupantwort-Grammatik aus ADR 0032
und dem Living Contract. Bei `d >= 6000` gewinnt der Setupcap und der
Nachrichteninhalt bleibt ungelesen. Eine ungültige, negative, rückläufige oder
werfende Clock und jede zweite Abtastung sind bestätigte Ablaufverletzungen
`V`. Erst drei eindeutig korrelierte, erfolgreiche und vollständig validierte
Antworten ergeben `setupReady`. Bei `Target.getTargets` werden zuerst alle
`page`-Einträge mit exakt gebundener URL unabhängig von `attached` gezählt;
nur bei genau einem Kandidaten wird `attached === false` geprüft und erst
danach dessen `targetId` gebunden. Persistierte 10-ms-Rundung beeinflusst die
rohe Capentscheidung nicht; späte Antworten werden verworfen.
...
vorher terminal zu `U` führen.
Ohne unabhängigen bestätigten Obserververstoß ergibt das ausschließlich
`observerGate: UNPROVEN`, `finding: inconclusive` und
`captureWindowState: not-started`. Evaluation, Factory und Transportstimulus
bleiben `zero`, bewusst gegatete Downstream-Kommandos `zero/unproven`, nicht
zuverlässig beobachtete Networkcounts `unknown` und die Sequenz `incomplete`.
Die bereits vor dem ersten Send gesetzte Stage 1 `observer-armed` bleibt auch
nach einem späteren `U` `observed/match`; ausschließlich die gegateten Stages 2
bis 8 werden `not-observed/unproven` mit `null` für Timing und `receiptOrder`.
Cleanup beginnt dennoch aus dem tatsächlich erreichten partiellen Setupzustand.
...
6.000-ms-Capturefenster. Für dieses Fenster gilt:

```text
S := genau ein gültiger Settlementkandidat
N := genau ein eindeutig dem POST zugeordnetes loadingFinished oder loadingFailed
P := S && N

productEvidenceComplete := P
observationClosed := V || C
```

`S`, `N` und `P` bezeichnen ausschließlich vorläufige Produkt-Evidence-
Completeness. Ohne `V` bleibt der Observer bis zum verarbeiteten `C` aktiv,
damit doppelte Evaluate-Antworten, weitere `OPTIONS`-/`POST`-Requests und andere
budgetrelevante Ereignisse im vollständigen Fenster erkennbar bleiben. Ein
sticky bestätigtes `V` friert sofort fail-closed ein und entwaffnet den
Capturetimer; positive Counts bleiben erhalten, nicht mehr abschließend
beweisbare Abwesenheitswerte werden `unknown`. Bei Cap-Gleichzeitigkeit zählt
allein die controllerlokale Verarbeitungsreihenfolge. Das Requestbudget trifft
nur Aussagen vom Zustand `observer-armed` bis zum verarbeiteten `C` oder frühen
`V`, nie über globale Abwesenheit danach. Der Record beschreibt genau einen
Diagnoseversuch und höchstens einen Stimulus; `PASS` und jeder nicht-
inkonklusive Befund verlangen exakt einen Stimulus.

Bei `U`, `V` oder `C` wird zuerst ein frischer, tief eingefrorener
Pre-Cleanup-Observation-Snapshot erzeugt. Er enthält ausschließlich Replay-
und Hashbindungen, Observer-, Setup- und Evaluationzustand, die ersten vier
Protokolloperationen, Requestbudget und Sequenz, Settlement, Stages 1 bis 8,
Timing und Completion sowie den sticky `FAIL`-Latch. Erst danach entsteht ein
getrenntes Cleanup-Ledger mit `Network.disable`,
`Target.detachFromTarget`, Stages 9 und 10, exakt 20 Cleanupchecks,
Cleanupfinalisierung und -resultat. Der Finalrecord wird frisch aus beiden
unveränderten Projektionen zusammengesetzt; die sechs Protokolloperationen
behalten ihre Reihenfolge.

Der Cleanup-Finalisierungscap beträgt getrennt exakt `60.000 ms`, beginnt
controller-monoton bei `cleanup-started` und ist kein allgemeiner Durationcap.
Offene Checks werden beim Cap `unproven`, ein bestätigtes `failed` behält
`FAIL`-Präzedenz. Der Record darf nach vollständiger Terminalität oder
verarbeitetem Cleanupcap materialisiert werden. Die geschlossene Completion-
Ableitung umfasst mindestens die Abschlussgründe `setup-cap`,
`setup-terminal-unproven`, `capture-cap` und `confirmed-violation`; freie
Callerwerte für Completion, Sequenz, Gates, Findings oder Cleanupstatus sind
verboten.

Ebenenlokale Dauern werden ohne Koerzierung auf `null` oder genau
`0, 10, …, 59990, 60000` projiziert. Ungültige oder negative Werte sind
`unavailable`, Werte unter 60.000 ms werden mit `10 * floor(d / 10)` gerundet,
und Werte ab 60.000 ms sind `at-or-above-cap`. Controller-, Setup-, Capture-,
Main-World-, Network- und Cleanupnullpunkte sind getrennt gebunden; Clock-
Domänen werden nie miteinander verrechnet. `deadline-compatible` gilt nur für
eine gültige gerundete Main-World-Dauer von einschließlich 4.500 bis 5.500 ms.
Stages besitzen eine feste Layer-/Clock-Matrix und eine je Layer lückenlose
`receiptOrder` ab 1.

Die Foundation-Hashdomäne ist SHA-256 über die exakten rohen Inhaltsbytes des
tatsächlich geladenen späteren Pfads
`scripts/browser/browserSyncTransportRuntimeDiagnosticObserver.js`, ohne BOM-,
EOL-, Unicode- oder sonstige Normalisierung. Die Checkoutbytes müssen vor
einem späteren Lauf bytegleich zum Git-Blobinhalt desselben Pfads unter
`replay.repositoryCommit` sein. `evaluationSha256` bindet UTF-8 ohne BOM über
exakt den tatsächlich gesendeten primitiven
`Runtime.evaluate.params.expression`-String; JSON- oder CDP-Envelopebytes
gehören nicht zur Domäne.

Die Main-World-Evaluation prüft URL, Origin, Top-Level- und Secure-Context-
Bindung vor Import, Factory und Transportaufruf und gibt ausschließlich einen
geschlossenen verschachtelten By-Value-Baum aus `preTransportContext`,
`execution` und optionalem `settlement` zurück. Nur viermal `match` darf exakt
einen Factory- und Transportaufruf auslösen. Alle konsumierten Felder
unvertrauenswürdiger materialisierter CDP-Werte werden über bei
Modulevaluation erfasste Intrinsics descriptorbasiert als eigene Data-
Properties geprüft. Beobachtbare Accessor-, Symbol-, Zusatzfeld-, Prototyp-
oder Hüllenverletzungen werden fail-closed behandelt; verbotene Felder werden
nur auf Own-Presence geprüft, ohne ihre Werte zu lesen. Diese Beobachtung
...
bestätigt werden; Hüllen und flüchtige Routingwerte werden danach verworfen.
...
Die totale Replayrelation verwendet
`relationId: adr-0032-causal-replay-v2`. `DIVERGED` folgt aus mindestens einem
Mismatch oder einem widersprochenen Kausalabweichungscheck, `UNPROVEN` aus jedem verbleibenden
unbewiesenen Wert und `EQUIVALENT` nur aus ausschließlich beobachteten Matches
plus bestätigter Abwesenheit unerklärter Abweichungen. Die Vergleichsmenge ist
exakt:

```text
8 Artefaktvergleiche
1 repository.state
50 kausale Kontextvergleiche
= 59 Vergleiche
```

Neu gebunden sind der achte Artefaktvergleich über das geschlossene
Frontend-Runtime-Source-Set-Manifest und der fünfzigste Kontextvergleich
`toolchain.vite.lockfileVersion`; Lockfileversion und tatsächlich verwendete
Vite-Version bleiben getrennte Nachweise.

ADR 0035 ist inzwischen angenommen, ersetzt ADR 0034 formal und übernimmt alle
nicht ausdrücklich korrigierten Regeln aus ADR 0034, ADR 0033 und ADR 0032.
ADR 0034 bleibt mit bytegleichem Hauptteil als historische Entscheidungsebene
erhalten. Als nächster Slice darf nur ein selbständiges, importinaktives und
vollständig netzwerkfreies effects-as-data-Modul am festgelegten
Foundationpfad samt fokussierter Tests getrennt implementiert werden. Es besitzt
in der unveränderten Produktionsquelle nur den Export
...
Cleanup-Intents sowie unvertrauenswürdige materialisierte In-Memory-
Beobachtungen. Debug-Pipe-Framing, Byte-/Message-, UTF-8-, JSON- und Duplicate-
Key-Parser, Queue- und Ressourcenlimits, Cap-/Timerfähigkeiten, Hashbindung,
Prozesslauncher, Browser-, Vite-, Gateway-, Dateisystem- und
Recordwriteradapter sind nicht Teil dieser Foundation. Adapter benötigen
danach weiterhin einen eigenen ADR und eine getrennte netzwerkfreie
Implementierung; erst nach beiden Implementierungsslices darf ein sichtbarer Diagnoselauf
gesondert autorisiert werden. Ein möglicher
Produkt-ADR, eine Produktimplementierung, ein vollständig neuer ADR-0029-Lauf
und Browserkomposition sowie Browser-End-to-End bleiben weitere getrennte
Schritte.
...
Hauptteil und ADR 0032 als historische Entscheidungsebenen erhalten. ADR 0035
...
Adaptergrenze vorgeschlagen; die K2-Konstruktion ist im begrenzten
...
vorgeschlagen und nicht annahmereif. Adapterimplementierung,
...
dann entstehen weder API noch Promise oder `FoundationResult`. Eine
erfolgreiche Factory liefert eine frische tief eingefrorene `{ run }`-API;
jeder kontrollierte `run()`-Pfad liefert ein lokal erzeugtes Promise und wirft
nicht synchron. Der erste Run wird vor seiner Arityprüfung alleiniger Owner.
Bei gültiger Arity transferiert er gemäß ADR 0037 beide Rollen gemeinsam:
`capturedExchange -> activeExchange` und
`capturedObservationClosed -> activeObservationClosed`. Beide Factoryslots
werden gelöscht; falsche Erst-Arity verwirft beide Rollen und terminalisiert
ohne Effekt oder Notification.
Nicht-Owner-Aufrufe erhalten je ein isoliertes statisches Fehler-Promise und
berühren weder Capabilityslot, Ownerzustand, Zähler, Effekte noch Cleanup. Die
Architektur besitzt einen Effectport mit genau zwei getrennten Rollen:

```text
effectPort.exchange -> capturedExchange -> activeExchange
    -> capability -> clocks/caps -> CDP sends/dequeues -> O0

effectPort.observationClosed -> capturedObservationClosed
    -> activeObservationClosed -> O0 -> Slot löschen -> Notification

erst danach: Old-Cap-Scan -> Cleanup-Ledger -> Cleanup
    -> FoundationProjection in NOT_EVIDENCE-FoundationResult
```

Die Notification ist kein Intent, Promise, Datenkanal oder zweiter Port.

Das allgemeine Referenzverbot erlaubt nach ADR 0037 exakt die beiden
Capabilityrollen, pro Rolle höchstens einen persistenten Slot und jeweils
zeitlich disjunkt vor und im Ownerlauf. Der Port besitzt exakt die beiden
eigenen aufzählbaren Data-Properties `exchange` und `observationClosed`.
Ihre Funktionsreferenzen werden einmal descriptorbasiert erfasst; der
Container wird verworfen. Zusätzlich wird der eigene nicht aufzählbare
Data-Descriptor `observationClosed.length` exakt einmal ohne Koerzierung auf
`0` geprüft. Gleiche Funktionsidentität beider Rollen ist zulässig; der
Notificationkonsum lässt eine noch benötigte Exchangerolle unverändert.

Exchangeaufrufe erfolgen nur über die erfasste Apply-Intrinsic mit
`undefined` und einem Frozen Intent. Exchange-Portschluss löscht
`activeExchange`, erhält aber eine noch für `O0` benötigte
`activeObservationClosed`-Rolle. Direkt vor der Notification wird diese in
einen synchronen Stackwert übernommen, ihr Slot gelöscht und ihr Zustand auf
`invoking` gesetzt; der Aufruf hat Receiver `undefined` und keine Argumente.
Nach Rückkehr oder beherrschtem Throw gilt `consumed`. Prestart ohne `O0`
verwirft die Rolle ohne Aufruf, jeder Terminalpfad löscht sie vor
Resulterfüllung. Beide Funktionsidentitäten werden nie ausgegeben,
serialisiert, eingefroren, persistiert, verglichen oder als Provenienz
verwendet.

Vier transiente Rollen sind architektonisch zulässig: aktueller
...
benötigte allowlistete Nachfahr-, Descriptor- und Prototypreferenzen. Nur die
festgelegte Clock-Überlappung ist erlaubt. Nullstellige Rejectionhandler lesen
keinen Grund; der erzeugte Folgepromise wird nicht gespeichert. Keine
Inputreferenz erreicht Result, Projection, Pre-Cleanup-Snapshot oder
Cleanup-Ledger. Ausschließlich zur unmittelbaren Notificationauswertung ist
ein synchroner Rückgabewert bis zum strikten Vergleich mit `undefined`
zulässig; fremde Referenzen werden weder reflektiert noch assimiliert und
erreichen keinen weiteren Übergang. Throwgründe werden nicht gebunden.
Abgesehen von der eng begrenzten ADR-0037-Notification bleiben Callback,
EventEmitter, Iterator, Stream, `AbortSignal` und zweiter Port verboten.

Die effects-as-data-Allowlist ist exakt `capability-probe`,
`controller-clock-sample`, `cap-arm`, `cap-cancel`,
`protocol-command-send`, `observation-dequeue`, `cleanup-step`. Intents sind
gewöhnliche geschlossene Deep-Freeze-Records `{ intentId, kind, payload }`;
IDs beginnen bei `1`, sind lückenlos und flüchtig. Akzeptiert wird
ausschließlich das
`currently-observable-local-native-promise-profile`: leere OwnKeys, der
erwartete lokale Promiseprototyp und unveränderte Constructor-, Species- und
Then-Livedescriptoren werden exact-once unmittelbar vor genau einer Anwendung
des erfassten nativen `then` geprüft. Das ist nur eine gegenwärtige
Beobachtungsform und behauptet weder Same-Realm-/Erzeugungsrealm-, Constructor-,
Subclass-, Native- noch Proxyfreiheit; ein vollständig verkleideter fremder
Kandidat kann sie bestehen. Beide kontrollierten Handler geben nur primitives
`undefined` zurück. Der nullstellige Rejectionhandler liest weder Parameter,
`arguments` noch Grund. Öffentliche Run-Promises stammen aus dem erfassten
lokalen Konstruktor, Reject bleibt ungenutzt und Resolve wird exakt einmal
verwendet.

Die Foundation bindet, kopiert, serialisiert, persistiert, loggt oder emittiert
keinen fremden Rejectiongrund über kontrollierte Handler oder Outputs. Diese
statische Redaction reicht nicht in einen getrennten hostabhängigen
`unhandledrejection`-/`unhandledRejection`-Kanal: Ein bereits oder später
abgelehntes Promise, dessen Profil vor Handlerinstallation scheitert, kann dort
seinen ursprünglichen Grund offenlegen; Eintritt, Zeitpunkt, Häufigkeit,
Inhalt, Prozessfortsetzung und Unterdrückbarkeit werden nicht garantiert.

Die Exchange-Lease ist vollständig geschlossen:
...
dauerhaft pending Promise hält den gesamten Run pending. Synchroner
`exchange`-Throw, malformed Promise-Kandidat, Promiseprofil-Reflectionthrow oder
Throw der nativen `then`-Anwendung vor Handlerzutritt ist
`settlement-unobservable` und setzt mit höchster Präzedenz exakt `lease:
closed`, `portState: closed`, `activeCapability: null`, `affectedCapState:
terminal-unknown` und `furtherExchangeCount: zero`; `activeExchange` wird
gelöscht. Pre-start endet portlos statisch, während Observation entstehen
sticky `V`, `O0` und portloser Cleanup, nach `O0` bleibt der Snapshot bei
`cleanupViolation` und nötigenfalls `cleanup-terminal-failure` unverändert.
Cancel, Retry und jeder Folge-Exchange sind ausgeschlossen. Ein erst im
beobachteten Fulfillment festgestelltes malformed Ack ist davon getrennt und
wird intent-spezifisch behandelt.

Fordert die interne Maschine während `observable-pending` einen weiteren
Exchange an, unterdrückt sie ihn vor Intentmaterialisierung, ID, Zähler und
Portaufruf. Das ursprüngliche Promise bleibt das einzige. Bei dessen Settlement
wird der Fulfillmentgraph ungelesen verworfen beziehungsweise kein
Rejectiongrund gelesen; die Lease wechselt atomar nach `closed`, niemals über
ein nutzbares `idle`. Vor Attemptstart entsteht ein statischer Pre-start-Fehler
ohne Snapshot oder Ledger, vor `O0` sticky `V` mit ausschließlich lokalem
Cleanup und nach `O0` allein `cleanupViolation` mit Cleanup-
Terminalisierung. Bleibt das Original pending, bleibt der Run pending. Ein
unterdrückter Exchange erzeugt niemals Intent, ID oder Count; ein im
verworfenem Ack möglicher Send bleibt `unknown` und wird nie als `multiple`
gezählt.

Die kontrollierten Rejectionpfade sind ausschließlich `observed-settlement` und
geschlossen. Probe und
...
Die Startgrenze liegt erst nach Capabilityprobe, gültigem `m_setup`, sicherer
absoluter 6.000-ms-Deadline, bestätigtem Setupcap und eingefrorener Stage 1.
Fehler davor ergeben keine Projection, keinen Snapshot und kein Cleanup.
Exact-once-Reflection gilt für Factoryoptionen, Binding, alle geschlossenen
Inputs und Arrayelemente; offene CDP-Hüllen werden nie aufgezählt, sondern nur
über allowlistete Descriptoren konsumiert. Nach `attemptStarted` und vor `O0`
sind bestätigte Protokollverletzungen sticky `V`; nach `O0` bleibt der Snapshot
unverändert und ausschließlich die Cleanupableitung darf sich ändern. Drei
Setupkommandos laufen seriell unter
einem globalen Cap. Für Setup und Cleanup gewinnt
`m_answer >= deadline` einschließlich Gleichheit vor jeder Envelope-Reflection
und verwirft den Wert ungelesen. Unterhalb der Deadline ist ein korreliertes
Setup-/Cleanup-Capereignis vorzeitig. Setup wird vor Capturearm und Evaluate
terminal storniert. Capture startet ausschließlich durch den exakten Ack
`sent-and-capture-cap-started`, der den bestätigten Evaluate-Send und die
Captureaktivierung atomar bindet. Evaluate-Intent, bestätigter Send,
Evaluate-Reply, Main-World-Auswertung, Factoryaufruf, Transportaufruf und
Networkrequest sind getrennte Countdomänen; Intent ist kein Sendbeweis, POST
kein Transportaufrufbeweis und Reply kein Beweis eines weiteren Sends.
Controllerabgeleitete Factory- und Transportcounts lauten jeweils
`zero|one|unknown`; `zero` ist nur auf konstruktiv geschlossenem Pfad zulässig,
und `unknown` ist niemals callerlieferbar.

Eine durch den kontrollierten Handler beobachtete Rejection des
`Runtime.evaluate`-Exchange-Promises ist `observed-settlement` und besitzt
Vorrang vor jedem allgemeinen Catch-all. Nach Freigabe der Lease dispatcht der
kontrollierte Handler zuerst über das intern gebundene Tupel aus Phase,
Intentart/-ID sowie Capart/-zustand. Für diesen Übergang gilt exakt:
...
Damit werden weder Send noch Nichtsenden, tatsächliche Capaktivierung,
Evaluation, Factory-, Transport- oder späterer Stimulus behauptet. Danach ist
nur genau ein `cap-cancel` der bekannten `armIntentId` zulässig: exakter Ack
setzt `cancelled` und behält `V`; beobachtete Rejection oder malformed Ack setzt
`terminal-unknown` und initialisiert nach `O0` `cleanupViolation`;
unobservables Settlement setzt das vollständige Closed-Tupel und führt
ausschließlich zur lokalen portlosen Cleanup-Terminalisierung ohne zweiten
Cancel oder sonstigen Exchange; ein
gültiges pending Cancel hält den Run unbegrenzt pending. Kein Pfad sendet ein
zweites Evaluate oder dequeuet Capture. Erst nach dieser Capquieszenz darf der
Snapshot entstehen. Der allgemeine Catch-all ist nur bei einem unmöglichen
internen Tupel zulässig und darf keinen spezifischen Arm-, Send-, Cancel- oder
Cleanupübergang überspringen.

`S && N` beendet Capture weiterhin nicht. Das erste irreversible
`connection-closed` während Capture erzeugt konservativ
...
Captureabschluss. Bei noch live gebundenem Capturecap erfolgt dessen genau ein
Cancel ausnahmsweise vor `O0`: exakter Ack erhält `U`, beobachteter Fehler
promoviert zu `V`; unobservables Settlement promoviert ebenfalls zu `V`, setzt
aber das vollständige Closed-Tupel und lässt nach `O0` nur portlosen Cleanup
ohne weiteren Exchange zu. Pending hält den Run pending.
Connection-Close und Capturecap werden ausschließlich in Dequeue-Reihenfolge
entschieden. Im gewöhnlichen Setup-`U|V` und Capture-`V` wird bei Lease `idle`
und offenem Port dagegen zuerst `O0` eingefroren und erst danach der alte Cap
storniert. Diese verschiedenen
Quieszenzreihenfolgen sind nicht vereinheitlichbar.

Die zehn privaten Capzustände lauten exakt `absent|arm-pending|armed|pending-activation|active|activation-unknown|cancel-pending|cancelled|fired|terminal-unknown`.
Arm, Aktivierung, genau ein Cancel, ein erfolgreiches, beobachtet fehlerhaftes,
unobservables oder dauerhaft pending Cancel sowie falsche, vorzeitige, späte
und doppelte Cap-IDs sind total; es gibt weder Retry noch zweiten Cancel.

`O0` ist ein frischer, referenzfreier und tief eingefrorener Observation-
Snapshot. Nach ihm dürfen `U`, `V`, `C`, `closeClass`, Stages 1 bis 8, Replay,
Settlement, Requestbudget, Counts oder andere Observationwerte nie verändert
werden. Ein bestätigter Clockverstoß vor `O0` wird `V`; nach `O0` wirkt er
ausschließlich als `cleanupViolation` und gegebenenfalls
`cleanup-terminal-failure`, niemals rückwirkend als `V`.

Cleanup verwendet genau die privaten Dequeue-Purposes
`await-cleanup-protocol-responses(openCommandIds)` und
`await-cleanup-fact(checkId)`. Disable wird zuerst gesendet; Detach folgt jedem
terminalen Disable-Sendeversuch und wartet nicht auf dessen Antwort. Erst nach
exactem Send-Ack wird eine Command-ID geöffnet; korrelierte Antworten dürfen
danach in beliebiger Reihenfolge terminalisiert werden. Alle offenen
Protokollantworten werden vor externen Fakten geleert. Unbekannte, doppelte,
falsch variantierte oder sicher lesbare fremde IDs setzen
`cleanupViolation`, werden aber nie zufällig einem Check zugeordnet;
unlesbares Routing oder ein unsicherer Identifier beendet die reguläre
Portverwendung mit terminalem Cleanupfehler. Bleiben Lease nach
...
Settlement setzt dagegen sofort das vollständige Closed-Tupel und erlaubt
weder Cancel noch sonstigen Exchange. Ein
erstes Connection-Close macht offene CDP-Checks `unproven`, verhindert weitere
CDP-Sends und lässt sichere externe Steps fortsetzen; eine Dublette ist ein
Cleanupverstoß.

Für alle zwölf externen Rohfakten gilt `false -> failed`, `true -> unproven`.
Nach terminalen Checks 1 bis 19 und ohne offenen Purpose folgt bei Lease `idle`
und offenem Port genau ein abschließender Cleanupcap-Cancel. Nur sein exakter
Ack erlaubt danach genau
einen `controller-clock-sample(cleanup-completion-after-cap-cancel)`; der
Sample bestimmt nur Stage-10-Timing und Receipt Order und kann die Deadline
nicht rückwirkend ändern. Ein gültiger Sample ergibt Check 20 `confirmed`,
Stage 10 `observed/match` und `all-steps-terminal`. Ein unobservables
Cancel-Settlement erlaubt keinen Completion-Sample und keinen weiteren
...
gehasht oder serialisiert. Die nach Handlerreihenfolge gültige `receiptOrder`
bleibt davon getrennt und ist niemals gültiges Timing. Ein ungültiger Sample,
Cancelfehler oder irreversibler Routing-/Clock-/Portfehler ergibt
`cleanup-terminal-failure`, Check 20 `failed` und Stage 10
`observed/mismatch`. Gewinnt die inklusive 60.000-ms-Deadline, bleiben offene
Checks `unproven`, Stage 10 `not-observed/unproven` und der Grund ist
`cleanup-cap`. Kein `pending` erreicht die Projection. `cleanup.result` folgt
der Präzedenz `FAIL` vor `UNPROVEN` vor `PASS`, und `cleanupFinalized` wird erst
nach einem dieser drei Abschlüsse wahr.

Die Foundation übernimmt die festen 59 Replayoperanden, sechs Operations,
17 Integritätschecks, zehn Stages, drei Clockdomänen, neun Requestzähler plus
Sequenz und 20 Cleanupchecks aus dem Living Contract. Sie projiziert jedoch
nur `candidateObserverGate` und `candidateFinding`. Die 17-Felder-Rootform
besitzt `projectionType: browser-transport-diagnostic-foundation-projection`,
kein `recordType`, kein echtes `observerGate` oder `finding`.
`FoundationResult` ist ein tief eingefrorener Sieben-Felder-Record mit
`[[Prototype]] === null` und ohne Own-`then`; er erzwingt
`evidenceStatus: NOT_EVIDENCE`,
`runtimeAuthorized: false` und `persistenceAuthorized: false`.

Der private Evaluationstring ist im ADR und Living Contract bytegleich als
einzeiliger ASCII-Wert mit 4259 UTF-8-Bytes und SHA-256
`a623ffafee8dfcbc1d2ddc374cc35f0dbf800defd97619a3b58337d972090f7b`
gebunden. Die reine Foundation setzt `foundationSha256: null`; Commit-,
Dateibyte-, Actual-Send-, Parser-, Pipe-, Timer-, Queue-, Ressourcen- und
Adapterprovenienz bleibt `unproven`. Ein callerlieferbarer Digest oder
positiver Fakt bestätigt keinen Integritätscheck.
...
ADR 0037 implementiert und in der 595/595-Foundation-Suite geprüft. Der aktuelle nächste Schritt ist ausschließlich der getrennt zu beauftragende
unabhängige Implementierungsreview. Adapterimplementierung, Adaptertests und sichtbarer
...
nicht ausdrücklich korrigierten Regeln aus ADR 0034, ADR 0033 und ADR 0032
gelten über ADR 0035 fort. ADR 0034 implementierte oder autorisierte weder
Foundation, Tests, Adapter noch Runtimevorgang und ersetzte keine weitere
Entscheidung.
...
### Angenommene Diagnostic Foundation Join and Internal Transition Testability Boundary / ADR 0035
...
Entscheidungsebene. Sämtliche nicht ausdrücklich korrigierten Regeln aus ADR
0034 sowie dessen fortgeltende ADR-0033-/ADR-0032-Regeln gelten unverändert
fort. Dieser Entscheidungsslice implementiert oder autorisiert weder Foundation
noch Testdatei, Adapter oder Runtimevorgang.

Die Korrektur adressiert ausschließlich den bisherigen Testbarkeitswiderspruch:
ADR 0033 verlangt einen dynamischen zweiten internen Exchange bei bereits
`observable-pending` gebundener Lease in `prestart`, `observation` und
`cleanup`; die öffentliche API macht diesen zweiten internen Aufruf
konstruktiv unerreichbar, während ADR 0034 bisher nur die zwei privaten
Candidateableitungen temporär öffnen durfte. Die beschlossene Testkopie v2
verwendet deshalb in einer eindeutig benannten `.mjs`-Datei in einem eindeutig
aufgelösten Betriebssystem-Temporärverzeichnis außerhalb des Repositorys
weiterhin exakte Produktionsbytes. Genau eine lexikalisch eindeutige
Exportdeklaration darf ausschließlich diese vier bereits produktiv
verwendeten privaten Bindings ergänzen:

| Binding | Arity | Rolle |
| --- | ---: | --- |
| `deriveCandidateObserverGate` | 1 | erhält exakt einen frischen, gewöhnlichen, geschlossenen und tief eingefrorenen `CandidateGateInput` mit genau den primitiven Booleans `{ hardViolation, proofIncomplete }`; gibt mit FAIL-vor-UNPROVEN-Präzedenz exakt `FAIL\|UNPROVEN\|PASS` zurück; rein |
| `deriveCandidateFinding` | 1 | erhält exakt einen entsprechenden `CandidateFindingInput` mit `{ candidateObserverGate, replayResult, stimulusCount, requestSequence, settlementOutcome, settlementStaticProfileResult }`; gibt nach fortgeltender Präzedenz exakt einen der fünf Findingwerte zurück; rein |
| `createBrowserSyncTransportRuntimeDiagnosticRunMachine` | 1 | erhält nach dem implementierten ADR-0037-Delta exakt einen foundationeigenen internen `RunMachineInput` mit `{ activeExchange, activeObservationClosed, runBinding }`: beide Capabilityrollen sind atomar transferiert, `runBinding` ist die validierte defensive tief eingefrorene interne Projektion; gibt exakt dieselbe private mutable Run-Machine-Referenz zurück, keinen Wrapper, keine API und kein Promise |
| `requestBrowserSyncTransportRuntimeDiagnosticExchange` | 2 | erhält exakt dieselbe Run-Machine-Identität und ihr produktiv erzeugtes frisches, gewöhnliches, geschlossenes und tief eingefrorenes `ExchangeRequestProfile` mit genau `{ phase, intentKind, payload }`; ist die einzige produktive Grenze für sämtliche sieben Intentarten und gibt stets `undefined` zurück |

Die beiden Ableitungen bleiben rein. Konstruktor und Exchange-Grenze sind
unveränderte Bestandteile des Produktionspfads, keine testexklusiven
Produktionsfunktionen. Die mutable Maschinenreferenz führt produktiv unter
anderem Owner-Run-Promise, Zustand, Lease, Testzähler, `O0` und Cleanup-Ledger;
sie wird von der öffentlichen API niemals ausgegeben. Nur die temporäre Kopie
erhält dieselbe Referenz unmittelbar aus dem Konstruktor. Der Test fordert die
Exchange-Grenze mit exakt dieser Identität und
`machine.activeExchangeRequestProfile` gegen deren aktive Lease an. Damit
werden Maschinenzustand, normales beobachtbares Effects-Protokoll und
Owner-Run-Settlement ohne fünften Inspector-, Debug- oder State-Export
beobachtet. Das Originalmodul behält genau einen öffentlichen Export.
Der öffentliche Pfad und der Test starten identisch mit der Maschine und ihrem
vorbereiteten `nextExchangeRequestProfile`; jeder kontrollierte Folgehandler
erzeugt das nächste Profil auf derselben Maschine und verwendet wieder nur
diese Grenze.

Die Konformitätskopie darf außer der einen Exportdeklaration keine
Funktionskörper, Konstanten, Inputs oder produktiven Callsites ändern. Sie wird
seriell über ihre aufgelöste File-URL importiert, bleibt zusammen mit allen
Resultaten `NOT_EVIDENCE`, wird im `finally` entfernt und ihre vollständige
Entfernung wird bestätigt. Davon disjunkte Mutantenkopien dürfen nur für die
geschlossen erlaubten mutationswirksamen Negativnachweise aus jeweils frischen
Produktionsbytes abgeleitet werden; weder sie noch die Konformitätskopie ändern
jemals die Produktionsquelle. Permanente `__test`- oder bedingte Exports,
Debug-/Environmentflags, zusätzliche Factoryoptionen, öffentliche
State-Inspection, zweite Produktdateien und manipulierte PASS-Projections
bleiben verboten.

Die einzige Exchange-Grenze muss vor allen sieben produktiven Intentarten
liegen. Bei `lease === observable-pending` prüft sie zunächst nur die
Run-Machine-Identität, setzt ausschließlich
`pendingInternalExchangeViolation` auf die aktuelle Phase und stoppt noch vor
jeder Reflection oder sonstigen Nutzung ihres zweiten Arguments sowie vor
Intentkonstruktion, nächster Intent-ID, Count- oder Ledgermutation, Cap-,
Snapshot- oder Cleanupmutation und Capability- oder Portaufruf. Die spätere
dynamische Matrix treibt dieselbe produktive Maschine über normale
Effects-as-Data-Übergänge getrennt nach `prestart`, `observation` und `cleanup`
in diesen Zustand, hält genau einen Exchange offen, nimmt unmittelbar davor
einen Baselinesnapshot und fordert über die echte Grenze den zweiten Exchange
an. Gegenüber der jeweiligen Baseline gilt:
...
aktuelle Exchanges  = exakt 1
```

Die finite Matrix umfasst exakt `3 × 2 × 3 = 18` Zellen: jede Maschinenphase
`prestart|observation|cleanup`, beide Ausgänge `fulfillment|rejection` und
jeweils alle drei Settlementzeitlagen:

| Zeitlage | Lage beim synchronen zweiten Boundaryaufruf |
| --- | --- |
| `pre-invocation` | `activeExchange` gibt ein bereits gesetteltes gültiges lokales natives Promise zurück; die installierten Handler sind noch nicht eingetreten. |
| `synchronous-post-invocation` | Das zurückgegebene Promise ist zunächst pending und wird nach Rücksprung der ersten Grenze, aber vor dem zweiten Aufruf synchron gesettelt. |
| `observable-pending` | Das Promise bleibt bis nach dem zweiten Aufruf pending und wird erst danach gesettelt. |

Zwischen erstem und zweitem Boundaryaufruf ist in allen Zellen jedes `await`,
jeder Microtask-Checkpoint, Timer oder andere Yield verboten. Der zweite
Aufruf erfolgt im selben Turn vor dem kontrollierten Handlerzutritt. Das
Promise-Settlement selbst ist kein Maschinenereignis; Lease und aktueller
Exchange bleiben bis zum Handler `observable-pending` beziehungsweise exakt
eins.

Das einzige neue Join-Ergebnis ist die phasengenaue Klassifikation
`pendingInternalExchangeViolation = prestart|observation|cleanup`. In allen 18
Zellen werden Fulfillment und Rejection ohne Lesen von Payload oder
Rejectiongrund geprüft. Danach gelten
`lease: closed`, `activeExchange: null`, `portState: closed` und
`furtherExchangeCount: zero`; alle noch lebenden Caps werden
`terminal-unknown`. Späte oder doppelte Handler sind token- und
zustandsgegatete `undefined`-No-ops. Die Folgen sind geschlossen:

| Phase | Folge nach kontrolliertem Fulfillment oder Rejection |
| --- | --- |
...
Das von den 18 settelnden Zellen getrennte Forever-pending-Oracle ist
ausdrücklich zweigeteilt. Dynamisch bleibt in
jeder Phase das erste gültige lokale native Promise ungesettelt, der zweite
Exchange wird durch die echte Grenze unterdrückt, die Delta-Invarianten werden
sofort und nach exakt drei vollständig durchlaufenen testlokalen Microtask-
Checkpoints geprüft. Danach bestehen weiter genau ein aktueller Exchange,
Run-Settlementcount 0, kein künstliches `O0`, kein vorzeitiger Cleanup, keine
Terminalmaterialisierung, keine weitere Capability und keine neue Intent-ID.
Eine zusätzliche Probe lässt das Promise bis zum Testende pending und wartet
den Owner-Run bewusst nicht ab. Reale Uhr, Timer, Timeout oder `Promise.race`
des Produktionspfads sind dafür verboten.

Separat muss die vollständige private Transitionstabelle strukturell zeigen,
dass aus `observable-pending` ohne Eintritt in einen kontrollierten
Fulfillment- oder Rejectionhandler kein Fortschritts-, Snapshot-, Cleanup-,
Terminalisierungs- oder Resolvepfad existiert: kein Timerersatz, zweiter Port,
konkurrierender Resolver, Retry, autonomes Polling, zusätzliche Capability oder
Übergang zu `idle`. Nur Präfixprobe und vollständige Tabelle zusammen bilden
den vertraglichen Forever-pending-Nachweis; eine empirisch beobachtete
unendliche Zeitspanne wird nie behauptet.

Der spätere Implementierungsslice muss außerdem mutationswirksam mindestens
das Entfernen oder Umgehen des Join-Guards, seine Verschiebung hinter
Intentkonstruktion, ID-Erhöhung oder Count-/Ledgermutation, einen zweiten Port-
oder Capabilityaufruf, Leasefreigabe zu `idle`, künstliches Owner-Settlement,
vorzeitiges `O0` oder Cleanup, Verlust der Phasenklassifikation und Mutation des
eingefrorenen `O0` im Cleanupfall erkennen. Diese Mutationen bleiben auf
kontrollierte, von der Konformitätskopie getrennte Testkopien beschränkt.

ADR 0035 präzisiert daneben drei öffentlich über den normalen Effects-Port
black-box testbare Regressionen; sie benötigen keinen privaten Zusatzexport:
...
- Der vollständig geprüfte erste eindeutig attribuierte Endpoint-Request
  ...
  Networkkandidat muss vor Stage-, Count-, Timing- oder Sequenzmutation einen
  primitiven numerischen, endlichen und nichtnegativen Timestamp besitzen;
  sowohl `timestamp * 1000` als auch die nichtnegative Differenz zum ersten
  Endpoint-Timestamp in Millisekunden müssen endlich und betragsmäßig höchstens
  `Number.MAX_SAFE_INTEGER` sein, und der Timestamp darf nicht kleiner als das
  letzte gültige Sample sein. Gleichheit ist zulässig, aktualisiert wird erst
  nach vollständiger Prüfung.
  Timing bleibt relativ zu `firstEndpointRequestTimestamp`. Ein Rücklauf vor
  `O0` ist sticky `V` mit `confirmed-violation`,
  `FAIL/observer-invalid` und `captureWindowState: truncated`; der verletzende
  Kandidat ändert weder letzten gültigen Timestamp noch Stage, Count,
  Receipt Order, Sequenz oder Timing. Die
  Pflichtfälle sind `10 → 12 → 11`, `10 → 9`, Gleichheit, `NaN`, `Infinity`
  und unsicherer Millisekundenüberlauf. Nach `O0` bleibt die Observation
  unverändert.
- Für `Network.responseReceived` lautet die descriptorbasierte
  Konsumreihenfolge nach Cap- und Dequeueguard exakt: eigene Data-Descriptoren
  von `method`, `sessionId` und `params`, dann `params.requestId`,
  `params.response` und dessen `url`; erst danach werden die gecachte URL mit
  dem gebundenen Endpoint und die gecachte Request-ID mit einer bereits
  gebundenen Identität verglichen. Jeder Descriptor wird höchstens einmal
  gelesen und sein Wert gecacht. Bei exakter Endpoint-URL und fehlgeschlagener
  Request-ID-Korrelation stoppt die Reflection unmittelbar; insbesondere
  bleiben `params.timestamp`, `response.status`, `statusText`, Header-, MIME-,
  Protocol-, Timing-, Security-, Cache-, Body- und Folgewerte ungelesen. Die
  begrenzte URL-Lesung trennt die unkorrelierte Produktendpoint-Response von
  einer fremden Response. Die Attribution wird bis `O0` sticky unbewiesen beziehungsweise mehrdeutig,
  ohne allein ein `V` zu erzeugen; es entsteht kein Requestcount und keine
  erfundene ID. Die Sequenz bleibt `ambiguous`, `productEvidenceComplete`
  bleibt `false` und der Candidate `UNPROVEN/inconclusive`. Ohne spätere
  korrelierte Response bleibt der betroffene Stage-Slot
  `not-observed/unproven/null/unavailable`; eine spätere korrekte Response darf
  den Slot `observed/match` setzen, stellt aber weder eindeutige Attribution
  noch Erfolgssequenz wieder her. Malformed Routing und unabhängige
  Obserververletzungen behalten ihre bestehende `V`-Ableitung.
- Exakt ein gültiger `Target.getTargets`-Sende-Ack bleibt stets
  `one/match`. Eine Antwortdublette ist weder Intent noch Ack und ändert dieses
  Operationsergebnis nie. Eine wohlgeformte Dublette vor Evaluate ergibt ohne
  künstliches `V` exakt `U`, `setup-terminal-unproven`, Targetprofil `unknown`,
  `singleTargetAndSessionConfirmed: unproven`, Evaluate-Intent `zero`,
  Evaluate-Send `zero/match`, Capture `not-started` und
  `UNPROVEN/inconclusive`; Stage 1 bleibt `observed/match`, Stages 2 bis 8
  bleiben `not-observed/unproven`. Während Capture bleibt die Operation
  `one/match`, Antwort und Targetprofil werden mehrdeutig beziehungsweise
  `unknown`, und der Candidate bleibt bis `C` `UNPROVEN/inconclusive`. Nur eine
  eindeutig routbare malformed Dublette vor `O0` ist
  `V/FAIL/observer-invalid`; nach `O0` gelten ausschließlich die bestehenden
  Cleanup-Purpose-Regeln.

Unverändert bleiben Schema 1, eine öffentliche Factory und ein Export, sieben
Intentarten, sechs Protocol Commands, 59 Replayvergleiche, 17
Integritychecks, zehn Stages, zehn Capzustände und 20 Cleanupchecks. Öffentlich
erreichbar bleiben nur `FAIL/observer-invalid` und
`UNPROVEN/inconclusive`; Candidate-`PASS`,
`none-contract-visible-detected`, PASS-spezifische Findings und PASS-Fallback
bleiben konstruktiv unerreichbar. ADR 0029 bleibt `overallGate: FAIL`, seine
...
Adapter-ADR-Abgleich, Adapterimplementierung und sichtbarer Diagnoselauf
bleiben getrennte nachgelagerte Schritte.
...
`Vorgeschlagen – 2026-09-06`. Der nach R1–R4 korrigierte Vorschlag arbeitet K2
...
aus; ADR 0036 bleibt vorgeschlagen und nicht annahmereif. Er
...
Adapterowner. Callerlieferbare Capabilities, `runBinding`, Gates, Findings,
Digests, Provenienz- und Cleanupwerte sind verboten.

K2 ist über zwei strikt disjunkte, aus bytegeprüften Produktionsbytes erzeugte
Vier-Export-Kopieprofile konstruktiv ausgearbeitet und im begrenzten R1–R4-Dokumentationsscope geprüft. `derivation-conformance` öffnet Factory plus
Gate/Finding/Finalizer und ersetzt den Capabilityselector durch einen exakten
synchronen Factorythrow vor Owner, API, Promise oder Hostzugriff.
`virtual-runtime-conformance` öffnet Factory plus denselben produktiven Owner,
die einzige produktive Producer-Eventgrenze und einen einmaligen virtuellen
Capabilityinstaller. Das vollständige Profil umfasst Entropie mit getrennten
...
sind gefroren; mutable Bytes werden defensiv kopiert, Handles bleiben opaque.
Raw-Fixtureevents erreichen Foundation-Dequeuewerte nur über gebundene Sinks,
produktiven Owner-/Generationguard, Parser und FIFO. Beide Profile bleiben
`adapterEvidenceEligible:false`; Selector-Poison- und Wiringmutanten müssen
Realfallback und Ersatzmaschinen töten. Das ADR-0035-Foundationtestkopieprofil
wird durch K2 weder geändert noch mit den Adapterprofilen kombiniert.

Die spätere Adaptertestsuite bleibt am Raw-Byte-/Producerpfad und prüft Setup
und Cleanup an `deadline-1`, `deadline`, `deadline+1`, Capture ausschließlich
ereignisbasiert sowie gemeinsamen Clockwert und Materialfreigabe. Der
produktive JSON-Pfad erzeugt keine Getter-/Proxy-Envelopes. Die konkrete
Foundation-Testlücke ist durch die ADR-0037-Tests geschlossen: Setup und
Cleanup erreichen bei `deadline` und `deadline+1` jeweils exakt null
`get`-, `getPrototypeOf`-, `ownKeys`- und
`getOwnPropertyDescriptor`-Traps. Bei `deadline-1` belegen gültige
Kontrollen die tatsächliche Envelopeprüfung. Derselbe Dequeue-/Clockvektor
tötet pro Phase die Mutation `>=` zu `>` sowie das Vorziehen der
Envelopeprüfung vor den Guard. Das ist ein Foundationnachweis; die grüne Suite
ersetzt keinen Adapter-Wiringnachweis.

Der Adapter würde die tatsächlich ausgeführte Foundation aus einem einmalig
Commit-geprüften Bytesnapshot byte-owned und ABA-geschützt laden. Er besitzt
genau den totalen Sieben-Intent-Effects-Port, eine einzige
controllerlokale FIFO, den Windows-Chrome-Debug-Pipe mit NUL-Framing, fatalem
UTF-8, bounded Duplicate-Key-Scanner und genau einem nativen `JSON.parse`,
den vollständigen Write-Ack-/Evaluate-Capture-Commit sowie die unveränderten
6.000-/6.000-/60.000-ms-Caps. Dequeue liest keine Clock; der nachgelagerte
Foundation-Clockintent liest einmal denselben primitiven Wert für Foundation
und Ledger. Nur Setup/Cleanup verwenden `>=`, Capture nur das FIFO-
`cap-fired`. Es entstehen weder Debug-TCP-Port noch siebter
CDP-Command, Polling, `Promise.race` oder Ersatzcap.

Der künftige Nodeprozess ist exakt an `--experimental-vm-modules` und
`--no-warnings` gebunden. Die Warnunterdrückung verhindert den
SourceTextModule-ExperimentalWarning-Write, beweist ohne unabhängigen Owner von
Adapter-stdout/-stderr aber keine Ausgabenabwesenheit; der Integritycheck
bleibt `UNPROVEN`.

Launcher-, Handle-, Environment-, Profil- und Cleanupidentitäten liegen in
einem gemeinsamen idempotenten Ressourcenledger. Der Adapter baut die neun
`runBinding`-Felder und alle 59 Replayoperanden selbst. Ein späterer
17-Felder-`BrowserTransportDiagnosticRecord` würde erst nach terminalem Cleanup
frisch aus der unveränderten `FoundationProjection` und authentischem
Adapterledger abgeleitet. Bestätigte Verletzung ergibt zuerst und unabhängig
von `zero|unknown|multiple|one` Stimuli `FAIL/observer-invalid`; erst danach
wirken Evidenzdemotion und `UNPROVEN`, und nur PASS/die drei Produktfindings
fordern exakt einen Stimulus. Für Operand 53 allein wird der rohe
Vier-Codeunit-Child-Portwert durch umgebende U+0022 in sechs Codeunits
projiziert. Writer und Persistenz liegen hinter einer getrennten Grenze.

Die festgelegte Node-Core-Grenze kann weder Windows-Prozessnachfahren über
einen Job-Owner binden noch Profil-/Fragmentpfade handle-relativ löschen.
Root-Childexit und Pfad-Vorcheck sind deshalb kein positiver Cleanupbeweis;
nach möglichem Spawn beziehungsweise Create bleiben die betroffenen Checks
`UNPROVEN`. Ein sichtbarer Lauf bleibt auch aus diesem Grund geschlossen.

Ohne zusätzliche authentische Quelle bleiben `browser.engineBuild`, globale
Portfreiheit, effektive Proxy-/VPN-/Policy-/Extension-/Permission-/
Service-Worker-/Cachewerte und eine unabhängige Adapterattestierung
`UNPROVEN`. Einzelne Cancelpayloads und spätere Projektionen tragen keine
O0-Phasenbindung; daraus folgt weder Injektivität noch Nicht-Injektivität der
vollständigen öffentlichen Historie. Der beschlossene bounded syntaktische
Tracker kann unter seinem Spiegelungsverbot keinen authentischen Pre-Cleanup-
Zeitpunkt für alle Pfade auswählen; ein Promisezaun ist auch für portlosen
Pre-Cleanup zu spät. Als minimale eindeutige Phasenbindung unter den
bestehenden Architekturgrenzen entscheidet ADR 0037 `D_K4`: genau eine
synchrone argumentlose Foundationnotification
`effectPort.observationClosed()` unmittelbar nach `O0` und vor Old-Cap-Scan,
portlosem oder regulärem Cleanup. Sie ist kein achter Intent und trägt keine
Ursache; die Auswahl ist kein informationstheoretischer Notwendigkeitsbeweis.
Die sieben Fallklassen beachten insbesondere Portschluss vor `O0` bei
unbeobachtbaren Observationfehlern, Old-Cap-Scan nach `O0` und Cleanup-Ledger
vor `cleanup-origin`. Die Entscheidung ist angenommen und das Foundationdelta netzwerkfrei
implementiert und geprüft; sein unabhängiger Implementierungsreview steht
aus. Es gibt weiterhin keinen Adapter und keinen authentischen `A_obs`-Freeze;
ein evidenzfähiger Record ist damit nicht nachgewiesen. ADR 0036
implementiert weder Adapter noch Tests,
Parser, Timer, Launcher, Recordwriter oder Runtimekomposition und autorisiert
keinen Diagnoselauf. ADR-0029-`overallGate: FAIL`,
`causeStatus: CAUSE_NOT_PROVEN`, geschlossene Browserkomposition und fehlendes
Browser-End-to-End bleiben unverändert.
...
Entscheidungsebene. ADR 0036 schlägt nun ausschließlich die dokumentarische
nach R1–R4 korrigierte Adaptergrenze vor; die K2-Konstruktion ist im begrenzten
R1–R4-Dokumentationsscope geprüft. Das angenommene ADR-0037-`D_K4`-Delta ist implementiert und mit 595/595
fokussierten Tests geprüft. Der unabhängige Implementierungsreview und der
anschließende ADR-0036-Load-/Hash-/Testabgleich stehen aus; ADR 0036 bleibt
...
Damals sollte als nächster Slice ausschließlich der weiterhin getrennte reale,
...
Browserkomposition oder End-to-End-Fluss. Damals war als nächster Slice nur die
isolierte, netzwerkfreie Implementierung mit `node:vm`-Regressionen und dem
193/192-Source-Mutationsnachweis freigegeben. Erst danach durfte ein eigener
...
11. ADR 0030 entscheidet die passive BrowserSyncTransport-Diagnosegrenze; ADR
    0031 ersetzt sie formal; ADR 0032 ersetzt ADR 0031 und totalisiert Setup-,
    vollständige Capture-, Timing-, Hash-, Projection-, Replay-, Snapshot-/
    Ledger- und Cleanupgrenzen, ohne Foundation, Adapter oder Lauf zu
...
14. ADR 0035 ersetzt ADR 0034 formal, korrigiert ausschließlich die interne
    Pending-Join- und Transition-Testbarkeitsgrenze und führt alle nicht
    ausdrücklich korrigierten Regeln fort; ADR 0034 bleibt mit bytegleichem
    Hauptteil historische Entscheidungsebene;
...
16. ADR 0036 schlägt Raw-Pipe-, Parser-, Queue-, Ressourcen-, Cap-/Timer-,
    Adapter-, Launcher-, Provenienz- und Hashbindung rein dokumentarisch vor
    und arbeitet K2 über zwei disjunkte Testkopieprofile konstruktiv aus;
    künftige Adaptertestnachweise bleiben offen;
17. der unabhängige R1–R4-Dokumentreview ist abgeschlossen;
18. ADR 0037 entscheidet `D_K4` als synchrone `observationClosed()`-Notification;
    der unabhängige dokumentarische Review ist abgeschlossen und Jan hat ADR
    0037 am 2026-09-08 ausdrücklich angenommen. Das danach gesondert beauftragte
    Foundationdelta ist implementiert und mit 595/595 fokussierten sowie
    2350/2350 Gesamttests geprüft. Als Nächstes steht ausschließlich sein
    getrennter unabhängiger Daybreak-Implementierungsreview mit `xhigh` aus;
    danach folgen Jans manueller Featurecommit und der gesonderte
    ADR-0036-Load-/Hash-/Testvertragsabgleich samt erneutem unabhängigen Review;
19. erst nach weiterem Review und ausdrücklicher ADR-0036-Annahme wird der
    Adapter getrennt netzwerkfrei implementiert und geprüft;
...
21. erst anschließend werden Zielbrowser, `T_replay`, Observer, höchstens ein
...
26. erst nach dessen Gesamt-`PASS` wird die Browserkomposition in einem
    weiteren getrennten Slice umgesetzt;
27. der lokale Browser-End-to-End-`syncTest` wird danach getrennt
    nachgewiesen;
...
Cleanup ist `PASS`; die Ursache bleibt `CAUSE_NOT_PROVEN`. ADR 0035 ist
angenommen, ersetzt ADR 0034 formal und übernimmt alle nicht ausdrücklich
korrigierten Regeln aus ADR 0034, ADR 0033 und ADR 0032; ADR 0034 bleibt mit
bytegleichem Hauptteil und ADR 0033 sowie ADR 0032 als historische
...
driftfrei. ADR 0036 bleibt nach R1–R4 dokumentarisch geprüft, vorgeschlagen
und nicht annahmereif. Als Nächstes steht ausschließlich der getrennte
unabhängige Implementierungsreview aus; danach folgen Jans manueller
Featurecommit und der gesonderte ADR-0036-Load-/Hash-/Testabgleich samt
erneutem unabhängigem Review. Adapterimplementierung, Adaptertests und
...
| [0034](decisions/0034-browser-sync-transport-diagnostic-foundation-grammar-derivation-and-testability-boundary.md) | BrowserSyncTransport Diagnostic Foundation Grammar, Derivation and Testability Boundary | Ersetzt durch ADR 0035 |
| [0035](decisions/0035-browser-sync-transport-diagnostic-foundation-join-and-internal-transition-testability-boundary.md) | BrowserSyncTransport Diagnostic Foundation Join and Internal Transition Testability Boundary | Angenommen – 2026-09-05 |
| [0036](decisions/0036-browser-sync-transport-runtime-diagnostic-adapter-boundary.md) | BrowserSyncTransport Runtime Diagnostic Adapter Boundary | Vorgeschlagen – 2026-09-06 |
