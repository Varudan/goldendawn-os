...
| Feld | Wert |
| --- | --- |
| Projektphase | `v0.3.0 – ADR 0035 und ADR-0037-D_K4-Delta angenommen – 2026-09-08; Foundationnotification implementiert und mit 595/595 fokussierten Tests geprüft; unabhängiger Implementierungsreview ausstehend; ADR 0036 vorgeschlagen und nicht annahmereif; Adapter und Diagnoselauf nicht autorisiert; overallGate FAIL; causeStatus CAUSE_NOT_PROVEN` |
| Vertragsversion | `1.0` |
...
Der implementierte Port ist exakt
`effectPort = { exchange, observationClosed }` in dieser Own-Key-Reihenfolge.
Beide Felder sind erforderlich und werden einmalig descriptorbasiert erfasst.
Für die funktionale Notification wird ausschließlich bei der öffentlichen
Factoryprüfung genau einmal ihr eigener nicht aufzählbarer Data-`length`-
Descriptor mit Wert `0` geprüft. Es gibt keine zusätzliche Native-, Realm-,
Prototype- oder Freezeklassifikation der Funktion und keinen Probeaufruf.
Die alte Ein-Feld-Form besitzt keinen No-op-Fallback. Der tatsächliche Aufruf
verwendet die erfasste Apply-Intrinsic, null Argumente und `undefined` als
Receiver; nur der synchrone Rückgabewert `undefined` ist gültig.

Die beiden getrennten Capabilityrollen besitzen jeweils höchstens einen
persistenten Slot und werden synchron an den einzigen Run-Owner transferiert;
beide Factoryslots werden dabei geleert. Identitätsalias der beiden Funktionen
ist zulässig. Ein Exchange-Portschluss vor `O0` verwirft die noch erforderliche
Notification nicht. Terminale Pre-start-Pfade verwerfen beide Rollen ohne
Notification; ein tatsächlich vor `O0` pending bleibender Exchange erzeugt
weder Snapshot noch Marker. Keine Capabilityreferenz und kein sonstiger
Eingabecontainer erreicht einen Outputgraphen.

Der einzige produktive Notificationaufruf liegt in
`freezeObservationSnapshot`: zuerst wird `O0` frisch erzeugt, tief eingefroren
und zugewiesen; unmittelbar danach wird der Notification-Slot vor dem Aufruf
auf `null` gesetzt und sein Zustand auf `invoking` gesetzt. Erst nach diesem
synchronen Schritt folgen Phasenwechsel, geordneter Altcapscan und regulärer
oder portloser Cleanup. Die Zustandsfolge `armed -> invoking -> consumed`
erlaubt höchstens einen Aufruf. Ein Throw, jeder andere Rückgabewert oder
interne Reentranz setzt eine sticky Notificationverletzung. Erst nach
Rückkehr beziehungsweise kontrolliertem Throwabschluss des Callbacks übernimmt
der zentrale Freezer die Verletzung in den initialen Cleanupfehler; während
`invoking` verändert der Reentranzguard keinen Cleanupzustand. `O0` und
sein observationsseitiges `V` bleiben unverändert. Rückgabe- und Throwwerte
werden weder reflektiert noch assimiliert. Der zentrale Exchangeguard stoppt
Notificationreentranz nach Arity-/Maschinenidentitätsprüfung, aber vor
Profilreflection, Pending-Join, Intent-, ID-, Count-, Ledger- oder Effektwirkung.
Öffentliche reentrante Nicht-Owner-Aufrufe bleiben separat inert.

Das private produktive `RunMachineInput` lautet jetzt exakt
`{ activeExchange, activeObservationClosed, runBinding }`; der einstellige
Konstruktor wiederholt die öffentliche Notification-`length`-Prüfung nicht.
Die temporäre Konformitätskopie besitzt unverändert Factory plus vier private
Exports, insgesamt fünf Namen. Zusätzlich dürfen ausschließlich die bereits
produktiven Werte `activeObservationClosed`, `observationNotificationState`
und `observationNotificationViolation` beobachtet werden; es gibt keinen neuen
Inspector oder Output. Schema, sieben Intents, sechs Commands, 17
Projectionrootfelder, 59 Replayvergleiche, 17 Integritätschecks, zehn Stages,
zehn Capzustände, 20 Cleanupchecks und drei Clockdomänen bleiben unverändert.
Die Notification ist kein Intent, Ack oder Promise und erhöht keinen
bestehenden Exchange-, Intent-, Send- oder Capabilityzähler.

Die sieben Ablaufklassen sind einschließlich terminaler, unobservabler und
pending Ausgänge geprüft. 27 isolierte Notificationmutanten und vier
Deadlinemutanten scheitern an ihren jeweiligen kausalen Nachweisen. Sechs
Deadlinebaselines prüfen Setup und Cleanup bei `deadline-1`, `deadline` und
`deadline+1` mit getrennten Zählern für `get`, `getPrototypeOf`, `ownKeys` und
`getOwnPropertyDescriptor`: bei Gleichheit und Überschreitung jeweils null
Traps, darunter positive Sensitivitätskontrolle. Damit ist diese bisher offene
Nachweislücke ausschließlich auf Foundationebene geschlossen. Die 18 finiten
Pending-Joins sowie drei Microtask-Checkpoints und strukturelles
Forever-pending-Oracle bleiben erhalten. Die unveränderten Regressionen
bestehen mit 423/423, 466/466 und 735/735 Tests. Die vollständige serielle
Gesamtsuite besteht mit 2350/2350 Tests, exakt `1755 + 595`. Alle Läufe
besitzen 0 Fehlschläge, Cancellations, Skips oder Todos. Der Produktions-Build
transformiert exakt 46 Module; `bundle:n8n:check` besteht mit Exitcode `0`
driftfrei. Die Foundationnachweise bleiben netzwerkfrei; die Gesamtregression
enthält ausschließlich die beiden unveränderten etablierten
Loopback-Regressionsfixtures, kein neues, externes oder diagnostisches Netzwerk.
...
Die übrigen 14 gebundenen Schutzdateien besitzen jeweils ihren bisherigen
Rohhash und sind bytegleich zu `HEAD`. Der 4259-Byte-Evaluationstring bleibt
bei `a623ffafee8dfcbc1d2ddc374cc35f0dbf800defd97619a3b58337d972090f7b`;
das Frontendmanifest bleibt in historischem Commit, `HEAD` und Worktree bei
51 Pfaden, 5606 Bytes und
`6f3d5740b043308b4d38df33b6293c9064d8dd1b3f0c5801d50844336c195591`.
ADR 0037 einschließlich seines Hauptteils bleibt bytegleich. Diese lokale
Prüfbindung ist keine Runtimeprovenienz: `foundationSha256` bleibt in der
FoundationProjection `null`, `evidenceStatus: NOT_EVIDENCE`,
`runtimeAuthorized: false` und `persistenceAuthorized: false` bleiben fest.

Der unabhängige Daybreak-xhigh-Implementierungsreview dieses D_K4-Slices steht
aus; die historischen Dokument- und ADR-0035-Reviews belegen ihn nicht. Nach
dieser Übergabe folgt ausschließlich sein getrennt zu beauftragender Review.
Danach bleiben Jans manueller Featurecommit und der separate Abgleich von ADR
0036 gegen die tatsächlich neue Foundation, ihre Load-/Rohhashbindungen und
Testverträge gesonderte Schritte. ADR 0036 muss anschließend erneut unabhängig
geprüft werden, bevor Jan über seine Annahme entscheidet. Seine bisherigen
Hashbindungen werden in diesem Slice nicht geändert. Adapterimplementierung,
Adaptertests, `A_obs`, Recordpersistenz und Diagnoselauf bleiben geschlossen;
...
`BrowserTransportDiagnosticRecord` und die durch ADR 0035 korrigierte,
inzwischen getrennt netzwerkfrei implementierte Effects-as-Data-
Foundationgrenze. ADR
...
ist `CAUSE_NOT_PROVEN`. ADR 0035 ist angenommen, ersetzt ADR 0034 formal und
führt alle nicht ausdrücklich korrigierten ADR-0034-/ADR-0033-/ADR-0032-Regeln
fort. ADR 0034 bleibt mit bytegleichem Hauptteil und zusammen mit ADR 0033 und
...
beziehungsweise driftfrei. Als
rein dokumentarische Grenze ist ADR 0036 nach R1–R4 korrigiert vorgeschlagen,
aber nicht angenommen. Seine K2-Konstruktion ist im begrenzten
...
fokussiert geprüft. Der unabhängige Daybreak-xhigh-Implementierungsreview steht
aus und ist nach Übergabe der einzige nächste gesondert zu beauftragende
Schritt. Danach folgen erst Jans manueller Featurecommit und der getrennte
ADR-0036-Abgleich; Adapterimplementierung, Adaptertests und sichtbarer
Diagnoselauf bleiben geschlossen.
...
## Historischer Browser Transport Diagnostic Foundation Effects Protocol / ADR 0033

[ADR 0033](decisions/0033-browser-sync-transport-diagnostic-foundation-effects-protocol-boundary.md)
ist durch ADR 0034 formal ersetzt. Er ersetzte ADR 0032 formal und übernahm
dessen nicht ausdrücklich korrigierte Regeln; ADR 0033 bleibt mit bytegleichem
Hauptteil und ADR 0032 als historische Entscheidungsebenen erhalten. Dieser
Living Contract beschreibt den fortgeltenden Vertrag der reinen,
importinaktiven, netzwerkfreien Foundation; sie ist inzwischen getrennt
implementiert und geprüft. ADR 0035 ist als enge Korrektur ihrer internen Join-Testbarkeit
angenommen, ersetzt ADR 0034 formal und führt dessen nicht ausdrücklich
korrigierte Regeln fort; ADR 0034 bleibt mit bytegleichem Hauptteil historische
...
R1–R4-Dokumentationsscope geprüft. Die `D_K4`-Foundationentscheidung ist durch
ADR 0037 angenommen und mit 595/595 fokussierten Tests umgesetzt. Ihr
unabhängiger Implementierungsreview steht aus. Die folgenden historischen
...
korrigiert sind. Als Nächstes ist nach Übergabe ausschließlich der getrennt zu
beauftragende Daybreak-xhigh-Implementierungsreview zulässig. Adapterimplementierung,
Adaptertests, Parser,
...
## Historische Totalisierung des Foundationvertrags / ADR 0034

[ADR 0034](decisions/0034-browser-sync-transport-diagnostic-foundation-grammar-derivation-and-testability-boundary.md)
wurde am `2026-09-04` angenommen und ersetzte ADR 0033 formal. ADR 0035 hat ADR
0034 am `2026-09-05` formal ersetzt; ADR 0034 bleibt mit bytegleichem Hauptteil
ab `## Kontext` als historische Entscheidungsebene erhalten. Alle nicht
ausdrücklich korrigierten Regeln aus ADR 0034, ADR 0033 und ADR 0032 gelten
fort. ADR 0034 implementierte oder autorisierte weder Foundation, Tests,
Adapter noch Runtimevorgang und ersetzte keinen weiteren ADR. ADR 0029, sein
historischer Evidence-Record, `overallGate: FAIL` und
`causeStatus: CAUSE_NOT_PROVEN` bleiben unverändert.

Diese Kardinalitäten bleiben unverändert:
...
### Prototyp-, Frische- und Freezegrammatik

Jeder geschlossene unvertrauenswürdige Recordknoten wird exakt einmal und in
dieser Reihenfolge geprüft:

1. primitiver `typeof`-/Nulltest;
2. die bei Modulevaluation erfasste `Array.isArray`-Intrinsik einmal mit
   Ergebnis `false`;
3. `getPrototypeOf` einmal;
4. direkter Prototyp exakt der erfasste lokale `Object.prototype`;
5. `Reflect.ownKeys` einmal;
6. jeder erforderliche Descriptor einmal;
7. keine erneute Property-, Key-, Prototyp-, Brand- oder Descriptorlesung.

Die Own-Key-Folge muss codeunitgenau der knotenspezifischen geschlossenen
String-Key-Folge entsprechen; Symbole, Zusatzkeys, fehlende Keys und eine
andere Reihenfolge sind unzulässig. Jeder erforderliche Key muss eine
eigene aufzählbare Data-Property ohne Getter oder Setter besitzen. Sein Wert
darf nur dem einmal gelesenen und danach gecachten Descriptor entnommen
werden.

Das erfasst mindestens Factoryoptions, `effectPort`, `runBinding`, dessen
geschlossene Nachfahren, Replayeinträge, geschlossene Ackprofile und den
geschlossenen Main-World-v2-Graphen. Für jedes geschlossene
unvertrauenswürdige Array gelten stattdessen exakt einmal `Array.isArray` mit
Ergebnis `true`, exakt einmal `getPrototypeOf`, der direkte Vergleich mit dem
erfassten lokalen `Array.prototype` und danach exakt die bereits beschlossene
Dense-, Length-, Key- und Descriptorprüfung. Das erfasst mindestens
`replayOperands`, `targetInfos` und jeden weiteren ausdrücklich geschlossenen
Arrayknoten.

Diese Arrayprüfung verlangt bei Länge `n` exakt die Own-Key-Folge
`"0", ..., String(n - 1), "length"`, bei `n === 0` nur `"length"`.
Der eigene native `length`-Data-Descriptor muss eine nichtnegative
Safe-Integer-Länge `n` tragen; Holes, Symbole und Zusatzkeys sind verboten.
...
allowlistete Descriptoren preis und erhalten keine Record-, Array-, Realm-,
Plain-Data- oder Proxyklassifikation. Functions und Capabilities
werden weder traversiert noch eingefroren. `FoundationResult` bleibt der
einzige ausdrücklich festgelegte Null-Prototyp-Root; foundationeigene
gewöhnliche Records und Arrays verwenden durch Konstruktion die erfassten
lokalen Object- beziehungsweise Arrayprototypen.

`fresh` bedeutet exakt: an dieser Vertragsstelle neu durch die
Foundation erzeugt, nicht referenzidentisch mit einem caller- oder
portgelieferten Knoten und nicht mit einem anderen separat frisch verlangten
Container geteilt. Ein frischer Baum verlangt Frische jedes enthaltenen
Containers. Frische kann nur konstruktiv hergestellt, nie aus
unbeschränkter Fremdgraphtraversierung abgeleitet werden. `deep frozen`
erfasst nur foundationeigene, durch Own-Data-Kanten erreichbare Record- und
Arrayknoten. Der endliche azyklisch konstruierte Baum muss von den
Blättern zum Root eingefroren werden. Danach muss für jeden erreichbaren
Record- und Arrayknoten `Object.isFrozen === true`, ausschließlich eigene
Data-Properties und die geschlossene knotenspezifische Keyfolge gelten;
enthaltene Record- und Arraywerte müssen dieselbe Regel rekursiv erfüllen.
Prototypen, Promises, Functions, Capabilities und fremde Eingabegraphen werden
weder eingefroren noch verfolgt. Keine Prüfung beweist Proxy-, Realm-, Parser-
oder Pipeprovenienz.

### Geschlossene Stringgrammatiken

`printable-ascii-v1` beziehungsweise `A<n>` verlangt einen primitiven String
mit Länge `1..n` UTF-16-Codeunits, wobei jede Codeunit zwischen `U+0020` und
`U+007E` einschließlich liegt. Koerzierung, Normalisierung, Trimmung,
Case-Faltung und Unicodeabbildung sind verboten. Führende oder
abschließende U+0020 bleiben typgültig und können beim exakten Vergleich
regulär `mismatch` ergeben. Speziellere Grammatiken wie H64, IDs und
HTTP-tchar bleiben vorrangig.

`iana-shaped-ascii-time-zone-v1` bezeichnet genau folgende reine Syntaxklasse:

```text
exakt "UTC"
oder
mindestens zwei durch "/" getrennte, nichtleere Komponenten;
jede Komponente beginnt mit ASCII A-Z oder a-z;
danach nur ASCII-Buchstaben, Ziffern, ".", "_", "+" oder "-";
Gesamtlänge 1..64 UTF-16-Codeunits; case-sensitive.
```

Leere Komponenten, Unicode, Backslash und Whitespace bestehen nicht.
Die Syntax bestätigt weder IANA-Mitgliedschaft noch Kanonizität, Aliasauflösung
oder TZDB-Version; Foundationabfragen über `Intl`, Host oder
Netzwerk bleiben verboten. `Europe/Berlin`, `Etc/GMT+1` und
`America/Argentina/Buenos_Aires` bestehen.

`S32` bezeichnet ausschließlich Core-SemVer nach folgender Grammatik:

```regex
^(0|[1-9][0-9]*)\.(0|[1-9][0-9]*)\.(0|[1-9][0-9]*)$
```

Zusätzlich gelten eine Länge von `1..32` ASCII-Codeunits und die ausschließlich
lexikalisch-codeunitgenaue Prüfung. Numerische Konvertierung,
`v`-Präfix, Vorzeichen, Whitespace, führende Nullen außer der einzelnen `0`,
Prerelease und Buildsuffix bestehen nicht. Die historischen Werte
`24.19.0` und `8.1.4` bestehen.

### I1–I8- und Replaytotalisierung

Shape-, Descriptor- und Typfehler von `runBinding` bleiben synchrone
Factory-`TypeError`s. Semantische Cross-Field-Abweichungen werden keine
Factoryfehler. Privat besitzt jedes I1–I7-Ergebnis
`match|mismatch|unproven`: Sobald ein benötigter Operand
`not-observed` oder `ambiguous` ist, ist das Ergebnis `unproven`; sind alle
Operanden beobachtet und erfüllen die exakte Relation, ist es `match`, sonst
`mismatch`. Die 59 öffentlichen Comparisonresults bleiben unabhängig davon
unverändert.

| Invariante | Exakte Relation ohne URL-Parser |
| --- | --- |
| I1 | `frontend.topLevelUrl === frontend.serializedOrigin + "/"` |
| I2 | `initialUrl` ist codeunitgenau `initialScheme + "://" + initialHost + ":" + canonicalDecimal(initialPort) + initialPath`; `canonicalDecimal` ist die eindeutige Dezimaldarstellung des Safe-Integer-Ports ohne Vorzeichen, Whitespace oder führende Null, außer `0` selbst |
| I3 | Initialhost und Listenerhost sind exakt `127.0.0.1`; Initial- und Listenerport sind gleich; `gateway.portEnvironmentValue === "\"" + canonicalDecimal(listenerPort) + "\""`; `gateway.endpoint` ist codeunitgenau `initialScheme + "://" + listenerHost + ":" + canonicalDecimal(listenerPort) + initialPath` |
| I4 | `gateway.allowedOrigin.relationToFrontend === "matches-frontend-origin"` und zugleich `gateway.allowedOrigin.value === frontend.serializedOrigin` codeunitgenau |
| I5 | `transportRequest.initialUrl === gateway.endpoint` codeunitgenau |
| I6 | `viteRuntimeVersionObservation === null` oder Row 59 nicht beobachtet ergibt `unproven`; zwei beobachtete, nichtnullische und codeunitgleiche Werte ergeben `match`; zwei beobachtete ungleiche Werte `mismatch` |
| I7 | Nur beobachtetes `profile.lifecycle === "fresh-disposable"` zusammen mit eindeutig abgeleiteter neuer Profilinstanz ergibt `match`; eine vollständig beobachtete widersprüchliche Kombination ergibt `mismatch`; jeder andere Fall `unproven` |
| I8 | Die acht historischen Digests und Comparison-Basen bleiben privat fest; Callerwerte können nur Werte vergleichen und niemals Commit- oder Dateibyteprovenienz bestätigen; I8 erzeugt kein sechzigstes Feld und geht nicht als Authentizitätsersatz in `EQUIVALENT` ein; `sourceUnmodified` bleibt ohne Adapterbindung `unproven` |

Die totale Relation lautet exakt:
...
Die privaten I-Ergebnisse werden nicht projiziert und überschreiben die 59
Zeilenergebnisse nicht.

### Observer- und Operationstotalisierung

Die bislang offenen Observerfelder werden wie folgt abgeleitet:

| Feld | Quelle und positiver Wert | Gegenfall | unbekannter Fall |
| --- | --- | --- | --- |
| `controllerExclusivity` | in der reinen Foundation nicht positiv ableitbar | in der reinen Foundation nicht klassifizierbar | stets `unknown` |
| `connectionProfile` | in der reinen Foundation nicht positiv ableitbar | in der reinen Foundation nicht klassifizierbar | stets `unknown` |
| `targetProfile` | vollständig gültige, eindeutig korrelierte Targetbeobachtung mit genau einem passenden `page`-Target, gebundener URL und `attached === false` ergibt `single-goldendawn-top-level` | vollständig lesbares, typgültiges und eindeutig auswertbares Gegenprofil einschließlich null passender Targets oder genau eines passenden Targets mit `attached === true` ergibt `other` | fehlend, malformed, mehrdeutig, unkorrelierbar, nicht ausgeführt oder mehrere passende Targets ergibt `unknown` |
| `foundationSha256` | keine Foundationquelle | kein Callerwert darf einen Gegenbeweis ersetzen | ausnahmslos `null` |
| `evaluationSha256` | fester Soll-Digest nur nach exakt beobachtetem gültigem `Runtime.evaluate`-Sende-Ack `sent-and-capture-cap-started` | kein fremder Digest wird übernommen | kein Intent, Rejection, malformed oder unobservables Ack und unbekannter Sendestatus ergeben `null` |
| `controllerEvaluateIntentCount` | nach Erzeugung des einzigen Evaluate-Intents exakt `one` | `multiple` ist produktiv unerreichbar und ein interner Verstoß | vor Intent exakt `zero` |
| `mainWorldEvaluationCount` | genau ein vollständig akzeptierter Main-World-v2-Reply ergibt `one` | mindestens zwei korrelierbare Replykandidaten ergeben `multiple` | konstruktiv gegatetes Evaluate `zero`, sonst ohne akzeptierten eindeutigen Wert `unknown` |
| `transportFactoryCallCount` | nur der vollständig akzeptierte Main-World-v2-Wert darf `zero\|one` liefern | ein nicht akzeptierbarer oder widersprüchlicher Graph wird nicht als Count verwendet | konstruktiv gegatetes Evaluate `zero`, nach Evaluate ohne akzeptierten Wert `unknown` |

Network-, Send- oder Ackdaten dürfen Main-World- und Factorycounts niemals
ersetzen. Für jede der sechs `protocolOperations` gilt die folgende
sendzustandsabhängige Matrix:

| Bedingung | `observedCountClass` | `result` |
| --- | --- | --- |
| kein Intent, weil die zugehörige Ressource konstruktiv sicher nie aktiviert wurde | `zero` | `match` |
...
`observedCountClass: zero` allein bestimmt kein Ergebnis;
Antwortdubletten zählen niemals als zusätzlicher Send-Ack.
Bestätigte protokollspezifische Verstöße behalten ihre sticky-`V`- und
Cleanupfolgen.

### Partielle Target-/Session-/Networkmatrix

Die Ressourcenableitung schließt nicht aus einer fehlenden Session-ID auf eine
geschlossene Session:

| Setup-/Cleanupzustand | Closecheck und Folgeoperation |
| --- | --- |
| `Target.attachToTarget` sicher nie gesendet | `targetSessionClosed = confirmed`; `Target.detachFromTarget` nicht senden; Detach-Operation `zero/match` |
| Attach möglicherweise oder bestätigt gesendet, aber keine Session-ID sicher bindbar | `targetSessionClosed = unproven`; keine Session-ID erfinden; keinen Detach senden; Detach-Operation `zero/unproven` |
| Session sicher gebunden und korrelierter exakter Detach-Erfolg | `targetSessionClosed = confirmed`; Detach-Sendcount aus der Operationsmatrix |
| Session sicher gebunden, Detach nicht sicher erfolgreich | bestehende `failed\|unproven\|cleanupViolation`-Regeln aus ADR 0033 |
| `Network.enable` sicher nie gesendet | `networkDomainClosed = confirmed`; `Network.disable` nicht senden; Disable-Operation `zero/match` |
| Enable möglicherweise oder bestätigt gesendet, aber nicht sicher deaktivierbar | `networkDomainClosed = unproven`; falls kein Disable möglich, Disable-Operation `zero/unproven`; bei Intent ohne belegbaren Sendestatus `unknown/unproven` |
| korrelierter exakter Disable-Erfolg | `networkDomainClosed = confirmed`; Disable-Sendcount aus der Operationsmatrix |

Keines der folgenden Ereignisse beweist jemals
`targetSessionClosed = confirmed`: Attach-Sendeintent mit fehlendem
oder unbekanntem Send-Ack; beobachtete Attach-Senderejection; malformed oder
unobservables Attach-Send-Settlement; bestätigter Attach-Send ohne bindbare
korrelierte Erfolgsantwort; fehlende, malformed, doppelte oder
unkorrelierbare Attach-Antwort; Connection-Close ohne korrelierten
Detach-Erfolg. Symmetrisch kann ein möglicherweise oder bestätigt gesendetes
Enable ohne korrelierten Disable-Erfolg keine geschlossene Networkdomain
beweisen. Bei sicher gebundener Session und sicher nie gesendetem Enable
wird kein Disable erzeugt, Detach aber nach der Sessionmatrix versucht.

### 17-zeilige Integritymatrix

Jeder Check besitzt genau `confirmed|violated|unproven`. Die Spalte
Foundation-v1 beschreibt ausschließlich die Reichweite der reinen Foundation;
ein späterer Adapter darf nur identitätsgebundene eigene Fakten ergänzen.

| `checkId` | `confirmed` genau wenn | `violated` genau wenn | `unproven` genau wenn | Foundation-v1-Reichweite |
| --- | --- | --- | --- | --- |
| `sourceUnmodified` | später gebundene Checkoutbytes, Commit-Blobs und Foundationdigest stimmen exakt | dieselbe gebundene Quelle weicht ab | keine solche Provenienzbindung vorliegt | nur `unproven`; Callerhash-Mismatch ist Replay-, kein Sourcebeweis; `violated` produktiv unerreichbar |
| `instrumentedSourceCopyAbsent` | gebundene Adapter-/Residueinventur schließt jede Laufzeitkopie aus | dieselbe Inventur weist eine verwendete instrumentierte Kopie nach | Inventur oder Bindung fehlt | nur `unproven`; `violated` produktiv unerreichbar |
| `compositionSeamsAbsent` | gebundene Evaluation-, Command- und Capabilityledger schließen Zusatzseams aus | ein gebundener Ledger weist einen Zusatzseam nach | Ledgerprovenienz fehlt | nur `unproven`; fremde positive Portdaten reichen nicht, `violated` produktiv unerreichbar |
| `protocolAllowlistOnly` | jeder foundationeigene Intent und Command gehört vollständig zur geschlossenen Allowlist | der controllerlokale Ledger enthielte einen nicht allowlisteten eigenen Intent oder Command | der eigene Ledger vor Projection nicht total ist | bei `ok:true` `confirmed` by construction; ein interner Konstruktionsverstoß endet statisch, daher ist ausgegebenes `violated` produktiv unerreichbar |
| `runtimeSurfaceMutationAbsent` | spätere authentisch gebundene Evaluationbytes und Runtimeledger Mutationsfreiheit bestätigen | dieselbe authentische Bindung eine Observermutation belegt | Actual-Send-, Ausführungs- oder Aktionsprovenienz fehlt | ausschließlich `unproven`; ein produktiver Foundation-Gegenpfad existiert nicht |
| `fetchInterceptionAbsent` | spätere authentische Runtime-, Operations- und Adapterledger Interception ausschließen | derselbe authentische Ledger Observer-Interception belegt | authentische Vollständigkeit fehlt | ausschließlich `unproven`; `violated` ist ohne Adapter produktiv unerreichbar |
| `debuggerBreakpointsAndSteppingAbsent` | ein späterer vollständiger authentischer CDP-/Adapterledger die Abwesenheit der Debuggerdomain, Breakpoints und Stepping belegt | derselbe Ledger eine solche Operation belegt | nur der geschlossene Foundationledger ohne Vollständigkeit der realen Verbindung vorliegt | ausschließlich `unproven`; eine Debuggeroperation ist über den Foundationport nicht erzeugbar |
| `profilerAndTracingAbsent` | ein späterer vollständiger authentischer CDP-/Adapterledger Profiler und Tracing ausschließt | derselbe Ledger eine solche Operation belegt | reale Verbindungsvollständigkeit fehlt | ausschließlich `unproven`; der Gegenpfad ist produktiv unerreichbar |
| `responseBodyReadAbsent` | ein späterer authentischer Command- und Feldzugriffsledger Bodykommando und Bodylesen vollständig ausschließt | derselbe Ledger einen Bodyread belegt | Parser-, Pipe- oder Adaptervollständigkeit fehlt | ausschließlich `unproven`; zusätzliche Hüllenfelder beweisen kein Lesen |
| `freeRawInspectionAbsent` | ein späterer authentischer Parser-, Pipe-, Queue- und Descriptorledger vollständig nur allowlistete Reads belegt | derselbe Ledger freie Rohinspektion belegt | die Foundation nur ihre eigenen Descriptorreads, nicht den vorgelagerten Rohpfad kennt | ausschließlich `unproven`; Proxy- oder Hüllendaten dürfen nicht bestätigen oder verletzen |
| `additionalNativeFetchAbsent` | spätere authentische Main-World- und Requestownership-Ledger exakt den erlaubten Transportpfad und keinen Zusatzfetch belegen | dieselben Ledger einen zusätzlichen nativen Fetch belegen | Main-World-v2-Counts oder Networkdaten ohne authentische Ownership vorliegen | ausschließlich `unproven`; ein zweiter Endpointrequest ist Budgetgegenpfad, aber ohne Ownership kein `violated` dieses Checks |
| `observerProductEndpointRequestAbsent` | spätere authentische Requestownership und vollständiges Endpointbudget keinen Observerrequest belegen | dieselben Quellen mindestens einen observereigenen Produktendpointrequest belegen | Requestownership oder Vollständigkeit fehlt | ausschließlich `unproven`; Foundation-Networkdaten allein authentifizieren Ownership nicht |
| `rawPersistenceAbsent` | gebundene Capability-, Output- und Residueledger schließen Persistenz aus | gebundene Residueinventur weist Rohpersistenz nach | Residuebindung fehlt | nur `unproven`; `violated` produktiv unerreichbar |
| `observerDiagnosticDuringRunOutputAbsent` | gebundener vollständiger Outputledger schließt Ausgabe bis zum Endrecord aus | eindeutig attribuierte Diagnoseausgabe während des Laufs wird beobachtet | Vollständigkeit oder Bindung fehlt | nur `unproven`; `violated` produktiv unerreichbar |
| `closedPrimitiveProjectionConfirmed` | die erfolgreiche Projection frisch, referenzfrei, geschlossen und auf allen foundationeigenen Own-Data-Kanten tief eingefroren ist | eine fehlerhafte Projection materialisiert würde | Selbstprüfung nicht abschließbar ist | bei `ok:true` `confirmed` by construction; sonst statischer Foundationfehler, daher ist ausgegebenes `violated` produktiv unerreichbar |
| `singleTargetAndSessionConfirmed` | spätere Adapterprovenienz genau ein Target und genau eine flache gebundene Session bestätigt | ein bereits definierter eindeutig beobachteter Mehrfach-/Widerspruchspfad vorliegt | nur positive Portdaten oder unvollständige Attribution vorliegen | positiv nur `unproven`; bestehender klarer Mehrfach-/Widerspruchspfad `violated` |
| `singleMainWorldEvaluationConfirmed` | spätere Adapterprovenienz genau einen Send und eine korrelierte Antwort bestätigt | ein bereits definierter eindeutig korrelierter Mehrfach-/Widerspruchspfad vorliegt | nur positive Portdaten oder unvollständige Attribution vorliegen | positiv nur `unproven`; bestehender Mehrfach-/Widerspruchspfad `violated` |

`interferenceObservation` ergibt `contract-visible-detected` bei mindestens
einem relevanten `violated`, `none-contract-visible-detected` nur bei allen
relevanten `confirmed` und keinem `violated`, sonst `unknown`. Diese
Ableitung darf einen gewöhnlichen `U`-Pfad nicht zu `FAIL` umklassifizieren.

### Zehnzeilige Stagetotalisierung

Allgemein ergibt genau eine gültige eindeutig attribuierte Beobachtung
`observed`, kein bis `O0` akzeptierter Kandidat
`not-observed/unproven/null/unavailable`, mehrere oder widersprüchliche
Kandidaten `ambiguous/unproven/null/unavailable` und genau ein eindeutig
attribuierter bekannter Gegenwert `observed/mismatch`.
`receiptOrder` wird nur für `observed` je Layer lückenlos vergeben; Timing
stammt nur aus einem gültigen ebenenlokalen Sample. Eine gültige Order kann
fehlendes Timing nicht ersetzen.

| Stage | einzige Quelle | `observed/match` | `observed/mismatch` | `not-observed` | `ambiguous` |
| --- | --- | --- | --- | --- | --- |
| 1 `observer-armed` | lokaler Übergang nach positivem Setupcap-Arm und vor `attemptStarted` | auf jeder `ok:true`-Projection Order 1 und `0/measured` | produktiv unerreichbar; fehlerhafter Übergang ergibt den statischen Foundationfehler | produktiv bei `ok:true` unerreichbar | produktiv bei `ok:true` unerreichbar |
| 2 `transport-call-dispatched` | `execution.transportCallCount` eines genau einmal akzeptierten Main-World-v2-Werts | exakt `one`; `0/measured` nur, wenn das akzeptierte Settlement desselben v2-Werts einen gültigen Main-World-Startsample belegt, sonst `null/unavailable` | exakt `zero` als eindeutig bekannter Gegenwert, mit `null/unavailable` | kein akzeptierter v2-Wert oder kein akzeptierter Ausführungswert bis `O0` | mindestens zwei korrelierbare Replies oder widersprüchliche akzeptable Ausführungswerte |
| 3 `preflight-request-observed` | erster eindeutig attribuierter Produkt-`Network.requestWillBeSent` | exakte Endpoint-URL und Methode `OPTIONS`; als erster eindeutiger Produktrequest `0/measured` | eindeutig attribuierter erster Produktrequest mit anderer URL oder Methode | kein akzeptierter Kandidat bis `O0` | mehrere oder widersprüchliche Kandidaten für den ersten Produktrequest |
| 4 `preflight-204-observed` | `Network.responseReceived` für exakt die gebundene Stage-3-Request-ID | exakte Endpoint-URL und ganzzahliger Status `204` | andere URL oder anderer endlicher nichtnegativer Integerstatus | keine akzeptierte korrelierte Response bis `O0` | mehrere oder widersprüchliche korrelierbare Responses |
| 5 `post-request-observed` | nächster vom Preflight verschiedener eindeutig attribuierter Produkt-`Network.requestWillBeSent` | exakte Endpoint-URL und Methode `POST` | eindeutig attribuierter zweiter Produktrequest mit anderer URL oder Methode | kein akzeptierter zweiter Kandidat bis `O0` | mehrere oder widersprüchliche Kandidaten für diesen Slot |
| 6 `post-response-200-observed` | `Network.responseReceived` für exakt die gebundene Stage-5-Request-ID | exakte Endpoint-URL und ganzzahliger Status `200` | andere URL oder anderer endlicher nichtnegativer Integerstatus | keine akzeptierte korrelierte Response bis `O0` | mehrere oder widersprüchliche korrelierbare Responses |
| 7 `post-loading-finished / post-loading-failed / post-loading-terminal` | Terminalereignis für exakt die Stage-5-Request-ID | genau ein `Network.loadingFinished` ergibt `post-loading-finished/observed/match` | genau ein `Network.loadingFailed` ergibt `post-loading-failed/observed/mismatch` | kein Terminalereignis ergibt `post-loading-terminal/not-observed/unproven` | beide Varianten oder mindestens zwei korrelierte Terminalkandidaten ergeben `post-loading-terminal/ambiguous/unproven` |
| 8 `public-promise-settled` | Settlement eines genau einmal akzeptierten Main-World-v2-Werts | genau ein Pair `static-redacted-rejection/match`; Timing nur aus dessen Main-World-Dauer | genau ein Pair `fulfilled/not-applicable` oder `other-rejection/mismatch` als bekannter Gegenwert zur historischen Reproduktion | `settlement: null`, kein akzeptierter v2-Wert oder kein Settlement bis `O0` | mindestens zwei korrelierbare oder widersprüchliche Settlementkandidaten |
| 9 `cleanup-started` | lokale Anlage von Ledger und Stage nach tief eingefrorenem `O0` | auf jeder `ok:true`-Projection Cleanup-Order 1; bei gültigem `m_cleanup` `0/measured`, sonst `null/unavailable` | produktiv unerreichbar; nicht herstellbare lokale Anlage ergibt den statischen Foundationfehler | produktiv bei `ok:true` unerreichbar | produktiv bei `ok:true` unerreichbar |
| 10 `cleanup-completed` | ausschließlich terminale ADR-0033-Cleanupfinalisierung | Checks 1..19 terminal, Cleanupcap exakt storniert und gültiger Completion-Sample ergeben `observed/match` unabhängig von einzelnen terminalen `failed\|unproven`-Checks | irreversibler Cleanup-Controlfehler, ungültiger Completion-Sample, portlose Terminalisierung oder terminal fehlgeschlagener Abschluss ergeben `observed/mismatch` | Gewinn der Cleanupdeadline ergibt `not-observed/unproven` | produktiv unerreichbar; ein interner Mehrfach-/Widerspruchszustand ist ein irreversibler Cleanup-Controlfehler und damit `observed/mismatch` |

Stages 3 bis 7 verwenden ausschließlich Networkereignisse; Stage 8
ausschließlich Main-World-Settlement. Networkdaten dürfen Stage 2 oder 8
nicht ersetzen.

### Cleanupabschlussprädikate

Bei jeder erfolgreichen `ok:true`-Projection werden diese fünf rein
foundationeigenen Checks nach lokalem Abschluss `confirmed`:

| Check | Abschlussprädikat |
| --- | --- |
| `cleanupStarted` | `O0` ist tief eingefroren; Cleanup-Ledger und Stage 9 sind lokal angelegt |
| `controllerObservationClosed` | `O0` wurde vor jedem Cleanupwert irreversibel erstellt und keine Observation kann ihn danach verändern |
| `objectGroupsAbsentOrReleased` | es wurde kein Object-Group-Intent erzeugt, und jeder foundationeigene Object-Group-Zustand ist geschlossen; Handle und Object-Group-Operation bleiben verboten |
| `rawEventsDiscarded` | alle gehaltenen Dequeue-, Ack-, offenen Hüllen-, Own-Key-, Descriptor- und transienten Nachfahrreferenzen wurden verworfen |
| `ephemeralIdentifiersDiscarded` | Target-, Session-, Request-, Command-, Cap- und Intentbindungen wurden vor Projectionmaterialisierung aus allen erreichbaren Outputs entfernt |

Kann ein foundationeigener Abschluss nicht hergestellt werden, entsteht der
statische Foundationfehler und keine `ok:true`-Projection. Für
alle externen Cleanupfacts bleibt `false -> failed` und `true -> unproven`.
Check 20, Cleanupresultat und die drei Finalisierungsgründe bleiben
unverändert. `settlement-unobservable` schließt den Port weiterhin und
verbietet jeden Folge-Exchange.

### Candidate-PASS- und Testbarkeitsgrenze

Über die unveränderte öffentliche Foundation-API sind im Foundationslice
`candidateObserverGate: PASS` und alle PASS-spezifischen Findings konstruktiv
unerreichbar. Öffentliche Foundationresultate können daher nur
`FAIL/observer-invalid` oder `UNPROVEN/inconclusive` enthalten. Die private
reine Gate-/Findingableitung enthält dennoch die FAIL-Präzedenz, die
UNPROVEN-Präzedenz, `DIVERGED`, alle drei hypothetischen PASS-Findings
`static-rejection-reproduced-after-http200`,
`original-failure-not-reproduced`, `network-signature-diverged` und den
PASS-Fallback `inconclusive`.

Der spätere Implementierungsslice darf diese privaten reinen Funktionen nur
so testen:

1. exakte Produktionsquellbytes in ein betriebssystemseitiges
   Temporärverzeichnis kopieren;
2. an einem exakt einmal passenden lexikalischen Anker ausschließlich
   testlokale Exports der privaten reinen Gate-/Findingfunktionen ergänzen;
3. Funktionskörper, Konstanten, Inputs und produktive Call-Sites unverändert
   lassen;
4. nur die temporäre Kopie seriell importieren;
5. die vollständige Wahrheitstabelle prüfen;
6. die Kopie im `finally` entfernen und die Entfernung bestätigen.

Permanenter `__test`-Export, Environment-/Debugschalter, weitere Produktdatei,
bedingter Export, manipulierte öffentliche PASS-Projection und Nutzung als
Runtimeevidenz bleiben verboten. Kopie und Resultate bleiben stets
`NOT_EVIDENCE`; die unveränderte Original-API muss black-box nachweisen,
dass PASS und PASS-Findings unerreichbar bleiben.

Die späteren Pflichttests müssen getrennt Attach sicher nie gesendet,
Attach-Intent mit unbekanntem Sendestatus, beobachtete Attach-Senderejection,
exakten Attach-Sende-Ack ohne Antwort, malformed, doppelte und
unkorrelierbare Attach-Antwort, gebundene Session mit korreliertem
Detach-Erfolg, gebundene Session mit Detachfehler, Connection-Close ohne
Detach-Erfolg sowie `zero/match` gegen `zero/unproven` abdecken. Kein Fall
darf aus einer fehlenden Session-ID auf eine geschlossene Session schließen.
Dieselbe epistemische Trennung wird symmetrisch für Enable und Disable geprüft.

### Netzwerk-/Regressionsgrenze

`vollständig netzwerkfrei` bezeichnet ausschließlich das neue Foundationmodul,
dessen neue fokussierte Tests und Effects sowie jeden Diagnoselauf dieses
Slices. Die unveränderte vollständige Repositoryregression darf
allein ihre zwei bereits bestehenden kurzlebigen Loopback-Fixtures ausführen:
...
Die Ausnahme erlaubt keine neue Foundation-Netzwerkfixture, keine Änderung
dieser Dateien, keine Foundationtest-Abhängigkeit davon, keine externe
Verbindung oder DNS-Auflösung, keinen manuellen Listener, Browser-, CDP-,
Vite- oder Diagnoseprozess und keinen neuen Childprozess außerhalb der
bestehenden Fixtures. Die bestehenden Cleanup- und
Portfreigabeprüfungen müssen bestehen. Ein späterer Bericht darf deshalb
nur festhalten: Kein neues, externes oder diagnostisches Netzwerk; nur die
zwei unveränderten etablierten Loopback-Regressionsfixtures liefen durch die
vorgeschriebenen bestehenden Suites.

<a id="browser-transport-diagnostic-foundation--adr-0035"></a>

## Angenommene Join- und Transitionstestbarkeitsgrenze / ADR 0035

[ADR 0035](decisions/0035-browser-sync-transport-diagnostic-foundation-join-and-internal-transition-testability-boundary.md)
ist `Angenommen – 2026-09-05` und ersetzt ADR 0034 formal, aber keinen weiteren
ADR. ADR 0034 bleibt mit bytegleichem Hauptteil historische Entscheidungsebene.
Sämtliche nicht ausdrücklich korrigierten ADR-0034-/ADR-0033-/ADR-0032-Regeln
gelten fort. Weder Foundation noch Tests, Adapter oder Runtimevorgang werden
durch die Annahme implementiert oder autorisiert.

Unverändert bleiben `schemaVersion: 1`, die 17 Projection-Rootfelder, genau ein
öffentlicher Export, sieben Intentarten, sechs Protocol Commands, 59
Replayvergleiche, 17 Integritychecks, zehn Stages, zehn Capzustände und 20
Cleanupchecks. Öffentlich erreichbar bleiben ausschließlich
`FAIL/observer-invalid` und `UNPROVEN/inconclusive`; Candidate-`PASS`,
`none-contract-visible-detected`, PASS-spezifische Findings und der
hypothetische PASS-Fallback bleiben konstruktiv unerreichbar. Jede Testkopie
und jedes Testergebnis bleibt `NOT_EVIDENCE`; `overallGate: FAIL` und
`causeStatus: CAUSE_NOT_PROVEN` ändern sich nicht.

### Implementierungsstatus des getrennten Foundation-Slices

Historisch wurde die öffentliche Foundation im ADR-0035-Slice in
`scripts/browser/browserSyncTransportRuntimeDiagnosticObserver.js`
implementiert; sie besaß genau einen Export und blieb importinaktiv. Die
fokussierte Suite in
`tests/browserSyncTransportRuntimeDiagnosticObserver.test.js` bestand mit
422/422 Tests und erzeugte außerhalb ihrer vorgeschriebenen temporären
Sourcekopien keine Dateisystem- oder Hostwirkung. Die unveränderten
Bestandssuites bestanden mit 423/423, 466/466 und 735/735 Tests; die
vollständige serielle Suite mit 2177/2177, exakt `1755 + 422`, jeweils ohne
Fehlschlag, Cancellation, Skip oder Todo. Der Build transformierte
exakt 46 Module, der n8n-Bundlecheck blieb driftfrei und alle historischen
Hashbindungen einschließlich 51-Pfade-/5606-Byte-Frontendmanifest,
Evidence-Record und 4259-Byte-Evaluationstring waren unverändert. Für diesen
Slice wurde kein Browser-, CDP-, Gateway-, Vite-, Netzwerk- oder Diagnoselauf
...
alle Läufe besitzen 0 Fehlschläge, Cancellations, Skips oder Todos. Build und
Bundlecheck bestehen bei exakt 46 Modulen beziehungsweise driftfrei. Der dafür
erforderliche unabhängige Implementierungsreview ist noch ausstehend. Die drei
historischen Reviews werden nicht auf die geänderten Produktions- und
Testbytes übertragen.

### Testkopie v2 und vier private Bindings

Die späteren Tests müssen die Produktionsbytes zunächst byte- und
SHA-identisch in eine pro Testlauf eindeutig benannte
`browserSyncTransportRuntimeDiagnosticObserver.<nonce>.conformance.mjs` in
einem vollständig aufgelösten Betriebssystem-Temporärverzeichnis außerhalb des
Repositorys kopieren. Nur diese Konformitätskopie darf über einen vorher und
nachher genau einmal passenden lexikalischen Anker genau eine zusätzliche
Exportdeklaration mit exakt vier bereits produktiv verwendeten Bindings
erhalten:

| Binding | Arity | geschlossenes Eingabeprofil | Rückgabe | Bindung |
| --- | ---: | --- | --- | --- |
| `deriveCandidateObserverGate` | 1 | `CandidateGateInput = { hardViolation, proofIncomplete }`, beide Boolean | `FAIL\|UNPROVEN\|PASS` | rein |
| `deriveCandidateFinding` | 1 | `CandidateFindingInput` mit Gate, Replayresultat, Stimuluscount, Requestsequenz, Settlementoutcome und statischem Settlementprofil | einer der fünf geschlossenen Findingwerte | rein |
| `createBrowserSyncTransportRuntimeDiagnosticRunMachine` | 1 | `RunMachineInput = { activeExchange, activeObservationClosed, runBinding }` gemäß ADR-0037-Delta | genau die private mutable Run-Machine-Identität | zustandsbehafteter, tatsächlich vom öffentlichen Run-Pfad verwendeter Konstruktor |
| `requestBrowserSyncTransportRuntimeDiagnosticExchange` | 2 | dieselbe Run-Machine-Identität und deren produktiv erzeugtes `ExchangeRequestProfile` | stets `undefined` | einzige zustandsbehaftete Exchange-Request-Grenze aller sieben Intentarten |

`CandidateFindingInput` hat exakt:
...
Die beiden Candidatefunktionen sind rein, halten keine Eingabereferenz und
behalten FAIL- vor UNPROVEN-Präzedenz sowie alle drei hypothetischen
PASS-Findings und den `inconclusive`-Fallback.

Der einzige Run-Owner transferiert gemäß ADR 0037 bereits synchron
`capturedExchange` nach `activeExchange` und `capturedObservationClosed` nach
`activeObservationClosed`, leert beide Factoryslots und gibt nur die beiden
Capabilities sowie die vollständig validierte, defensiv kopierte und tief
eingefrorene interne `runBinding`-Projection an den Konstruktor. Dieser prüft
die Funktionalität beider Capabilities, wiederholt aber die öffentliche
Notification-`length`-Prüfung nicht. Der Konstruktor bindet die eine Machine-Identität für
`activeRunToken === 1` im Zustand `runState === active` samt Owner-Run-Promise
und bereitet nur das erste `capability-probe`-
`nextExchangeRequestProfile` vor. Der öffentliche Run-Pfad und der temporäre
Test starten diese identische Machine danach über dieselbe echte
Exchange-Grenze. Das Originalmodul gibt die Machine nie aus; nur der temporäre
Export des echten Konstruktors macht dieselbe Identität in der
Konformitätskopie testlokal erreichbar.

Das geschlossene `ExchangeRequestProfile` ist noch kein Intent und hat
exakt:
...
Während `observationNotificationState === "invoking"` stoppt der durch ADR 0037
ergänzte zentrale Guard bereits nach Arity- und Maschinenidentitätsprüfung,
vor jeder Profilreflection und vor der folgenden Pending-Join-Behandlung.
Er setzt ausschließlich `observationNotificationViolation`; `O0` und der
Cleanupzustand bleiben während `invoking` unverändert. Erst nach Rückkehr
des Callbacks übernimmt der zentrale Freezer die Verletzung in
`cleanupInitialViolation`. Außerhalb dieses Guards darf nur bei `lease === idle`
die Grenze daraus das frische Intent mit der nächsten ID konstruieren und die
erfasste Capability anwenden. Bei
`lease === observable-pending` muss sie nach Identitätsprüfung der Machine,
aber vor jeder Reflexion oder Verwendung des zweiten Arguments stoppen. Der
zweite Testaufruf verwendet dieselbe Machine und deren referenzidentisches
`activeExchangeRequestProfile`. Ausschließlich das phasengenaue
`pendingInternalExchangeViolation` darf gesetzt werden; Intentkonstruktion,
ID-Reservierung, Count-, Budget-, Sequenz-, Ledger-, Cap-, Observation-,
Snapshot- oder Cleanupmutation sowie Capability-, Port- oder Resolveraufruf
bleiben davor geschlossen.

Ohne fünften Inspector- oder Debugexport darf die Konformitätsprüfung nur
die bereits produktiv notwendigen Machineblätter, das gebundene
Owner-Run-Promise und ihre selbst kontrollierten Capabilityaufrufe beobachten.
Das umfasst insbesondere Lease, aktives Requestprofil, `nextIntentId`, Intent-,
Port-, Send-, Capability-, Current-Exchange- und Run-Settlementcount,
`pendingInternalExchangeViolation`, `O0`, Cleanup-Ledger, Capzustände und
Cleanupabschluss. ADR 0037 ergänzt ausschließlich die produktiven Werte
`activeObservationClosed`, `observationNotificationState` und
`observationNotificationViolation`. Das ist keine öffentliche Inspection-API.

Die unveränderte Konformitätskopie und jede mutationswirksame Mutantenkopie
sind disjunkte Kopietypen. Jeder Mutant wird als eigene `.mjs` unmittelbar
aus denselben verifizierten Produktionsbytes erzeugt und darf zusätzlich zur
gleichen Exportdeklaration genau eine benannte Mutation tragen. Alle Kopien
werden seriell über ihre aufgelöste `file:`-URL importiert, im `finally`
entfernt und anschließend auf vollständige Nichtexistenz geprüft. Permanenter
`__test`-Export, zusätzliche Produktfunktion, zweiter Sourcepfad, Flag,
Factoryoption, produktive Callsiteänderung, öffentliche State-Inspection oder
PASS-Projection bleiben verboten.

### Drei-Phasen-Join und Settlement

Die spätere dynamische Matrix muss dieselbe produktive Machine über normale
Effects-as-Data-Übergänge getrennt bis zu genau einer
`observable-pending`-Lease in jeder Phase treiben:
...
Der finite Join-Nachweis ist exakt das kartesische Produkt:

```text
phase = prestart | observation | cleanup
outcome = fulfillment | rejection
timing = pre-invocation | synchronous-post-invocation | observable-pending
Pflichtfälle = 3 × 2 × 3 = 18
```

`pre-invocation` bedeutet, dass `activeExchange` ein bereits gesetteltes
gültiges lokales natives Promise zurückgibt. Bei
`synchronous-post-invocation` gibt `activeExchange` ein pending Promise zurück, das der Test
unmittelbar nach dem synchronen Rücksprung der ersten Grenze settelt. Bei
`observable-pending` bleibt das Promise durch den zweiten Boundaryaufruf
hindurch pending und wird erst danach gesettelt. In den ersten beiden Fällen
ist der kontrollierte Handler nur als Microtask eingeplant, aber noch nicht
eingetreten.

Unmittelbar vor dem zweiten Boundaryaufruf wird in jeder der 18 Zellen ein
Baselinesnapshot aller produktiven Testzähler und beobachteten
Capabilitywirkungen erfasst. Zwischen dem synchronen Rücksprung des ersten und
dem zweiten Boundaryaufruf sind `await`, Microtask-Checkpoint, Timer und jedes
andere Yield verboten. Unmittelbar danach und noch vor Handlerzutritt müssen
gelten:
...
Die Deltas sind erforderlich, weil Observation und Cleanup bereits frühere
Exchanges enthalten können. Die zweite Anforderung setzt nur
`pendingInternalExchangeViolation = prestart|observation|cleanup` und erzeugt
weder Intent, ID, Capability noch Promise.

Fulfillment und Rejection des ursprünglichen Exchange-Promises müssen in allen
18 Kombinationen getrennt getestet werden. Das bloße Promise-Settlement ist
vor dem Handlerzutritt kein Maschinenereignis; Lease und aktueller Exchange bleiben
bis dahin `observable-pending` und exakt eins. Fulfillmentpayload und
Rejectiongrund bleiben vollständig ungelesen. Nach dem ersten kontrollierten
Handler gilt atomar:
...
Alle noch lebenden Caps werden `terminal-unknown`; Cancel, Retry,
Ersatz-Exchange und weiterer Portzugriff bleiben verboten.

| Phase | Folge des Fulfillment- oder Rejection-Joins |
| --- | --- |
| `prestart` | statischer Foundationfehler; kein `O0`; kein Cleanup-Ledger; Notification verworfen, null Aufrufe |
| `observation` | sticky `V`; `O0` frozen; danach genau eine Notification trotz geschlossenem Exchange-Port; erst danach lokaler portloser Cleanup |
| `cleanup` | `O0` unverändert; keine zweite Notification; `cleanupViolation: true`; verbleibende Portchecks `unproven`; `cleanupFinalizeReason: cleanup-terminal-failure` |

Späte oder doppelte Handler bleiben run-token-, lease- und zustandsgegatete
`undefined`-No-ops.

### Zweiteiliges Forever-pending-Oracle und Mutationspflicht

Diese Probe ist von den 18 settelnden Fällen getrennt und keine vierte
Settlementzeitlage. Pro Phase bleibt das erste gültige lokale native Promise
für die vollständige dynamische Probe ungesettelt. Nach dem
unterdrückten zweiten
Boundaryaufruf werden die Deltas sofort, dann nach exakt drei einzeln gezählten
testlokalen Microtask-Checkpoints erneut geprüft. Es müssen weiterhin genau
ein aktueller Exchange, null Owner-Run-Settlements, kein künstliches `O0`, kein
vorzeitiger Cleanup, keine Terminalmaterialisierung, keine weitere Capability
und keine neue Intent-ID gelten. Eine zusätzliche Probe lässt das Promise bis
zum Testende pending und wartet den Owner-Run bewusst nicht ab. Reale Uhr,
Timer, Timeout, produktives `Promise.race`, Polling und Zusatzcap sind
verboten.

Dieser endliche Präfix ist allein kein Unendlichkeitsbeweis. Zusätzlich muss
die vollständige private Transitionstabelle zeigen, dass aus
`observable-pending` ohne kontrollierten Fulfillment- oder Rejectionhandler
kein Fortschritts-, Snapshot-, Cleanup-, Terminalisierungs-, Resolve- oder
`idle`-Pfad existiert. Zulässig sind nur: ohne Handler unverändertes Pending;
zweiter Request mit ausschließlich phasengenauer Joinklassifikation;
kontrollierter Handler ohne Join mit normalem Dispatch; kontrollierter Handler
nach Join mit atomarem Closed-Tupel; und im bereits geschlossenen Zustand
inertes `undefined`. Timerersatz, zweiter Port, konkurrierender Resolver, Retry,
Polling oder zweite Exchange-Capability existieren nicht. Die getrennte
ADR-0037-Notification erzeugt keinen Fortschritt aus einem vor `O0` pending
bleibenden Exchange; im Cleanup ist sie bereits konsumiert. Nur Präfix und Tabelle
zusammen dürfen als Forever-pending-Nachweis gelten; eine empirisch beobachtete
unendliche Zeit darf nie behauptet werden.

Gesonderte Mutantenkopien müssen mindestens einen entfernten oder umgangenen
Guard, Guard nach Intentkonstruktion, ID- oder Count-/Ledgermutation, zweiten
Port-/Capabilityaufruf, Leasefreigabe zu `idle`, künstliches Owner-Settlement,
vorzeitiges `O0` oder Cleanup, verlorene Phasenklassifikation und Mutation des
bereits Frozen `O0` im Cleanupfall töten. Der Produktionssource und die
Konformitätskopie werden niemals mutiert.

### Drei öffentliche Ableitungsregressionen

Die Run-Machine führt zusätzlich zu `firstEndpointRequestTimestamp` privat
`lastValidBrowserNetworkTimestamp`. Der erste eindeutig attribuierte
Endpoint-Request initialisiert beide Werte. Jeder weitere eindeutig
attribuierte Networkkandidat muss vor Stage-, Count-, Timing- oder
Sequenzmutation einen primitiven endlichen nichtnegativen Timestamp, sichere
absolute und relative Millisekundenumrechnung sowie
`timestamp >= lastValidBrowserNetworkTimestamp` erfüllen. Gleichheit ist
zulässig; erst danach wird der letzte gültige Timestamp aktualisiert. Timing
bleibt ausschließlich relativ zum ersten Endpoint-Request. `10 -> 12 -> 11`
und `10 -> 9` sind Rückläufe; `NaN`, Infinity und unsicherer
Millisekundenüberlauf sind ebenfalls Verletzungen. Vor `O0` ergeben sie sticky
`V`, `confirmed-violation`, `FAIL/observer-invalid` und `truncated`, ohne den
letzten Timestamp, Stage, Count, Timing oder Sequenz fortzuschreiben. Nach
`O0` darf ein Networkevent die Observation nicht ändern.

Für `Network.responseReceived` ist nach dem absoluten Cap-/Dequeueguard die
Konsumfolge exakt: Event-`method`, Event-`sessionId`, Event-`params`,
`params.requestId`, `params.response`, `response.url`, Endpointvergleich und
Request-ID-Korrelation. Bei exaktem Endpoint und unbekannter oder ungebundener
Request-ID wird sofort gestoppt; `params.timestamp`, `response.status`,
Header-, Body- und sonstige Responsewerte bleiben ungelesen. Die
Attributionsunsicherheit wird sticky `ambiguous/unproven`, ohne unabhängigen
Grund aber nicht `V`; Requestsequenz bleibt `ambiguous`, Produktevidenz
`false`, Candidate `UNPROVEN/inconclusive`. Der nach Sequenzzustand betroffene
Stage-4- oder Stage-6-Slot bleibt ohne korrelierte Response
`not-observed/unproven` mit null Order und Timing. Eine spätere korrelierte
Response darf ihren eigenen Slot `observed/match` setzen, die sticky
Attributionsunsicherheit und mehrdeutige Sequenz aber nicht heilen. Es gibt
weder zusätzlichen Requestcount noch erfundene Request-ID. Malformed Routing
behält seine bestehende `V`-Ableitung.

Für eine doppelte `Target.getTargets`-Antwort bleibt bei exakt einem gültigen
Send-Ack die Protocol Operation ausnahmslos `one/match`. Eine Antwortdublette
ist nie zweites Intent oder Ack und ändert den Send-Ledger nicht. Eine
wohlgeformte Dublette vor Evaluate ergibt `U/setup-terminal-unproven`,
`targetProfile: unknown`, `singleTargetAndSessionConfirmed: unproven`,
Evaluate-Intent `zero`, Evaluate-Send `zero/match`, `not-started` und
`UNPROVEN/inconclusive`; Stage 1 bleibt `observed/match`, Stages 2 bis 8
`not-observed/unproven`. Während Capture bleiben Operation `one/match`,
Targetprofil `unknown`, Antwortprofil `multiple/ambiguous` und Candidate bis
`C` unbewiesen. Eine eindeutig routbare malformed Dublette vor `O0` bleibt
`V/FAIL/observer-invalid`; nach `O0` ändert sie die Observation nicht.

Diese drei Regressionen müssen ausschließlich black-box über den normalen
öffentlichen Effects-as-Data-Port geprüft werden und benötigen keinen privaten
Export. Damit ist die historische `one/unproven`-Antwortregel ausdrücklich
durch die ADR-0034-Operationsmatrix überholt: Antwortdubletten ändern niemals
einen bestätigten Send-Ack oder dessen Operationsergebnis.

ADR 0036 ist als ausschließlich dokumentarische Adaptergrenze vorgeschlagen.
...
geprüft. Der unabhängige Implementierungsreview und der danach getrennte
Abgleich von ADR 0036 gegen neue Load-/Rohhashbindungen und Testverträge stehen
aus; deshalb ist ADR 0036 ausdrücklich nicht annahmereif.
Adapterimplementierung, Adaptertests,
sichtbarer Diagnoselauf, Browserkomposition und Browser-End-to-End bleiben
geschlossen.

<a id="browser-sync-transport-runtime-diagnostic-adapter--adr-0036"></a>

## Vorgeschlagener Runtime Diagnostic Adaptervertrag / ADR 0036

[ADR 0036](decisions/0036-browser-sync-transport-runtime-diagnostic-adapter-boundary.md)
ergänzt ADR 0035 und ersetzt keinen ADR. Sein Status ist
`Vorgeschlagen – 2026-09-06`. Er implementiert und autorisiert nichts.

Der künftige Adapterpfad und seine einzige öffentliche API sind:

```text
scripts/browser/browserSyncTransportRuntimeDiagnosticAdapter.js

createBrowserSyncTransportRuntimeDiagnosticAdapter() -> frozen { run }
Factory.length = 0
run.length = 0
```

Import und Factory bleiben inaktiv; jeder erste `run`-Aufruf latcht den
One-shot-Owner vor der Argumentauswertung. Nur der argumentlose Erstaufruf darf
wirken; ein argumentbehafteter Erstaufruf verbraucht den Owner ohne Wirkung und
rejected statisch. Es existieren keine callerlieferbaren Optionen,
Capabilities, `runBinding`-, Gate-, Finding-, Digest-, Provenienz-, Cleanup-
oder Writerwerte. K2 verwendet genau zwei disjunkte, niemals kombinierte
Kopieprofile aus denselben vor Instrumentierung längen- und hashgeprüften
Produktionsbytes:

```text
derivation-conformance (exakt vier Exports)
  createBrowserSyncTransportRuntimeDiagnosticAdapter
  deriveBrowserSyncTransportRuntimeDiagnosticRecordGate       // length 1
  deriveBrowserSyncTransportRuntimeDiagnosticRecordFinding    // length 1
  finalizeBrowserSyncTransportRuntimeDiagnosticRecord         // length 1

virtual-runtime-conformance (exakt vier Exports)
  createBrowserSyncTransportRuntimeDiagnosticAdapter
  createBrowserSyncTransportRuntimeDiagnosticAdapterOwner     // length 1
  enqueueBrowserSyncTransportRuntimeDiagnosticAdapterEvent    // length 2
  installBrowserSyncTransportRuntimeDiagnosticVirtualCapabilities // length 1
```

Der produktive Owner akzeptiert nur das vollständige tief eingefrorene Profil
`adr-0036-runtime-capabilities-v1`, erzeugt ohne Capabilityaufruf dieselbe
`{run}`-API und wird von der öffentlichen Factory tatsächlich verwendet. Die
zweistellige Eventgrenze ist der einzige produktive Eingang aller rohen Pipe-,
Timer-, Child- und Ressourcenrückläufe in den Ownerdispatcher. Erst danach
führen der produktive Parser beziehungsweise die Timer-/Child-/
Ressourcenübergänge erlaubte Werte in FIFO oder Ledger; der Parser ist kein
externer Producer. Nur die instrumentierte Kopie besitzt den
einmaligen Installer; sein nicht exportierter Arity-0-Consumer ersetzt exakt
den einmaligen produktiven Capabilityselector. Virtuelle Fähigkeiten liefern
nur niedrigstufige Rohwerte, niemals IDs, Replayoperanden, Digests,
Effect-Acks, Checks, Gate, Finding oder Record.

`derivation-conformance` fügt am einmaligen
`ADR-0036-ADAPTER-TESTCOPY-EXPORT-ANCHOR-V1` genau seine private Arity-0-Sperre
`rejectBrowserSyncTransportRuntimeDiagnosticDerivationCapabilities` und die
feste Exportdeklaration ein. Die Sperre wirft ausschließlich synchron den
frischen statisch redigierten
`TypeError("browserSyncTransportRuntimeDiagnosticAdapterFailed")`. Dieses
Profil ersetzt sowohl `const adapterEvidenceEligible = true` durch `false` als
auch exakt die einmalige Selector-RHS von der realen Nodefactory auf die
Sperre. Deshalb erzeugt selbst ein versehentlicher öffentlicher Factoryaufruf
weder Owner, API noch Promise und erreicht vor dem Throw keine Entropie-,
Clock-, Environment-, Datei-, Timer-, Pipe-, Prozess- oder andere Hostquelle;
die drei reinen Ableitungen bleiben erreichbar.

`virtual-runtime-conformance` fügt am Anchor genau einen festen Block aus
kopielokalem Slot, Installer, nicht exportiertem Consumer und seiner festen
Exportdeklaration ein. Es ersetzt dieselbe Evidencekonstante durch `false` und
dieselbe Selector-RHS ausschließlich durch den Consumer. Beide
Selectorvarianten bleiben disjunkt; `adapterEvidenceEligible:false` ist nur
Evidenzdemotion und sperrt den virtuellen produktiven Owner nicht. Null-,
Mehrfachtreffer oder weitere Byteabweichung brechen vor Import ab. Jede Kopie
liegt außerhalb des Repositorys, wird seriell als `.mjs` importiert und im
`finally` samt Temproot nachweislich entfernt. Reine Ableitung darf
hypothetisches `PASS` prüfen; Testfinalisierung ergibt bei bestätigter
Verletzung `FAIL`, sonst höchstens `UNPROVEN`, immer
`evidenceStatus:NOT_EVIDENCE`, `runtimeRecord:null` und Writercount null. Die
reale Capabilityfactory ist in beiden Profilen unerreichbar. Die spätere
Gegenprobe `derivation-no-host-selector-poison` ersetzt im Ableitungsprofil
genau diese RHS durch einen inline benannten nullstelligen Sentinelthrow; sie
muss einen entfernten Schutz vor tatsächlichem I/O töten. Eine
Sourcestrukturprüfung verwirft jede Rückkehr zur realen Nodefactory. Die bestehende
ADR-0035-Foundationkopie mit vier privaten und insgesamt fünf Exports bleibt
unverändert und wird nie kombiniert.

Das Capabilityroot besitzt exakt und in Reihenfolge
`{profile,entropy,clock,runtime,scheduler,pipe,launcher,resources}`. `entropy`
hält die zwei Arity-0-Funktionen
`readDiagnosticRunIdEntropyBytes -> Uint8Array(17)` und
`readReplayContextIdEntropyBytes -> Uint8Array(15)`; real entsprechen sie zwei
unabhängigen einmaligen `crypto.randomBytes`-Reads, deren Bufferresultate sofort
in gewöhnliche frische `Uint8Array`-Views normalisiert und verworfen werden.
`clock` hält
`readControllerNanoseconds`, `readWallMilliseconds` und `readTimeZone`.
`runtime` hält exakt `readProcessPlatform`, `readProcessArchitecture`,
`readProcessVersion`, `readProcessExecutablePath`,
`readProcessExecArguments`, `readProcessEnvironmentMatches(name)` und
`readWorkingDirectory`. Die zwölf case-insensitiven Environmentabfragen sind
auf `NODE_OPTIONS` plus die elf Child-Allowlistnamen begrenzt. Pro Abfrage
dürfen höchstens 256 begrenzte Own-Namen enumeriert, aber nur Werte passender
Namen gelesen werden; fremde Werte und das gesamte Environment bleiben
unlesbar. Die R0-Reihenfolge, String-/Arraycaps und statischen
Fehler stehen vollständig in ADR 0036 Abschnitt 1; wiederholte Pfad-/Handle-/
Bytesprüfungen sind getrennte Ressourcenoperationen und keine zweiten
R0-Messungen.

`scheduler` hält `armTimer(callback,milliseconds)` und `cancelTimer(handle)`;
`pipe` hält `openDebugPipe(childHandle,producerSink)`,
`writeDebugPipe(pair,bytes,completionSink)` und `closeDebugPipe(pair)`;
`launcher` hält `spawnChild(profile,producerSink)`, `terminateChild(handle)`
und `closeChild(handle)`; `resources` hält
`performResourceOperation(operation,producerSink)` und
`closeResource(handle,producerSink)`. Spawnprofile sowie die zehn geschlossenen
Ressourcenvarianten für Canonicalize, Path-/Open-Identity, Open, Read,
Directoryliste, exklusives Temp-/Directory-/File-Create und passive
TCP-Listener besitzen die in ADR 0036 festgelegten exakten Shapes, Caps und
Rohresultate. Keine Teilcapability, Calleroption oder Environmentauswahl ist
zulässig; direkter Hostfallback außerhalb dieser Gruppen ist verboten.

Capabilityroot und -container werden tief eingefroren;
Ressourcenoperationsroot und gewöhnliche Inputcontainer selektiv bis vor
Typed-Array- und Handleblättern. Ein Entropieresultat wird unmittelbar nach
Capabilityrückkehr, jedes Pipe-/Child-/Ressourcenbyte synchron beim
Sinkeintritt defensiv kopiert und danach freigegeben. Outbound-Versuchviews
liegen bis Completion oder Pairclose als read-only Loan bei der
Pipecapability; der Adapter hält währenddessen keinen mutable Alias. Timer-,
Pipe-, Child-, Ressourcen- und Operationhandles bleiben undurchsichtige
identitäts- und generationsgebundene Blätter und werden nie reflektiert,
gefreezt oder ausgegeben. `ProducerEventProfile` ist exakt
`{profile,producer,producerHandle,generation,event}` und geschlossen auf
Scheduler-Fire, Pipe-Chunk/EOF/Readerror/Drain/Writeerror/Writecompletion,
Child-stdout/-stderr/-exit/-close/-error und Ressourcencompletion. Owner,
Handle und Generation werden vor Payloadreflection geprüft; ein fremder Owner
rejected statisch ohne Mutation, unbekannte Handles oder zukünftige
Generationen verletzen, bekannte stale/späte/doppelte Terminalcallbacks bleiben
inert, malformed aktive Ereignisse führen statisch redigiert in Cleanup.
Ownergebundene Timer-, Raw- und Completion-Closures dürfen erst nach
Capabilityrückkehr und Handle-/Tokenbindung eintreten; ein früher synchroner
Callback oder Sink verletzt, ohne ein Producerevent zu erzeugen. Alle drei
Closurearten geben auch bei intern gelatchtem Fehler nur `undefined` zurück.
Streamseitiges Pipe-Writeerror bleibt
vom writegebundenen Completion-Sink unabhängig; eine zweite Completion
derselben Writegeneration ist stale.

Die tief eingefrorene Fixture-API ist exakt `{dispatch,snapshot}` mit Arity
`1/0`. `dispatch` akzeptiert nur geskriptete Entropie-/Clock-/Runtimewerte oder
-throws, den nächsten effectful Capabilitythrow, synchrone Pipewritergebnisse
einschließlich Partial-/Backpressurefällen sowie timer-, pipe-, child- und
ressourcenbezogene Rawereignisse über fixturelokale Ordinale. Resource-opened
und -created minten Handles ausschließlich fixtureintern; Caller liefern sie
nicht. Je Pipe ist höchstens ein synchrones Writergebnis und eine aktive
Writegeneration erlaubt; Completion adressiert nur über das Pipeordinal den
gebundenen aktiven Write. Ein Writergebnis und ein Failurearm für
`writeDebugPipe` sind gegenseitig ausgeschlossen; ein fehlendes oder zweites
Script scheitert als Fixturezustandsfehler. Die Fixture akzeptiert weder
`ProducerEventProfile` noch
Foundationwerte. `snapshot` liefert nur die
geschlossene primitive Zähler-/Ordinalprojektion
`{profile,dispatchCount,pendingSourceResultCount,
pendingCapabilityFailureArmCount,pendingPipeWriteResultCount,callCounts,
liveOrdinals,producerTurnCount,fixtureOwnedByteLength}`, nie Handles, Bytes, Pfade, Acks oder
Checks. `timer-fire` adressiert die Timerclosure, Writecompletion den
Completion-Sink und die übrigen asynchronen Varianten ihren Raw-Sink. Nur
erfolgreich übernommene Dispatches zählen; Ordinale werden erst an
produktiven Erfolgsübergängen live und an korrelierten Terminalübergängen
einschließlich Timercancel entfernt, Producerturns erst nach Raw-Sink-Rückkehr
gezählt und gehaltene Bytes
nach Capability- beziehungsweise Sinkrückkehr ausgebucht. R0-Quellen gehen
unmittelbar in defensive Ownerprojektion und lokale Ableitung. Der effectful
Pfad bleibt: Capabilityrückkehr und Handle-/Tokenbindung -> spätere Timer-,
Raw- oder Completion-Closure -> produktiver `ProducerEventProfile` ->
Owner-/Generationsguard -> Parser oder Timer-/Child-/Ressourcenübergang ->
Ledger; nur ein erlaubter Foundation-Dequeuewert erreicht danach die einzige
FIFO und `effectPort.exchange`.

Base32, ISO-Rückprojektion, SHA-256, UTF-8/Scanner/JSON, Git-Object-/Packdecode,
Pfad-/URL-Ableitung und `vm.SourceTextModule` sind ausschließlich lokale
deterministische Verarbeitung bereits gebundener Bytes oder Primitive. Sie
öffnen keine Hostquelle; `process.*`, Environment, Entropie, Clock,
Filesystem, Timer, Pipe, Child und passive Portquelle sind nur über das
ausgewählte Capabilityprofil erreichbar.

Die Foundation wird aus einem einmalig gelesenen, fatal UTF-8-dekodierten und
gegen Commit-Blob sowie festen SHA-256 geprüften Bytesnapshot byte-owned als
`vm.SourceTextModule` mit kanonischer File-URL, null Imports und genau einem
Export geladen. Standardimport plus Vorher-/Nachherhash ist wegen TOCTOU/ABA
kein gültiger Identitätsnachweis. Adapterquelle und Foundationpfad werden
realpath-, Parent-, File-/Volumen-ID-, Symlink-, Junction- und
reparsegebunden; ein Pfadtausch demotiert oder verletzt.
Der Adapterprozess verlangt exakt `--experimental-vm-modules` und danach
`--no-warnings`. Letzteres verhindert den standardmäßigen VM-Warnwrite, ist
aber kein Stillenachweis; ohne unabhängigen Owner von Adapter-stdout/-stderr
bleibt `observerDiagnosticDuringRunOutputAbsent` `unproven`.

Der heutige `effectPort.exchange(intent)` bleibt total für exakt die bestehenden
...
422/422-Suite. Im vorgeschlagenen Adaptervertrag erhält jeder Exchange ein adaptereigenes natives
...
vollständige lokale Annahme eines einzelnen NUL-Frames durch
`Writable.write`, nicht Callback, Flush oder Browserempfang. Cleanup-Ack
bedeutet nur Annahme; das Resultat folgt separat als FIFO-`cleanup-fact`.
Der dreistufige Adapterzustand wird total abgebildet: `failed -> false`,
`confirmed|unproven -> true`. `true` bleibt in der Foundation stets
`unproven`; nur der terminale Adapterledger darf den Finalrecord später auf
`confirmed` setzen. Nach verarbeitetem Cleanupcap bleibt ein spätes Fact inert.

Der Windows-Pipeparser verwendet allein `--remote-debugging-pipe`, die
erbbaren Handles 3/4, einen NUL-Akkumulator, fatalen UTF-8-Decode, einen
generischen lexikalischen Duplicate-Key-Scanner einschließlich escape-
äquivalenter Namen und danach exakt einen nativen `JSON.parse` ohne Reviver.
Die Caps sind:

```text
Readchunk                   65_536 Bytes
Frame                      262_144 Bytes
Akkumulator                262_145 Bytes
Text                       262_144 UTF-16-Codeunits
JSON-Tiefe                      32
JSON-Knoten                  4_096
Member                       8_192
...
Inboundmessages                512
stdout/stderr              je 65_536 Bytes Inspektionscap
Outboundframe inkl. NUL         65_536 Bytes
ausstehende Outboundframes           1
ausstehende Outboundbytes       65_536 Bytes
```

Alle CDP-Messages, Capereignisse, Connection-Close- und Cleanupfacts teilen
eine einzige controllerlokale FIFO. Es gibt keine Vorfilterung,
Deduplizierung oder Umordnung. Evaluate-Frameannahme, Aktivierung des pending
Capturearms und Ack bilden einen nicht reentranten Commit. Die drei
Clockdomänen bleiben getrennt; Setup/Capture/Cleanup bleiben exakt
6.000/6.000/60.000 ms. `observation-dequeue` liest keine Clock, sondern liefert
nur den ältesten Envelope. Die Foundation hält ihn unreflektiert und fordert
danach ihren phasengenauen `controller-clock-sample`; erst dessen Bearbeitung
liest exakt einmal und speist denselben primitiven Wert in Adapterledger und
Foundationresultat. Setup und Cleanup verwenden inklusiv
`sample >= deadline`; Capture schließt ausschließlich durch das korrelierte
FIFO-`cap-fired`, niemals numerisch. Der 129. Dequeue-Aufruf rejected vor
Entnahme und Clockread. Jeder Arm hat höchstens einen Cancel und ein `fired`
wird nie `cancelled`.

Auf dem Wire werden ausschließlich die sechs bestehenden Commands als
`{id,method,params}` beziehungsweise bei gebundener Session exakt als
`{id,method,params,sessionId}` materialisiert. `params` ist bei jedem Command
vorhanden; `sessionId` ist, falls erlaubt, stets das vierte Rootfeld.
Foundation-`commandId` wird zu `id`, `command` zu `method`.
`captureArmIntentId` ist rein controllerlokal und erscheint nie auf dem Wire.
Exakt ein `JSON.stringify`, UTF-8-Encoding und ein abschließendes NUL-Byte
erzeugen den einzelnen Frame.

Der Adapter baut die neun `runBinding`-Felder und alle 59 Replayoperanden aus
gebundenen Quellen selbst. Eindeutig gemessen wird `observed`, fehlend
`not-observed/null`, widersprüchlich `ambiguous/null`. Insbesondere bleiben
`repository.state`, Windowsarchitektur/-edition/-version/-build/-patch,
Chromeprodukt/-channel/-version/-engine/-engineBuild und die vor der Factory
noch nicht tatsächlich beobachtbaren Kontextoperanden 37 bis 40
`not-observed/null`. Globale Portfreiheit und effektive Proxy-/VPN-/Policy-/
Extension-/Permission-/Service-Worker-/Cachewerte bleiben ohne eigene
authentische Quelle ebenfalls unbewiesen. Es gibt keinen siebten CDP-Command
und keinen Probe-Listener.

Der an Own-Data und Gatewayspawn gebundene rohe
`GOLDENDAWN_SYNC_GATEWAY_PORT`-Wert ist exakt der Vier-Codeunit-String `8787`
mit `[56,55,56,55]` und enthält keine Quotezeichen. Ausschließlich Operand 53
erhält danach die separate `adr-0034-I3-contract-projection`: U+0022, dieselben
vier Codeunits, U+0022, also Länge 6 und
`[34,56,55,56,55,34]`. Trimmen, Typkonvertierung, Double-Wrap,
Child-Environment-Quotes und Normalisierung einer geänderten Ziffer sind
verboten; Abweichung stoppt vor Spawn oder bleibt `mismatch/DIVERGED`.

Node Core besitzt in dieser Grenze weder einen handlegebundenen Windows-Job-
Object-/Prozessbaumowner noch eine handle-relative, nicht folgende
Deleteprimitive. Nach einem möglichen Browser-, Vite- oder Gatewayspawn
bestätigt deshalb ein Root-Childexit keinen vollständigen Stop; nach einem
möglichen Profil-/Fragmentcreate ist pfadbasiertes Löschen verboten. Die
betroffenen Checks bleiben `unproven`, sofern nicht ein sicher nie versuchtes
Create/Spawn `confirmed` oder ein authentisch aktiver Rest `failed` belegt.
Der Setup- beziehungsweise Cleanupcap behält Präzedenz vor jeder
Envelope-Reflection, sobald das erst im nachgelagerten Foundation-
`controller-clock-sample` einmalig gelesene `m_answer >= deadline` ist.

Der spätere Adaptertest belegt diese Integration nur über
Raw-`Uint8Array` -> produktiven Parser -> gewöhnlichen gefrorenen Own-Data-Graph
-> FIFO, mit Setup und Cleanup jeweils an `deadline-1`, `deadline` und
`deadline+1`; er prüft null Clockreads im Dequeue, das eine gemeinsame
Folgesample und Materialfreigabe. Capture bleibt ausschließlich
`cap-fired`-ereignisbasiert. Getter-/Proxy-Envelopes entstehen aus diesem
unveränderten JSON-Pfad nicht. Der historische Foundationtest
`entscheidet Setup- und Cleanup-Caps unterhalb, exakt und oberhalb roh vor
Reflection` belegte bei `=`/`>` null Aufrufe eines gewöhnlichen Accessorgetters,
nicht null Descriptor- oder Proxytraps. Die damaligen Proxytests am
Pending-Join und am 129.-Dequeue-Guard betrafen andere Grenzen. Der
ADR-0037-Implementierungsslice schließt diese konkrete Lücke auf Foundationebene:
sechs Baselines prüfen Setup und Cleanup jeweils unterhalb, exakt und oberhalb
der Deadline mit vier getrennten Trapzählern. Bei `=`/`>` bleiben `get`,
`getPrototypeOf`, `ownKeys` und `getOwnPropertyDescriptor` jeweils null;
unterhalb belegt eine positive Kontrolle die Sensitivität. Vier isolierte
Mutanten, je Phase `>=` zu `>` sowie Reflection vor Deadlineguard, werden
kausal getötet. Damit wird kein ausgeführter Adapter-Wiringnachweis behauptet;
dieser bleibt separat ausstehend.

Ein einzelnes Cancelpayload und eine spätere
`confirmed-violation`-Projection tragen keine `O0`-Phasenbindung. Die
vollständige öffentliche Intent-, Exchange-, Producer- und
Foundationresultathistorie enthält zusätzliche geordnete Fakten; aus gleichen
Payloads oder Endprojektionen folgt daher weder ihre Injektivität noch ihre
Nicht-Injektivität, und beides wird nicht behauptet. Der beschlossene bounded
syntaktische Tracker darf keine CDP-Semantik spiegeln, Projection rückdatieren
oder aus fehlenden Intents Positives ableiten. Unter genau dieser Grenze kann
er nicht für jeden Pfad einen gespeicherten Cancelcheckpoint als authentischen
Zeitpunkt unmittelbar nach `O0` und vor Cleanup auswählen. Auch
`cleanup-origin` ist zu spät, weil die Foundation zuvor bereits ihr
Cleanup-Ledger initialisiert. Ein Promisezaun läuft nach der atomaren
Foundationreaktion und damit nach möglicher Old-Cap-Wirkung oder portlosem
Foundationcleanup; er darf nichts rückdatieren.

Der unter den bestehenden Architekturgrenzen als minimale eindeutige
Phasenbindung gewählte Vertrag `D_K4` ist als Erweiterung des Foundationports
um genau `observationClosed.length === 0` implementiert. Die Foundation
erfasst die Identität bei Factorykonstruktion und ruft sie bei erreichtem `O0`
pro Lauf genau einmal mit null Argumenten synchron unmittelbar nach dessen
tiefem Freeze und Zuweisung auf, aber vor Phasenwechsel, Old-Cap-Scan und jeder
regulären oder portlosen Cleanupwirkung. Ohne `O0` erfolgt kein Aufruf. Die
künftige Adapterfunktion soll `A_obs` vor ihrer Rückgabe `undefined` einfrieren;
dieser Adapterzustand ist weiterhin nicht implementiert. Sie trägt weder
Purpose noch Closegrund und ist kein achter Intent, Command, Promise, Timer,
Clockread, Request oder Recordfeld. Die Auswahl behauptet weder einen
Injektivitätsbeweis noch die informationstheoretische Notwendigkeit dieses
Designs.

Der gebundene Adaptertracker ist exakt
`PREATTEMPT -> OBSERVATION_NO_AOBS -> AOBS_BOUND -> CLEANUP ->
TERMINAL_ELIGIBLE|TERMINAL_NO_RECORD`. Ein früher, fehlender, reentranter,
doppelter oder verspäteter Marker ist bestätigte Verletzung/no-record; ein
früheres authentisches `A_obs` wird bei portlosem Cleanupfehler nie ersetzt.
Die sieben Klassen folgen der tatsächlichen Foundationreihenfolge: Setup-ready-
und Rejection-Quieszenz-Cancel liegen vor `O0`; Old-Cap scannt nach `O0`
zuerst Setup, dann Capture; regulärer Cleanup initialisiert sein Ledger vor
...
und geprüft; ihr unabhängiger Daybreak-xhigh-Implementierungsreview steht aus.
Adapterabhängige positive Claims bleiben `unproven`, und ohne früheres
authentisches `A_obs` ist ein Record verboten. Erst nach dem getrennten Review
und Jans manuellem Featurecommit folgt der gesonderte ADR-0036-Abgleich gegen
die neuen Foundation-, Load-/Rohhash- und Testverträge samt erneutem
unabhängigem Dokumentreview. ADR 0036 bleibt bis dahin nicht annahmereif;
seine bisherigen Rohhashbindungen bleiben in diesem Slice unverändert.

Nach terminalem Cleanup wird ein neuer referenzdisjunkter Record mit exakt den
bestehenden 17 Rootfeldern materialisiert. `projectionType` wird zu
`recordType`; Candidate-Gate/Finding werden als echtes Gate/Finding neu
abgeleitet. `observer.controllerExclusivity`, `connectionProfile`,
`foundationSha256`, alle 17 Integritywerte und deren
`interferenceObservation` sowie alle 20 Cleanupwerte, `cleanup.result` und die
cleanupseitigen Completionwerte werden aus dem identitätsgebundenen finalen
Ledger frisch neu berechnet. Dasselbe gilt für `evaluationSha256`, alle sechs
`protocolOperations` mit Foundation-/Wire-Kreuzvalidierung und die neun
Requestbudgetcounter plus Sequenz aus Capability-, Launch-, Main-World- und
Networkledger. Konstruktive Foundationplatzhalter werden nicht kopiert.
`FAIL > UNPROVEN > PASS`; Foundation-`FAIL` bleibt sticky. Bestätigte
Foundation- oder Adapterverletzung ergibt zuerst `FAIL/observer-invalid`, bei
jedem Stimuluszustand `zero|unknown|multiple|one`. Nur ohne Verletzung führen
fehlende Evidenzfähigkeit oder ein unbewiesenes Blatt zu
`UNPROVEN/inconclusive`; erst danach ist `PASS` möglich. Exakt ein Stimulus ist
ausschließlich für PASS und die drei Produktfindings erforderlich. Die
`FoundationProjection` bleibt unverändert `NOT_EVIDENCE`. Ein Writer ist ein
getrennter späterer Owner und darf einen finalisierten Record nie
rückwirkend mutieren.
...
Antwortcount `multiple`; der Send-Ledger-Count bleibt davon unverändert. Bei
exakt einem gültigen Send-Ack bleibt das betroffene Operationsergebnis nach der
vorrangigen ADR-0034-Matrix `one/match`. Die Dublette erzeugt allein kein `V`,
setzt aber Antwortprofil und Targetprofil auf mehrdeutig beziehungsweise
`unknown` und hält den endgültigen Ausgang bis `C`
`UNPROVEN/inconclusive`. Eine
...
Zählklasse und Parameterprofil. Die frühere historische Ableitung, nach der
eine fehlende, fehlerhafte oder mehrdeutige Antwort trotz exakt eines gültigen
Send-Acks `one/unproven` ergeben könne, ist durch die totale ADR-0034-Matrix
überholt; der angenommene ADR 0035 bestätigt diese Präzedenz ausdrücklich:

- Intent mit nicht belegbarem Sendestatus: `unknown/unproven`;
- exakt ein gültiger Send-Ack unabhängig von Antwortstatus oder Antwortdublette:
  `one/match`;
- nach `U` oder `V` bewusst nie erzeugtes Downstreamkommando, dessen
  zugehörige Ressource konstruktiv sicher nie aktiviert wurde: `zero/match`;
- kein Intent bei möglicherweise aktiver oder nicht beweisbarer Ressource:
...
1. Die pure netzwerkfreie Effects-as-Data-Foundation
   in einem getrennten Slice implementieren und testen.
...
erhalten. ADR 0035 ist als enge Testbarkeitskorrektur angenommen, ersetzt ADR
...
dokumentarische Adaptergrenze vorgeschlagen; seine K2-Konstruktion ist im
...
Der unabhängige Implementierungsreview und der danach getrennte ADR-0036-
Abgleich gegen neue Load-/Rohhashbindungen und Testverträge stehen aus; deshalb
bleibt ADR 0036 nicht annahmereif. Nach Übergabe ist als Nächstes nur der
gesondert beauftragte Daybreak-xhigh-Implementierungsreview zulässig. Adapterimplementierung,
Adaptertests und Runtimevorgang bleiben geschlossen.
Zielbrowser,
...
Ursache bleibt `CAUSE_NOT_PROVEN`. ADR 0034 ersetzt ADR 0033 formal und
übernimmt alle nicht ausdrücklich korrigierten ADR-0033-/ADR-0032-Regeln; ADR
0033 bleibt mit bytegleichem Hauptteil und ADR 0032 als historische
Entscheidungsebenen erhalten. ADR 0035 ist als enge Pending-Join-
Testbarkeitskorrektur angenommen, ersetzt ADR 0034 formal und führt dessen
...
und mit 595/595 fokussierten Tests geprüft. ADR 0036 schlägt ausschließlich die nach R1–R4
korrigierte dokumentarische Adaptergrenze vor; seine K2-Konstruktion ist im
begrenzten R1–R4-Dokumentationsscope geprüft. Die
`D_K4`-Foundationentscheidung ist durch ADR 0037 angenommen und fokussiert
umgesetzt sowie geprüft. Ihr unabhängiger Implementierungsreview und der
anschließend getrennte ADR-0036-Abgleich gegen neue Load-/Rohhashbindungen und
Testverträge stehen aus; deshalb bleibt ADR 0036 nicht annahmereif.
